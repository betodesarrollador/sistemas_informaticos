<?php
  ...
  class UtilidadesContablesModel extends Db{  
   ...
   public function getEmpresas($Conex){
     ...
     $select = "SELECT e.empresa_id AS value,CONCAT_WS(' ',t.primer_nombre,t.segundo_nombre,t.primer_apellido,t.segundo_apellido,t.razon_social) AS text FROM empresa e,tercero t 
	  ...
	  public function getConsecutivo($oficina_id,$tipo_documento_id,$periodo_contable_id,$Conex){
		  ...
		  if($consecutivo_periodo == 1){
            ...
            if($consecutivo_por == 'E'){
			  ...
			  $select = "SELECT consecutivo FROM encabezado_de_registro WHERE periodo_contable_id = $periodo_contable_id AND tipo_documento_id = $tipo_documento_id";
			  $result = $this -> DbFetchAll($select,$Conex,true);
			  ...
			  if(count($result) > 0){
			   ...
			   return $consecutivo;
				 ...
				 $select = "SELECT consecutivo FROM tipo_de_documento WHERE tipo_documento_id = $tipo_documento_id AND empresa_id = (SELECT empresa_id FROM oficina 
			   ...
			   if($consecutivo > 0){
			     $consecutivo = $consecutivo + 1;
				  ...
				  return $consecutivo;
					...
					$select = "SELECT consecutivo FROM consecutivo_documento_oficina WHERE tipo_documento_id = $tipo_documento_id AND oficina_id = $oficina_id";
					$result = $this -> DbFetchAll($select,$Conex,true);							  
			  		$consecutivo  = $result[0]['consecutivo'];
  ...
  public function getCentroCostosId($oficina_id,$Conex){
  
     $select = "SELECT GROUP_CONCAT(centro_de_costo_id) AS centro_de_costo_id  FROM centro_de_costo ";
     $result = $this -> DbFetchAll($select,$Conex);	 	 
  ...
  public function getDocumentos_sincierre($Conex){
   
     $select = "SELECT tipo_documento_id AS value,nombre AS text FROM tipo_de_documento WHERE de_cierre=0 ORDER BY text";
	 $result = $this -> DbFetchAll($select,$Conex); 
