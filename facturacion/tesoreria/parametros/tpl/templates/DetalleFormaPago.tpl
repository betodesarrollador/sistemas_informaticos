    ...
    <tr>
     ...
     <td>
	   <select name="oficina_id" class="required">
	     <option value="NULL">(... Seleccione ...)</option>		 
		 {foreach name=oficinas from=$OFICINAS item=c}
		 <option value="{$c.value}" {if $c.value eq $d.oficina_id}selected{/if}>{$c.text}</option>
		 {/foreach}
	   </select>
	 </td>     
     <td><a name="saveDetalle"><img src="/envipack/framework/media/images/grid/save.png" /></a></td>
