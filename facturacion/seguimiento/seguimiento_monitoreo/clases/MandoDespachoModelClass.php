...
//// GRID ////
  ...
  public function getQueryMandoTraficoGrid(){
	...
	IF(
	 ...
	 (SELECT IF(TIMESTAMPDIFF(MINUTE,CONCAT_WS(' ',ds.fecha_reporte,ds.hora_reporte),NOW())<(SELECT MIN(p.tiempo_verde) FROM  parametros_reporte_trafico p, detalle_despacho d, remesa r  WHERE d.despachos_urbanos_id=t.despachos_urbanos_id AND r.remesa_id=d.remesa_id AND p.cliente_id=r.cliente_id),'<div class=\"transito\">&nbsp;</div>' ,IF(TIMESTAMPDIFF(MINUTE,CONCAT_WS(' ',ds.fecha_reporte,ds.hora_reporte),NOW())<(SELECT MIN(p.tiempo_amarillo) FROM  parametros_reporte_trafico p, detalle_despacho d, remesa r  WHERE d.despachos_urbanos_id=t.despachos_urbanos_id AND r.remesa_id=d.remesa_id AND p.cliente_id=r.cliente_id),'<div class=\"amarillosem\">&nbsp;</div>',IF((SELECT COUNT(*) FROM  parametros_reporte_trafico p, detalle_despacho d, remesa r  WHERE d.despachos_urbanos_id=t.despachos_urbanos_id AND r.remesa_id=d.remesa_id AND p.cliente_id=r.cliente_id )>0,'<div class=\"pendiente\">&nbsp;</div>','<div>&nbsp;</div>') ) )  AS indic  FROM detalle_seguimiento ds WHERE ds.trafico_id=t.trafico_id AND ds.fecha_reporte IS NOT NULL ORDER BY ds.orden_det_ruta DESC	 LIMIT 0,1),
	 IF(TIMESTAMPDIFF(MINUTE,CONCAT_WS(' ',t.fecha_inicial_salida,t.hora_inicial_salida),NOW())<(SELECT MIN(p.tiempo_verde) FROM  parametros_reporte_trafico p, detalle_despacho d, remesa r  WHERE d.despachos_urbanos_id=t.despachos_urbanos_id AND r.remesa_id=d.remesa_id AND p.cliente_id=r.cliente_id),'<div class=\"transito\">&nbsp;</div>',IF(TIMESTAMPDIFF(MINUTE,CONCAT_WS(' ',t.fecha_inicial_salida,t.hora_inicial_salida),NOW())<(SELECT MIN(p.tiempo_amarillo) FROM  parametros_reporte_trafico p, detalle_despacho d, remesa r  WHERE d.despachos_urbanos_id=t.despachos_urbanos_id AND r.remesa_id=d.remesa_id AND p.cliente_id=r.cliente_id),'<div class=\"amarillosem\">&nbsp;</div>',IF((SELECT COUNT(*) FROM  parametros_reporte_trafico p, detalle_despacho d, remesa r  WHERE d.despachos_urbanos_id=t.despachos_urbanos_id AND r.remesa_id=d.remesa_id AND p.cliente_id=r.cliente_id )>0,'<div class=\"pendiente\">&nbsp;</div>','<div>&nbsp;</div>')))   
																							 ) AS indicador,
	...
	(SELECT s.placa FROM  despachos_urbanos s, oficina o WHERE s.despachos_urbanos_id=t.despachos_urbanos_id AND o.oficina_id=s.oficina_id),'</a>') AS placa,
	(SELECT CONCAT_WS('','<div title=\"Observaciones: ',observaciones,'- Fecha Estimada Salida:',fecha_estimada_salida,'- Hora Estimada Salida:',hora_estimada_salida,'\">','DU No ',despacho,'</div>') AS despa FROM despachos_urbanos WHERE despachos_urbanos_id=t.despachos_urbanos_id) AS trafico_id,		
	...
	(SELECT CONCAT('<div title=\"Movil ',tr.movil,'\">',s.nombre,'</div>') AS nombre FROM  despachos_urbanos s, conductor c, tercero tr, oficina o WHERE s.despachos_urbanos_id=t.despachos_urbanos_id AND o.oficina_id=s.oficina_id AND c.conductor_id=s.conductor_id AND tr.tercero_id=c.tercero_id) AS conductor,
	...
	FROM trafico t WHERE t.estado!='F' AND t.estado!='A' AND t.despachos_urbanos_id>0 AND t.seguimiento_id IS NULL AND t.manifiesto_id IS NULL  ORDER BY (SELECT nombre FROM ubicacion WHERE ubicacion_id=t.destino_id)   ASC)
