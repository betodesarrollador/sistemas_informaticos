...
final class SalidaTurnoModel extends Db{
...
//// GRID ////
  ...
  public function getQuerySalidaTurnoGrid(){	//MODIFICAR FECHA INICIAL Y DIAS
				
	$Query = "SELECT s.enturnamiento_id,
	IF((s.estado='L' AND s.fecha_salida_turno IS NULL),CONCAT('','<a href=\"javascript:void(0)\" onClick=\"viewDoc(',enturnamiento_id,')\">','<input type=\"button\" class=\"btn btn-primary\" value=\"CONFIRMAR SALIDA\">','</a>' ), '')AS enturnamiento,
	(SELECT placa FROM wms_vehiculo WHERE wms_vehiculo_id=s.wms_vehiculo_id)AS wms_vehiculo_id,
	IF((s.muelle_id>0),(SELECT nombre FROM wms_muelle WHERE muelle_id=s.muelle_id),'SIN MUELLE')AS muelle_id,(CASE WHEN s.estado ='D' THEN 'DISPONIBLE' WHEN s.estado ='B' THEN 'BLOQUEADO' WHEN s.fecha_salida_turno IS NULL THEN 'LEGALIZADO' ELSE 'FINALIZADO' END)AS estado,
	(SELECT CONCAT_WS(' ',te.primer_nombre,te.segundo_nombre,te.primer_apellido,te.segundo_apellido,te.razon_social)AS usuario_id FROM tercero te, usuario u WHERE te.tercero_id = u.tercero_id AND u.usuario_id=s.usuario_id)AS usuario_id,
	s.fecha_registro,
	(SELECT CONCAT_WS(' ',te.primer_nombre,te.segundo_nombre,te.primer_apellido,te.segundo_apellido,te.razon_social)AS usuario_id FROM tercero te, usuario u WHERE te.tercero_id = u.tercero_id AND u.usuario_id=s.usuario_actualiza_id)AS usuario_actualiza_id,
	s.fecha_actualiza,
	(SELECT CONCAT_WS(' ',te.primer_nombre,te.segundo_nombre,te.primer_apellido,te.segundo_apellido,te.razon_social)AS usuario_id FROM tercero te, usuario u WHERE te.tercero_id = u.tercero_id AND u.usuario_id=s.usuario_salida_id)AS usuario_salida_id,
	s.fecha_salida_turno,
	(SELECT CONCAT_WS(' ',te.primer_nombre,te.segundo_nombre,te.primer_apellido,te.segundo_apellido,te.razon_social)AS usuario_id FROM tercero te, usuario u WHERE te.tercero_id = u.tercero_id AND u.usuario_id=s.usuario_muelle_id)AS usuario_muelle_id,s.fecha_actualiza_muelle,s.observacion

	
	FROM wms_enturnamiento s"; 
   ...
   public function Save($Campos,$Conex){	
	...
	$this -> assignValRequest('fecha_salida_turno',date('Y-m-d H:i:s'));
  ...
  public function getQuerySalidaTurnoGrid1($Conex){	//MODIFICAR FECHA INICIAL Y DIAS
