...
function suma_cantidad(){
	...
	$("input[name=cant_item_orden_compra]").keyup(function() {	
		var Fila        = $(this).parent().parent();
		var cant 		= $(Fila).find("input[name=cant_item_orden_compra]").val();	
		var valoruni 	= $(Fila).find("input[name=valoruni_item_orden_compra]").val();
	...
	$("input[name=valoruni_item_orden_compra]").keyup(function() {	
		var Fila        = $(this).parent().parent();															   
		var cant 		= $(Fila).find("input[name=cant_item_orden_compra]").val();	
		var valoruni 	= $(Fila).find("input[name=valoruni_item_orden_compra]").val();
		if(cant!='' && cant!=0  && valoruni!='' && valoruni!=0){
...
function saveDetalle(obj){
    ...
    var item_orden_compra_id		= $(Fila).find("input[name=item_orden_compra_id]").val();	
	var orden_compra_id 			= $("#orden_compra_id").val();
	var cant_item_orden_compra  	= $(Fila).find("input[name=cant_item_orden_compra]").val();		
	var desc_item_orden_compra 		= $(Fila).find("input[name=desc_item_orden_compra]").val();	
	var valoruni_item_orden_compra 	= $(Fila).find("input[name=valoruni_item_orden_compra]").val();	
	var remesa_id 					= $(Fila).find("input[name=remesa_id]").val();	
	var checkProcesar          		= $(Fila).find("input[name=procesar]");					
	

				
	if(!item_orden_compra_id.length > 0){

   	  item_orden_compra_id    = 'NULL';
		  
	  var QueryString = "ACTIONCONTROLER=onclickSave&orden_compra_id="+orden_compra_id+"&item_orden_compra_id="+
	  item_orden_compra_id+"&cant_item_orden_compra="+cant_item_orden_compra+"&desc_item_orden_compra="+desc_item_orden_compra+
	  "&valoruni_item_orden_compra="+valoruni_item_orden_compra+"&remesa_id="+remesa_id;	
	  ...
	  $.ajax({
	    ...
	    success    : function(response){
          ...
          if(!isNaN(response)){
            ...
            $(Fila).find("input[name=item_orden_compra_id]").val(response);	
			...
			$(newRow).find("input[name=cant_item_orden_compra]").focus();
	  ...
	  var QueryString = "ACTIONCONTROLER=onclickUpdate&orden_compra_id="+orden_compra_id+"&item_orden_compra_id="+
	  item_orden_compra_id+"&cant_item_orden_compra="+cant_item_orden_compra+"&desc_item_orden_compra="+desc_item_orden_compra+
	  "&valoruni_item_orden_compra="+valoruni_item_orden_compra+"&remesa_id="+remesa_id;;	
...
function deleteDetalle(obj){
    ...
    var Fila                    = obj.parentNode.parentNode;
    var item_orden_compra_id	= $(Fila).find("input[name=item_orden_compra_id]").val();	
    var QueryString             = "ACTIONCONTROLER=onclickDelete&item_orden_compra_id="+item_orden_compra_id;			
