...
final class OrdenModel extends Db{
  ...
  public function selectDatosOrdenId($orden_compra_id,$Conex){
     $select    = "SELECT 
	 				o.orden_compra_id, 
					o.consecutivo,
					...
					b.nombre_bien_servicio AS tiposervicio,
					b.tipo_bien_servicio_id AS tipo_bien_servicio_id,
					o.descrip_orden_compra,
					o.observ_orden_compra,
					o.forma_compra_venta_id ,
					o.estado_orden_compra,
					o.usuario_id,
					o.ingreso_orden_compra,
					o.oficina_id as sucursal_id,
					o.placa_id,
					o.kilometraje,
					(SELECT placa FROM vehiculo WHERE placa_id = o.placa_id)as placa
					FROM orden_compra o, proveedor p, tercero t, tipo_bien_servicio b 
	                WHERE o.orden_compra_id = $orden_compra_id AND p.proveedor_id=o.proveedor_id AND t.tercero_id=p.tercero_id AND b.tipo_bien_servicio_id=o.tipo_bien_servicio_id";
  ...
  public function selectDepartamento($Conex){
	...
	public function getArea($departamento_id,$area_id,$Conex){
		
		$area_id = $area_id >0 ? $area_id : 0;
		$select="SELECT area_id AS value, nombre AS text, IF($area_id>0,$area_id,0) AS selected
	...
	public function getArea1($orden_id,$Conex){
		...
		$select="SELECT area_id AS value, nombre AS text 
			FROM area 
			WHERE departamento_id = (SELECT departamento_id FROM orden_compra WHERE orden_compra_id=$orden_id)";
  ...
  public function liquidar($empresa_id,$oficina_id,$Conex){
	...
	$this -> Begin($Conex);
	  ...
	  $fec_liq_orden_compra    	= $this -> requestDataForQuery('fec_liq_orden_compra','text');
	  $descrip_liq_orden_compra = $this -> requestDataForQuery('descrip_liq_orden_compra','text');
	  $descrip_orden_compra 	= $this -> requestDataForQuery('descrip_orden_compra','text');
	 ...
	 if($movimientos ==0){
		 ...
		 foreach($result_cic as $item_id){
			...
			$insert="INSERT INTO item_liquida_orden (item_liquida_orden_id,orden_compra_id,cant_item_liquida_orden,desc_item_liquida_orden,valoruni_item_liquida_orden,fecha_item_liquida,usuario_id)
					SELECT $item_liquida_orden_id,orden_compra_id,cant_item_orden_compra,desc_item_orden_compra,valoruni_item_orden_compra,fecha_item_orden,usuario_id  FROM item_orden_compra WHERE item_orden_compra_id=$item_id[item_orden_compra_id]"; 
	  ...
	  $select  = "SELECT  c.despuc_bien_servicio,
				...
				WHERE o.orden_compra_id = $orden_compra_id AND c.tipo_bien_servicio_id=o.tipo_bien_servicio_id ORDER BY c.contra_bien_servicio";
	 ...
	 foreach($result as $resultado){
		 ...
		 if(($resultado[porcentaje]=='' || $resultado[porcentaje]==NULL) && $resultado[contra_bien_servicio]!=1){
				$ingresa	= 1;			 
				$parcial	= round($resultado[total]);
				$base		= '';
				$porcentaje	= '';
				$formula	= '';
		
		 }elseif($resultado[porcentaje]>0 && $resultado[contra_bien_servicio]!=1 &&  $resultado[monto]<=$resultado[total] && (($resultado[exento]=='RT' && $resultado[autorete]=='N' ) || ($resultado[exento]=='RIC' && $resultado[retei]=='N') || ($resultado[exento]=='IC' && $resultado[retei]=='N') || ($resultado[exento]=='CR' && $resultado[renta]=='N') || ($resultado[exento]=='NN') )){
			 	 $ingresa	= 1;
				 $base		= round($resultado[total]);
		 ...
		 if($ingresa==1){
			 ...
			 $insert = "INSERT INTO item_puc_liquida_orden (item_puc_liquida_id,orden_compra_id,puc_id,base_item_puc_liquida,porcentaje_item_puc_liquida,formula_item_puc_liquida,desc_item_puc_liquida,deb_item_puc_liquida,cre_item_puc_liquida,contra_liquida_orden,centro_de_costo_id,area_id,departamento_id,unidad_negocio_id,sucursal_id)
						VALUES ($item_puc_liquida_id,$orden_compra_id,$resultado[puc_id],'$base','$porcentaje','$formula','$resultado[descrip_orden_compra]','$debito','$credito',$resultado[contra_bien_servicio],$resultado[centro_de_costo_id],$resultado[area_id],$resultado[departamento_id],$resultado[unidad_negocio_id],$resultado[sucursal_id])";
						//exit($insert);
			 $this -> query($insert,$Conex,true);			
		 }
	 }

	  $update = "UPDATE orden_compra SET liq_usuario_id=$liq_usuario_id, fec_liq_orden_compra=$fec_liq_orden_compra, estado_orden_compra='L', descrip_liq_orden_compra=$descrip_liq_orden_compra 
	  				WHERE orden_compra_id=$orden_compra_id AND estado_orden_compra='A'"; 
  ...
  public function Checkconfig($orden_compra_id,$empresa_id,$oficina_id,$Conex){
	 ...
	 foreach($result as $resultado){
		 ...
		 }elseif($resultado[porcentaje]>0 && $resultado[contra_bien_servicio]!=1 &&  $resultado[monto]<=$resultado[total] && (($resultado[exento]=='RT' && $resultado[autorete]=='N' ) || ($resultado[exento]=='IC' && $resultado[retei]=='N') || ($resultado[exento]=='RIC' && $resultado[retei]=='N') || ($resultado[exento]=='CR' && $resultado[renta]=='N')  || ($resultado[exento]=='NN') )){
  ...
  public function getTotal($orden_compra_id,$Conex){
	  
	  $select = "SELECT SUM(valoruni_item_orden_compra) AS subtotal,SUM(valoruni_item_orden_compra*cant_item_orden_compra) AS total FROM item_orden_compra  WHERE orden_compra_id=$orden_compra_id";
  ...
  public function Save($Campos,$Conex){	
    ...
    $this -> Begin($Conex);
	  ...
	  $orden_compra_id    	= $this -> DbgetMaxConsecutive("orden_compra","orden_compra_id",$Conex,true,1);
  	  $fecha_orden_compra 	= $this -> requestDataForQuery('fecha_orden_compra','date');
	  $descrip_orden_compra = $this -> requestDataForQuery('descrip_orden_compra','text');
	  $observ_orden_compra 	= $this -> requestDataForQuery('observ_orden_compra','text');
      ...
      $ingreso_orden_compra	= $this -> requestDataForQuery('ingreso_orden_compra','date');
	  ...
	  $select_con = "SELECT (MAX(consecutivo)+1) AS consecut FROM orden_compra  WHERE 	oficina_id = $oficina_id";	  
	  $result_con = $this -> DbFetchAll($select_con,$Conex,true); 
	  $consecutivo = $result_con[0]['consecut']>0 ?$result_con[0]['consecut']:1;

 	  $item_pre_orden_id	= $this -> requestDataForQuery('item_pre_orden_id','text');
	  $item_pre_orden_id	=	str_replace("'","",$item_pre_orden_id);
	 
	  $insert = "INSERT INTO orden_compra (orden_compra_id,consecutivo,fecha_orden_compra,descrip_orden_compra,observ_orden_compra,centro_de_costo_id,proveedor_id,tipo_bien_servicio_id,forma_compra_venta_id,estado_orden_compra,usuario_id,ingreso_orden_compra,oficina_id,area_id,departamento_id,unidad_negocio_id,sucursal_id,placa_id,kilometraje,item_pre_orden_id) 
	  				VALUES ($orden_compra_id,$consecutivo,$fecha_orden_compra,$descrip_orden_compra,$observ_orden_compra,$centro_de_costo_id,$proveedor_id,$tipo_bien_servicio_id,$forma_compra_venta_id,$estado_orden_compra,$usuario_id,$ingreso_orden_compra,$oficina_id,$area_id,$departamento_id,$unidad_negocio_id,$sucursal_id,$placa_id,$kilometraje,$item_pre_orden_id)"; 
			//echo $insert;		
	  $this -> query($insert,$Conex,true);
	  
	  
	  //SI VIENE DE UNA SOLICITUD INGRESAMOS LOS DETALLES .......
	 
	  if(strlen($item_pre_orden_id)>0){
		  $insert = "INSERT INTO item_orden_compra 
	            (cant_item_orden_compra,desc_item_orden_compra,valoruni_item_orden_compra,orden_compra_id,fecha_item_orden,usuario_id) 
	            SELECT 
				  
				  cant_item_pre_orden_compra as cant_item_orden_compra,
				  desc_item_pre_orden_compra as desc_item_orden_compra,
				  valoruni_item_pre_orden_compra as valoruni_item_orden_compra,
				  $orden_compra_id as orden_compra_id,
				  '".date('Y-m-d H:m')."' as fecha_item_orden,
				  $usuario_id as usuario_id
				  FROM 
				  item_pre_orden_compra WHERE item_pre_orden_compra_id IN ($item_pre_orden_id)
				";
      	$this -> query($insert,$Conex,true);
		
		$update = "UPDATE item_pre_orden_compra SET estado = 'P' WHERE item_pre_orden_compra_id IN ($item_pre_orden_id)";
		$this -> query($update,$Conex,true);
	  ...
	  if(!strlen(trim($this -> GetError())) > 0){
	  	$this -> Commit($Conex);		 
  	  	return array(orden_compra_id=>$orden_compra_id,consecutivo=>$consecutivo);
	  }
  }
  ...
  public function Update($Campos,$Conex){	
    $this -> Begin($Conex);
	  ...
	  $tipo_bien_servicio_id= $this -> requestDataForQuery('tipo_bien_servicio_id','integer');
	  $forma_compra_venta_id= $this -> requestDataForQuery('forma_compra_venta_id','integer');
	  $estado_orden_compra	= $this -> requestDataForQuery('estado_orden_compra','alphanum');
	  $usuario_id           = $this -> requestDataForQuery('usuario_id','integer');
	  $oficina_id           = $this -> requestDataForQuery('oficina_id','integer');
      $area_id           	= $this -> requestDataForQuery('area_id','integer');
	  $departamento_id      = $this -> requestDataForQuery('departamento_id','integer');
	  ...
	  if($_REQUEST['orden_compra_id'] == 'NULL'){
		  $insert = "INSERT INTO orden_compra (orden_compra_id,fecha_orden_compra,descrip_orden_compra,observ_orden_compra,centro_de_costo_id,proveedor_id,tipo_bien_servicio_id,forma_compra_venta_id,estado_orden_compra,usuario_id,ingreso_orden_compra,oficina_id,area_id,departamento_id,unidad_negocio_id,sucursal_id) 
						VALUES ($orden_compra_id,$fecha_orden_compra,$descrip_orden_compra,$observ_orden_compra,$centro_de_costo_id,$proveedor_id,$tipo_bien_servicio_id,$forma_compra_venta_id,$estado_orden_compra,$usuario_id,i$ngreso_orden_compra,$oficina_id,$area_id,$departamento_id,$unidad_negocio_id,$sucursal_id)"; 
		  $this -> query($insert,$Conex,true);
		  ...
		  $update = "UPDATE orden_compra SET fecha_orden_compra= $fecha_orden_compra,
						descrip_orden_compra = $descrip_orden_compra,
						observ_orden_compra = $observ_orden_compra,
  ...
  public function cancellation($Conex){
	...
	$this -> Begin($Conex);
      ...
      $anul_orden_compra    	= $this -> requestDataForQuery('anul_orden_compra','text');
	  $desc_anul_orden_compra   = $this -> requestDataForQuery('desc_anul_orden_compra','text');
	  $anul_usuario_id          = $this -> requestDataForQuery('anul_usuario_id','integer');	
	  
	  $update = "UPDATE orden_compra SET estado_orden_compra= 'I',
	  				causal_anulacion_id = $causal_anulacion_id,
					anul_orden_compra=$anul_orden_compra,
					desc_anul_orden_compra =$desc_anul_orden_compra,
  ...
  public function selectEstadoEncabezadoRegistro($orden_compra_id,$Conex){
	  
    $select = "SELECT estado_orden_compra FROM orden_compra  WHERE 	orden_compra_id = $orden_compra_id";	  
	$result = $this -> DbFetchAll($select,$Conex,true); 
	$estado = $result[0]['estado_orden_compra'];
	
	return $estado;	  
	  
  }

  public function selectItemliquida($orden_compra_id,$Conex){
	  
    $select = "SELECT COUNT(*) AS movimientos FROM  item_orden_compra   WHERE 	orden_compra_id = $orden_compra_id";	  
	$result = $this -> DbFetchAll($select,$Conex,true); 
   ...
   public function ValidateRow($Conex,$Campos){
	 ...
	 $Data = new ValidateRow($Conex,"orden_compra",$Campos);
	 return $Data -> GetData();
   }
   ...
   public function GetQueryOrdenGrid(){
     ...
     $Query    	= "SELECT 
					 CONCAT('','<a href=\"javascript:void(0)\" onClick=\"viewDoc(',orden_compra_id,')\">',o.consecutivo,'</a>') AS orden_compra_id,
					...
					FROM orden_compra o, proveedor p, tercero t, tipo_bien_servicio b, forma_compra_venta f 
	                WHERE p.proveedor_id=o.proveedor_id AND t.tercero_id=p.tercero_id AND b.tipo_bien_servicio_id=o.tipo_bien_servicio_id AND f.forma_compra_venta_id =o.forma_compra_venta_id ORDER BY o.fecha_orden_compra DESC LIMIT 2000 ";
   return $Query;
   }
}
