...
final class DetalleDevolucionProveedorModel extends Db{
   ...
   public function getAnticiposPlaca($proveedor_id,$Conex){
	 ...
	 FROM anticipos_proveedor a WHERE a.proveedor_id = $proveedor_id  AND devolucion=1  ORDER BY a.anticipos_proveedor_id desc";
   ...
   public function getAnticiposPlacaCruce($proveedor_id,$Conex){
   
     $select    = "SELECT a.*
	 FROM anticipos_proveedor a WHERE a.proveedor_id = $proveedor_id  AND devolucion=0  ORDER BY a.anticipos_proveedor_id desc";
     $anticipos = $this -> DbFetchAll($select,$Conex,true);
   ...
   public function getRegDataplaca($proveedor_id,$Conex){
     ...
     $select    = "SELECT (MAX(numero)+1) AS numeros FROM anticipos_proveedor WHERE proveedor_id = $proveedor_id AND devolucion=1 ";
     $anticipos_num = $this -> DbFetchAll($select,$Conex,true);
  ...
  public function Delete($empresa_id,$oficina_id,$modifica,$usuario_id,$Conex){
    $this -> Begin($Conex);
	 ...
	 if(!$movimientos1>0){
		 $delete="DELETE FROM   anticipos_proveedor WHERE anticipos_proveedor_id=$anticipos_proveedor_id ";
  ...
  public function save($empresa_id,$oficina_id,$modifica,$usuario_id,$Conex){
    ...
    $this -> Begin($Conex);
	 ...
	 $sub_anticipos_proveedor_id  = $this -> requestData('sub_anticipos_proveedor_id');
	 ...
	 $select_totaldev     = "SELECT SUM(valor) AS valor_devolucion  FROM anticipos_proveedor 
	 WHERE sub_anticipos_proveedor_id = $sub_anticipos_proveedor_id AND estado!='A' AND devolucion=1";
     $result_totaldev     = $this -> DbFetchAll($select_totaldev,$Conex,true);	
	 $valor_devolucion=$result_totalant[0]['valor_devolucion']+$valor;
	 
	 $select_totalant     = "SELECT valor  FROM anticipos_proveedor 
	 WHERE anticipos_proveedor_id = $sub_anticipos_proveedor_id AND estado!='A' AND devolucion=0";
	...
	$concepto                =  "ANTICIPO :  VEHICULO PLACA  $placa " ;
	$estado                  = "C";
	...
	if($consecutivo!=''){
		$select1 = "SELECT * FROM encabezado_de_registro WHERE consecutivo = $consecutivo AND tipo_documento_id=(SELECT tipo_documento_id FROM encabezado_de_registro WHERE encabezado_registro_id = (SELECT encabezado_registro_id FROM anticipos_proveedor WHERE anticipos_proveedor_id = $sub_anticipos_proveedor_id))"; 
		$result1 = $this -> DbFetchAll($select1,$Conex,true);			
		if(count($result1) > 0){
	...
	$select = "(SELECT tipo_documento_id FROM encabezado_de_registro WHERE encabezado_registro_id = (SELECT encabezado_registro_id FROM anticipos_proveedor WHERE anticipos_proveedor_id = $sub_anticipos_proveedor_id))"; 
	    $result = $this -> DbFetchAll($select,$Conex,true);		
   ...
   (SELECT tercero_id FROM proveedor WHERE proveedor_id = $proveedor_id),$periodo_contable_id,$mes_contable_id,$consecutivo,'$fecha_egreso','$concepto','$estado','$fecha_registro','$modifica',$usuario_id,
   ...
   $select = "SELECT * FROM parametros_anticipo_proveedor WHERE parametros_anticipo_proveedor_id = $parametros_anticipo_proveedor_id"; 
	...
	if(!count($centro_de_costo_id) > 0){
	 ...
	 if($utilidadesContables -> requiereTercero($puc_id,$Conex)){
	  ...
	  $select = "SELECT * FROM tercero WHERE tercero_id = (SELECT tercero_id FROM proveedor WHERE proveedor_id = $proveedor_id)";
	  $result = $this  -> DbFetchAll($select,$Conex,true);			   
    ...
    $descripcion = "DEVOLUCION $numero PLACA $placa";	
   ...
   $insert = "INSERT INTO  anticipos_proveedor (anticipos_proveedor_id,consecutivo,fecha,oficina_id,proveedor_id,numero,proveedor,valor,estado,forma_pago_id,cuenta_tipo_pago_id,encabezado_registro_id,numero_soporte,fecha_egreso,observaciones,devolucion,sub_anticipos_proveedor_id,parametros_anticipo_proveedor_id)VALUES 
   ($anticipos_proveedor_id,$consecutivo,'$fecha_egreso',$oficina_id,$proveedor_id,$numero,'$nombre',$valor,'P',$forma_pago_id,$cuenta_tipo_pago,$encabezado_registro_id,'$numero_soporte','$fecha_registro','$observaciones',1,$sub_anticipos_proveedor_id,$parametros_anticipo_proveedor_id)";
   ...
   $this -> query($insert,$Conex,true);			  		
	...
	$select = "SELECT * FROM tipo_de_documento WHERE tipo_documento_id = (SELECT tipo_documento_id FROM encabezado_de_registro WHERE encabezado_registro_id = (SELECT encabezado_registro_id FROM anticipos_proveedor WHERE anticipos_proveedor_id = $sub_anticipos_proveedor_id))";
    $result = $this -> DbFetchAll($select,$Conex,true);		
