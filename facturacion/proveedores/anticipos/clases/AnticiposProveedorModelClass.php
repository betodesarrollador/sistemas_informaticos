...
final class AnticiposProveedorModel extends Db{

  
  
  public function TieneAnticiposProveedor($proveedor_id,$Conex){
  
    $select = "SELECT * FROM vehiculo WHERE proveedor_id = $proveedor_id";
  ...
  public function selectTenedor($tenedor_id,$Conex){
    ...
    public function selectVehiculo($proveedor_id,$Conex){
	...
	FROM vehiculo v WHERE proveedor_id = $proveedor_id ";	  
	...
	if($tecnicomecanica_vencio == 'SI' || $soat_vencio == 'SI'){
	  ...
	  $update = "UPDATE vehiculo SET estado = 'B' WHERE proveedor_id = $proveedor_id";
	  $result = $this -> query($update,$Conex,true);
