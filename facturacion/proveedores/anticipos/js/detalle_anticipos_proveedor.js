...
//	  var conductor_id                 = parent.document.getElementById("conductor_hidden").value;
	  ...
	  if(forma_pago_id == 'NULL' || cuenta_tipo_pago == 'NULL'){
	      ...
	      var QueryString = "ACTIONCONTROLER=onclickSave&anticipos_proveedor_id="+anticipos_proveedor_id+"&numero="+numero+"&consecutivo="+consecutivo+"&nombre="+nombre+"&fecha_egreso="+fecha_egreso+"&forma_pago_id="
		  +forma_pago_id+"&numero_soporte="+numero_soporte+"&cuenta_tipo_pago="+cuenta_tipo_pago+"&valor="+valor+"&proveedor_id="+proveedor_id
		  +"&observaciones="+observaciones+"&parametros_anticipo_proveedor_id="+parametros_anticipo_proveedor_id;	      				      
		  
	      $.ajax({
		      url        : "DetalleAnticiposProveedorClass.php?rand="+Math.random(),
		      data       : QueryString,
		      beforeSend : function(){
			    showDivLoading();
		      },
		      success    : function(resp){
				  ...
				  try{
					 ...
					 if(!isNaN(encabezado_registro_id)){
					   ...
					   parent.document.getElementById("frameRegistroContable").src = "AnticiposProveedorClass.php?"+QueryString+"&rand="+Math.random();
	  ...
	  $.ajax({
		  ...
		  success    : function(resp){
			  ...
			  try{
					   removeDivLoading();
					   parent.recargar_anticipos(proveedor_id);
					   alertJquery(resp,"Anulacion Ant. Placa");					
			  }catch(e){
				  removeDivLoading();
				  alertJquery(resp,"Error : "+e);
				  return false;
	...
	if(parent.document.getElementById("borrar")){
	  ...
	  $.ajax({
		  ...
		  success    : function(resp){
			  ...
			  try{
					   removeDivLoading();
					   alertJquery(resp,"Eliminacion Ant. Placa");
					   if(resp=='Anticipo Eliminado Exitosamente')
					   parent.recargar_anticipos(proveedor_id);
