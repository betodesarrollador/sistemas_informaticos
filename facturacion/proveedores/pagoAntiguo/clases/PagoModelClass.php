...
final class PagoModel extends Db
{
    ...
    {
        return $this->DbFetchAll("SELECT tipo_documento_id  AS value,nombre AS text FROM tipo_de_documento  WHERE tipo_documento_id IN(17,15)", $Conex,
            $ErrDb = false);
    }
    ...
    public function Save($empresa_id, $Campos, $Conex)
    {
        ...
        $insert = "INSERT INTO abono_factura_proveedor (abono_factura_proveedor_id,cuenta_tipo_pago_id,num_cheque,tipo_documento_id,proveedor_id,fecha,valor_abono_factura,concepto_abono_factura,causaciones_abono_factura,valores_abono_factura,usuario_id,oficina_id,ingreso_abono_factura,estado_abono_factura)
		VALUES ($abono_factura_proveedor_id,$cuenta_tipo_pago_id,$num_cheque,$tipo_documento_id,$proveedor_id,$fecha,'$valor_tot_pago',$concepto_abono_factura,$causaciones_abono_factura,$valores_abono_factura,$usuario_id,$oficina_id,$ingreso_abono_factura,$estado_abono_factura)";
        ...
        foreach ($causacion_id as $causaciones) {
            ...
            if ($causaciones > 0) {
                ...
                if ($valor_pago_ind > 0) {
					...
					VALUES ($relacion_abono_factura_id,$causaciones,$abono_factura_proveedor_id,'$valor_pago_ind')";
	...
	$mes_contable_id,$consecutivo,'$fecha','$concepto',$puc_id,'C','$fecha_registro','$modifica',$usuario_id,'$numero_documento_fuente',$id_documento_fuente)";
		...
		formula_abono_factura,cre_item_abono_factura,deb_item_abono_factura,sucursal_id
    ...
    public function getTotalDebitoCredito($abono_factura_proveedor_id, $Conex)
    {
        ...
        $resultv = $this->DbFetchAll($selectv, $Conex, true);
	...
	FROM abono_factura_proveedor a WHERE a.abono_factura_proveedor_id=$abono_factura_proveedor_id";
        ...
        foreach ($result_item as $result_items) {
			...
			formula_abono_factura,deb_item_abono_factura,cre_item_abono_factura,area_id,departamento_id,unidad_negocio_id,sucursal_id
			FROM item_abono_factura WHERE abono_factura_proveedor_id=$abono_factura_proveedor_id AND item_abono_factura_id=$result_items[item_abono_factura_id]";
    ...
    public function GetQueryPagoGrid()
    {
        ...
        $Query = "SELECT CONCAT('','<a href=\"javascript:void(0)\" onClick=\"viewDoc(',abono_factura_proveedor_id,')\">',IF((a.encabezado_registro_id)>0,(SELECT consecutivo FROM encabezado_de_registro WHERE encabezado_registro_id=a.encabezado_registro_id), 'VER'),'</a>') AS consecutivo,
		a.fecha, a.ingreso_abono_factura,
		...
		WHERE p.proveedor_id=a.proveedor_id ORDER BY a.fecha DESC LIMIT 0,800";
