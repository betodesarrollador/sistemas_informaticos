      ...
      <table align="center" id="tableDetalles" width="98%">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td name="numero_factura_proveedor">{$i.consecutivo_id}</td>
            <td>{$i.tipo}</td>
            <td>{$i.fecha}</td>
            <td>{$i.vencimiento}</td>
            ...
            {foreach name=descuentos from=$DESCUENTOS item=j}     
            	<td>
                	...
                	<input type="hidden" name="descuento_id{$incremen}" id="descuento_id{$incremen}"  value="{$j.parametros_descuento_factura_proveedor_id}"  />
