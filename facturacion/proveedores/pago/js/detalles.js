...
function autocompleteCodigoContable(){
	...
	$("input[name=puc]").result(function(event, data, formatted) {				   
		...
		if (data){
			...
			if($(this.parentNode.parentNode).find("input[name=descripcion]").val()!='' ){
				var comodin='';
			}else{
  ...
  $.ajax({
	...
	success    : function(response){
      ...
      try{
		...
		if(fila.find("input[name=tercero]").val()=='' || fila.find("input[name=tercero]").val()=='null' || fila.find("input[name=tercero]").val()==null){
			fila.find("input[name=tercero]").val("");			
			fila.find("input[name=tercero_id]").val("");											  		
		}
		if(fila.find("input[name=centro_de_costo]").val()=='' || fila.find("input[name=centro_de_costo]").val()=='null' || fila.find("input[name=centro_de_costo]").val()==null){
		...
		if($.trim(require_base) == "true"){
          ...
          fila.find("input[name=deb_item_abono_factura]").attr("readonly","true");					  
          fila.find("input[name=cre_item_abono_factura]").attr("readonly","true");					  		  	  
        }else{
            ...
            fila.find("input[name=deb_item_abono_factura]").attr("readonly","");					  
            fila.find("input[name=cre_item_abono_factura]").attr("readonly","");			
		  }		  
		...
		success : function(response){
          ...
          try{
		   ...
		   if($.trim(datosCalculo['naturaleza']) == 'D'){
		      ...
		      $(objBase).parent().parent().find("input[name=cre_item_abono_factura]").val("");			   			  
           }else{
			    $(objBase).parent().parent().find("input[name=deb_item_abono_factura]").val("");			   
...
function saveDetalle(obj){
	...
	/*if((parseInt(deb_item_abono_factura) > 0 || parseInt(cre_item_abono_factura))&& saldo>=deb_item_abono_factura &&  saldo>=cre_item_abono_factura){*/
	  ...
	  "&base_abono_factura="+base_abono_factura+"&deb_item_abono_factura="+deb_item_abono_factura+"&cre_item_abono_factura="+cre_item_abono_factura;	
...
function saveDetalleNuevo(obj){
	...
	cre_item_abono_factura 		= cre_item_abono_factura.length > 0 ? cre_item_abono_factura: 'NULL';
	  ...
	  item_abono_factura_id+"&puc_id="+puc_id+"&tercero_id="+tercero_id+"&centro_de_costo_id="+centro_de_costo_id+"&desc_abono_factura="+desc_abono_factura+
	  "&base_abono_factura="+base_abono_factura+"&deb_item_abono_factura="+deb_item_abono_factura+"&cre_item_abono_factura="+cre_item_abono_factura+"&relacion_abono_factura_id="+relacion_abono_factura_id+"&abono_factura_proveedor_id="+abono_factura_proveedor_id;	
