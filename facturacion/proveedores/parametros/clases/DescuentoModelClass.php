...
final class DescuentoModel extends Db{
  ...
  public function selectDatosDescuentoId($parametros_descuento_proveedor_id,$Conex){
     $select    = "SELECT p.*,
	 				(SELECT CONCAT_WS('-',codigo_puc,nombre) FROM puc WHERE puc_id=p.puc_id) AS puc
					FROM parametros_descuento_proveedor  p
	                WHERE p.parametros_descuento_proveedor_id = $parametros_descuento_proveedor_id";
  ...
  public function Save($Campos,$Conex){	
    ...
    $this -> Begin($Conex);
	  ...
	  $parametros_descuento_proveedor_id    = $this -> DbgetMaxConsecutive("parametros_descuento_proveedor","parametros_descuento_proveedor_id",$Conex,true,1);
	
      $this -> assignValRequest('parametros_descuento_proveedor_id',$parametros_descuento_proveedor_id);
      $this -> DbInsertTable("parametros_descuento_proveedor",$Campos,$Conex,true,false);  
	  $this -> Commit($Conex);  
  ...
  public function Update($Campos,$Conex){	
    $this -> Begin($Conex);
	  if($_REQUEST['parametros_descuento_proveedor_id'] == 'NULL'){
	    $this -> DbInsertTable("parametros_descuento_proveedor",$Campos,$Conex,true,false);			
      }else{
          $this -> DbUpdateTable("parametros_descuento_proveedor",$Campos,$Conex,true,false);
	    }
	$this -> Commit($Conex);
  }
  
  public function Delete($Campos,$Conex){
  	$this -> DbDeleteTable("parametros_descuento_proveedor",$Campos,$Conex,true,false);
  }	
		
   public function ValidateRow($Conex,$Campos){
	 require_once("../../../framework/clases/ValidateRowClass.php");
	 $Data = new ValidateRow($Conex,"parametros_descuento_proveedor",$Campos);
	 return $Data -> GetData();
   }
