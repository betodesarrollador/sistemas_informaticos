...
<html><head>
  ...
  <fieldset> <legend>{$TITLEFORM}</legend>
      ...
      <table>
        <tbody>
          <tr>
            ...
            <td>{$BUSQUEDA}{$ENTURNAMIENTO_ID}</td>
            <td align="center"><h6><font color="#FF0000"><b>DESPACHOS NACIONALES&nbsp;&nbsp;&nbsp;</b></font></h6></td>   
            <td class="borderTop" align="center">
              <input type="button" onclick="irRemesa();" class="btn btn-primary" value="Crear Remesa" />&nbsp;&nbsp;&nbsp;
            </td>  
            <td class="borderTop" align="center">
              <input type="button" onclick="irOrdenCargue();" class="btn btn-primary" value="Crear Orden Cargue" />
            </td>  
          </tr>
        </tbody>
      </table>
    ...
    <fieldset> 
      ...
      {$USUARIOID}{$USUARIOREGISTRA}{$USUARIONUMID}{$IDMOBILE}
      <table align="center">
        <tbody>
          <tr>
            <td align="center">
              <fieldset class="section">
               <legend>MANIFIESTO CARGA</legend>
               <table align="center">
                <tbody>
                  <tr>
                    <td><label>Manifiesto No. : </label></td>
                    <td>{$MANIFIESTO}{$MANIFIESTOID}{$SERVICIOTRANSPORTE}{$UPDATEMANIFIESTO}{$FECHAREGISTRO}{$HORAREGISTRO}</td>
                    <td><label>Tipo Operaci&oacute;n : </label></td>
                    <td>{$TIPOMANIFIESTO}{$EMPRESAID}{$OFICINAID}</td>
                    <td><label>Fecha Manifiesto : </label></td>
                  ...
                  <tr>
                    <td valign="top"><label>Contenedor : </label></td>
                    <td valign="top">{$CONTENEDOR}</td>
                    <td valign="top"><label>Precinto :</label></td>
                    <td valign="top">{$NUMEROPRECINTO}</td>
                  </tr>
                  <tr>
                    <td valign="top"><label>Fecha Estimada Salida : </label></td>
                    <td valign="top">{$FECHASALIDA}</td>
                    <td valign="top"><label>Hora Estimada Salida : </label></td>
                    <td valign="top">{$HORASALIDA}</td>
                    <td valign="top"><label>Fecha Estimada Entrega : </label></td>
                    <td valign="top">{$FECHAENTREGA}</td>
                  </tr>
                  <tr>
                    <td valign="top"><label>Hora Estimada Entrega : </label></td>
                    <td valign="top">{$HORAENTREGA}</td>
                    <td valign="top"><label>Modalidad :</label></td>
                  ...
                  <tr>
                    ...
                    <td rowspan="2" valign="top">{$OBSERVACIONES}</td>
                    <td valign="top"><label>Direcciones de Entrega : </label></td>
                    <td rowspan="2" valign="top">{$DIR_ENTREGAS}</td>
                  </tr>
                  <tr>
                    <td valign="top">&nbsp;</td>
                    <td valign="top">&nbsp;</td>
                  ...
                  <tr>
                    <td valign="top"><label>Numero Aprobacion : </label></td>
                    <td valign="top">{$REPORTADOMIN}</td>
                    <td valign="top"><label>Numero Cumplido Min : </label></td>
            ...
            <table align="center" id="tableDataConductor">
              <tbody>
                <tr>
                  ...
                  <td>{$NUMEROIDENTIFICACION}{$NUMEROIDENTIFICACIONCONDUCTORCODIGO}</td>
      ...
      <tr>
        <td colspan="6" align="center">
          ...
          <fieldset class="section">
           <legend>Datos Titular Manifiesto</legend>
           <table>
            <tr>
...
<tr>
 <td>
   <fieldset class="section" id="remesasManifestar"> 
     ...
     <iframe id="detalleManifiesto" src=""></iframe> 
   </fieldset>
 </td>
</tr>
<tr>
  <td>
   <div id="messages">&nbsp;</div>
   <fieldset class="section" id="tiemposManifestar">
    <legend>TIEMPOS CARGUE</legend>
    <iframe id="tiemposManifiesto"></iframe>
  </fieldset>
</td>
</tr>
<tr>
  ...
  <fieldset class="section"  id="anticipos"> 
    ...
    <table id="tableAnticipos">
      <tr>
       ...
       <td>
        <input type="hidden" name="anticipo" id="anticipos_manifiesto_id" value="" />
...
<fieldset class="section"  id="fletePactado"> 
  ...
  <table id="tableFletePactado">
   ...
   <td colspan="4">
     ...
     <input type="text" name="impuesto" id="porcentaje" value=""  />                              
 ...
 <tr id="ica" >
  <td>
    <input type="hidden" name="impuesto1" id="impuestos_manifiesto_id" value="" />
    <select name="impuesto1" id="impuesto_id" class="required">
      <option value="">Seleccione</option>
      {foreach name=impuestos1 from=$IMPUESTOS1 item=i}
      <option value="{$i.impuesto_id}">{$i.nombre}</option>
      {/foreach}  
    </select>  
  </td>
  <td colspan="4">
   <input type="hidden" name="impuesto1" id="base" value=""  /> 
   <input type="text" name="impuesto1" id="porcentaje" value=""  />                             
...
<div align="center" id="divToolBarButtons">{$MANIFESTAR}&nbsp;{$ACTUALIZAR}&nbsp;{$IMPRIMIR}&nbsp;{$ANULAR}&nbsp;{$CAUSAR}<span style="display: none">{$LIQUIDAR}&nbsp;{$LIQUIDAR1}</span> {$LIMPIAR}</div>
