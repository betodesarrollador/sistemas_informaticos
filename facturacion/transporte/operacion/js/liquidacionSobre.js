...
function calcularFlete(afterCalculation,objId,formulario){
	...
	$.ajax({
	  url        : "LiquidacionSobreClass.php?rand="+Math.random(),
	  data       : QueryString,
	  beforeSend : function(){
		  showDivLoading();
	  },
...
function beforePrint(formulario,url,title,width,height){
	...
	if(isNaN(liquidacion_despacho_sobre_id)){
	  ...
	  alertJquery('Debe seleccionar un Manifiesto a imprimir !!!','Impresion Liquidacion Sobre Flete');
	  return false;
...
function Contabilizar(formulario){
	...
	if(parseInt(liquidacion_despacho_sobre_id)>0){
		...
		$.ajax({
		 ...
		 success  : function(resp){
			 //$("#refresh_QUERYGRID_LiquidacionDescuManifiestos").click();
			 if($.trim(resp)=='true'){
...
function setDataFormWithResponse(liquidacion_despacho_sobre_id){
	...
	$.ajax({
      ...
      success    : function(resp){
	    ...
	    $("#concepto").val("LIQUIDACION SOBRE FLETE MC: ");
		...
		try{
		 ...
		 var Tabla           = document.getElementById('tableLiquidacionSobre');
...
function LiquidacionSobreManifiestoOnSaveOnUpdateonDelete(formulario,resp){

   LiquidacionSobreManifiestoOnReset(formulario);
...
function LiquidacionSobreManifiestoOnReset(formulario){
	...
	$("#refresh_QUERYGRID_LiquidacionSobreManifiestos").click();
			 ...
			 success    : function(resp){
			   ...
			   try{
				 ...
				 removeDivLoading();
				  ...
				  QueryString  = "ACTIONCONTROLER=getLiquidacionSobreManifiesto&manifiesto_id="+manifiesto_id;
					...
					success    : function(resp){
					  ...
					  try{
						 ...
						 if($.trim(resp).length == 0 || resp == 'null'){
							...
							alertJquery("<p align='center'>Este manifiesto no tiene anticipos !! <br> No se puede legalizar</p>","Validacion LiquidacionSobre"); 
							Reset(formulario);
							return true;
...
function onclickSave(objId,formulario){
	  ...
	  var QueryString       = '';	
	  var tableLiquidacionSobre  = document.getElementById('tableLiquidacionSobre');
	  ...
	  QueryString = 'manifiesto_id='+manifiesto_id+'&manifiesto='+manifiesto+'&liquidacion_despacho_sobre_id='+liquidacion_despacho_sobre_id+'&encabezado_registro_id='+encabezado_registro_id
	  ...
	  $(tableLiquidacionSobre).find(".rowImpuestos").each(function(){			
		   ...
		   $.ajax({
			 ...
			 success  : function(resp){				
				 ...
				 if(!isNaN(liquidacion_despacho_sobre_id)){
					 ...
					 var url = "LiquidacionSobreClass.php?ACTIONCONTROLER=onclickPrint&liquidacion_despacho_sobre_id="+liquidacion_despacho_sobre_id+"&rand="+Math.random();
					 
					 popPup(url,10,900,600);
					 
//					 document.location.href = "LiquidacionSobreDespachosClass.php?"+QueryString;
				...
				// alertJquery(resp,"LiquidacionSobre");				 
				 removeDivLoading();	
                 LiquidacionSobreManifiestoOnSaveOnUpdateonDelete(formulario);				 
		     }
