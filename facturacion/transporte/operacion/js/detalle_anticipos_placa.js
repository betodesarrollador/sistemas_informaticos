...
function generarAnticipoPlaca(obj){
	...
	if(validaRequeridosDetalle(obj,row)){
	 ...
	 // var tenedor_id                   = $(Fila).find("input[name=tenedor_id]").val();
	  ...
	  var a_conductor                  = $(Fila).find("input[name=a_conductor]").val();
	  ...
	  if(forma_pago_id == 'NULL' || cuenta_tipo_pago == 'NULL'){
		  ...
		  +"&tenedor_id="+tenedor_id+"&observaciones="+observaciones+"&tipo_doc="+tipo_doc+"&a_conductor="+a_conductor;	      				      
		  
	      $.ajax({
		      url        : "DetalleAnticiposPlacaClass.php?rand="+Math.random(),
		      data       : QueryString,
		      beforeSend : function(){
			    showDivLoading();
		      },
