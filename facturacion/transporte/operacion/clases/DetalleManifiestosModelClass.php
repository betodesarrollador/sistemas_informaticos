...
final class DetalleManifiestosModel extends Db{
  ...
  public function getDetallesManifiesto($Conex){
	...
	if(is_numeric($manifiesto_id)){
		...
		$select  = "SELECT d.detalle_despacho_id,d.manifiesto_id,r.remesa_id,(SELECT t.tipo_remesa FROM tipo_remesa t WHERE 
		tipo_remesa_id = r.tipo_remesa_id) 
		tipo_remesa,r.orden_despacho,r.numero_remesa,(SELECT CONCAT_WS(' ',primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,razon_social) 
		...
		'' as obser_rm, m.estado FROM manifiesto m,detalle_despacho d, remesa r WHERE m.manifiesto_id = $manifiesto_id AND m.manifiesto_id = d.manifiesto_id 
