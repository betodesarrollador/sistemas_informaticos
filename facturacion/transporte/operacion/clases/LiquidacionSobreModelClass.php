...
final class LiquidacionSobreModel extends Db{
  ...
  public function selectLiquidacionSobreManifiesto($manifiesto_id,$Conex){
  ...
  public function existeLiquidacionSobre($manifiesto_id,$Conex){
  
     $select = "SELECT * FROM liquidacion_despacho WHERE fuente_servicio_cod = 'MC' AND manifiesto_id = $manifiesto_id AND estado_liquidacion='C'";
     $result = $this  -> DbFetchAll($select,$Conex,true);	    
  ...
  public function existeLiquidacionSobreFlete($manifiesto_id,$Conex){
  
     $select = "SELECT * FROM liquidacion_despacho_sobre WHERE fuente_servicio_cod = 'MC' AND manifiesto_id = $manifiesto_id AND estado_liquidacion!='A'";
     $result = $this  -> DbFetchAll($select,$Conex,true);	    
  ...
  public function getEstadoLiquidacionSobre($manifiesto_id,$Conex){
  
     $select = "SELECT estado_liquidacion FROM liquidacion_despacho WHERE fuente_servicio_cod = 'MC' AND manifiesto_id = $manifiesto_id";
     $result = $this  -> DbFetchAll($select,$Conex,true);	   
  ...
  public function getEstadoMani($manifiesto_id,$Conex){
  ...
  public function Save($Campos,$empresa_id,$oficina_id,$modifica,$usuario_id,$Conex){		
	...
	$this -> Begin($Conex);
	  ...
	  if(!count($result) > 0){
	    print "<p align='center'>No ha ingresado los parametros de liquidacion!!<br>estos parametros se asignan por Parametros Modulo -> LiquidacionSobre</p>";
		$this -> RollBack($Conex);
		exit();
	  }else{
		  ...
		  if(!is_numeric($sobre_flete_id))     exit("<div align='center'><b>Aun no ha parametrizado la cuenta contable para concepto de sobre flete</b></div>
 		                                             <div align='center'><br><b>modulo de Transporte -> Parametros Modulo -> LiquidacionSobre</b></div>");		  
		  ...
		  if(is_numeric($centro_de_costo_id)){	
			...
			,oficina_id,estado_liquidacion,valor_sobre_flete,observacion_sobre_flete,manifiesto_id,concepto,consecutivo,observaciones,valor_neto_pagar) VALUES ($liquidacion_despacho_sobre_id,'$fecha',$tipo_documento_id,$tercero_id,$centro_de_costo_id,'$numero_despacho','$fuente_servicio_cod'
			,$oficina_pago_id,'$estado_liquidacion',$valor_sobre_flete,$observacion_sobre_flete,$manifiesto_id,'MC: $numero_despacho   Lugar Autorizado Para Pago : $LugarPago',$consecutivo,$observaciones,$valor_neto_pagar);";									
				  ...
				  for($i = 0; $i < count($impuestos); $i++){
					...
					if(count($result) > 0){
					  ...
					  if(count($dataImpuesto) > 0){
						 ...
						 impuesto,impuestos_manifiesto_id,valor,porcentaje,formula,descripcion) VALUES ($detalle_liquidacion_despacho_sobre_id,
  ...
  public function Update($Campos,$empresa_id,$oficina_id,$Conex){
   ...
   $this -> Begin($Conex);
    ...
    $select                    = "SELECT oficina_id FROM manifiesto WHERE manifiesto_id = (SELECT manifiesto_id FROM liquidacion_despacho_sobre WHERE 
	                              liquidacion_despacho_sobre_id = $liquidacion_despacho_sobre_id)";
	$dataOficinaPlanillo       = $this  -> DbFetchAll($select,$Conex,true);		   	  	  
	$oficina_planillo_id       = $dataOficinaPlanillo[0]['oficina_id'];	
	
	if($estado_liquidacion != 'L'){
	   exit("LiquidacionSobre no puede ser modificado, estado cambio!!!!!");
	}
	  ...
	  if(!count($result) > 0){
	    print "<p align='center'>No ha ingresado los parametros de liquidacion!!<br>estos parametros se asignan por Paramtros Modulo -> LiquidacionSobre</p>";
		$this -> RollBack($Conex);
		exit();
	  }else{ 
	...
	/*
	 ...
	 if($tip_guardar_rem=='total'){
		 ...
		 manifiesto_id FROM liquidacion_despacho_sobre WHERE liquidacion_despacho_sobre_id = $liquidacion_despacho_sobre_id)) AND clase_remesa = 'NN'";
		 ...
		 if($valor_facturar_total > 0){
		   ...
		   for($i = 0; $i < count($remesas); $i++){
			 ...
			 $update = "UPDATE remesa SET valor_costo = $valor_costo, conductor_id = (SELECT conductor_id FROM manifiesto WHERE 
						manifiesto_id = (SELECT manifiesto_id FROM liquidacion_despacho_sobre WHERE liquidacion_despacho_sobre_id = $liquidacion_despacho_sobre_id)) WHERE remesa_id = $remesa_id";
			...
			for($i = 0; $i < count($remesa); $i++){	
				 ...
				 if($remesa[$i]['estado']=='LQ' && $remesa[$i]['valor_costo']!=''){
					 ...
					 $update = "UPDATE remesa SET valor_costo = ".$valor_costo.", 
								...
								conductor_id = (SELECT conductor_id FROM manifiesto WHERE manifiesto_id = (SELECT manifiesto_id FROM liquidacion_despacho_sobre WHERE liquidacion_despacho_sobre_id = $liquidacion_despacho_sobre_id))
  ...
  public function getTiposDocumentoContable($Conex){
   ...
   public function selectLiquidacionSobre($liquidacion_despacho_sobre_id,$Conex){
	...
	ld.liquidacion_despacho_sobre_id,ld.encabezado_registro_id,ld.fecha,ld.oficina_id,ld.estado_liquidacion,m.manifiesto_id,m.manifiesto,m.tenedor,m.tenedor_id,
	m.placa,m.placa_id,(SELECT nombre FROM ubicacion WHERE ubicacion_id = m.origen_id) AS origen,m.origen_id,(SELECT nombre FROM ubicacion WHERE ubicacion_id = m.destino_id) 
	 AS destino,m.destino_id,CONCAT('LIQUIDACION SOBRE FLETE MC: ',m.manifiesto) AS concepto,ld.valor_sobre_flete,ld.observacion_sobre_flete,ld.observaciones FROM manifiesto m, 
	 liquidacion_despacho_sobre ld WHERE m.manifiesto_id = ld.manifiesto_id AND ld.liquidacion_despacho_sobre_id = $liquidacion_despacho_sobre_id";
    ...
    $select = "SELECT im.*,dl.detalle_liquidacion_despacho_sobre_id,dl.impuestos_manifiesto_id,dl.valor FROM detalle_liquidacion_despacho_sobre 
	dl,impuestos_manifiesto im WHERE dl.liquidacion_despacho_sobre_id = $liquidacion_despacho_sobre_id AND dl.impuesto = 1 AND dl.impuestos_manifiesto_id = 
  ...
  public function getContabilizarReg($liquidacion_despacho_sobre_id,$empresa_id,$oficina_id,$usuario_id,$Conex){
   ...
   public function getColumnsImpGridLiquidacionSobre($Conex){
      
//      $select    = "SELECT DISTINCT im.nombre FROM detalle_liquidacion_despacho_sobre dl,impuestos_manifiesto im WHERE dl.impuestos_manifiesto_id = im.impuestos_manifiesto_id";
   ...
   public function getColumnsDesGridLiquidacionSobre($Conex){
      
//      $select     = "SELECT DISTINCT dm.nombre FROM detalle_liquidacion_despacho_sobre dl,descuentos_manifiesto dm WHERE dl.descuentos_manifiesto_id = dm.descuentos_manifiesto_id";
...
//// GRID ////   
  public function getQueryManifiestosGrid($oficina_id,$colsImpuestos,$colsDescuentos){
	 ...
	 for($i = 0; $i < count($colsImpuestos); $i++){	 	 	 
	   ...
	   $columnsImpuestos .= "(SELECT SUM(valor) FROM detalle_liquidacion_despacho_sobre WHERE impuesto = 1 AND impuestos_manifiesto_id IN (SELECT impuestos_manifiesto_id 
	 ...
	 for($i = 0; $i < count($colsDescuentos); $i++){	 
	   ...
	   $columnsDescuentos .= "(SELECT SUM(valor) FROM detalle_liquidacion_despacho_sobre WHERE descuento = 1 AND descuentos_manifiesto_id IN (SELECT descuentos_manifiesto_id 
	 ...
	 FROM liquidacion_despacho_sobre ld, manifiesto m WHERE ld.manifiesto_id = m.manifiesto_id";
