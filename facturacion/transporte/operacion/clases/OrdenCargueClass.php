...
//BUSQUEDA
    ...
    protected function onclickGenerarDoc()
    {
        ...
        $estado = $Model->selectorden_cargue_estado($orden_cargue_id, $this->getConex());

        if ($estado[0]['manifiesto'] > 0 || $estado[0]['remesa'] > 0) {
            $remesa_r = $estado[0]['remesa'] > 0 ? 'Remesa: ' . $estado[0]['remesa'] : '';
            $manifiesto_r = $estado[0]['manifiesto'] > 0 ? ' Manifiesto: ' . $estado[0]['manifiesto'] : '';
            exit('Esta orden ya esta asociada a los siguientes documentos: ' . $remesa_r . ' - ' . $manifiesto_r);
        }
        $data = $Model->selectorden_cargue($orden_cargue_id, $this->getConex());

        if ($data[0]['orden_cargue_id'] > 0) {
            //remesa inicio

            $_REQUEST['nacional'] = 1;
            $_REQUEST['tipo_remesa_id'] = 1;
            $_REQUEST['naturaleza_id'] = 1;
            ...
            if ($valorMaximoPoliza == null) {
                ...
                $result = $Model->SaveRemesa($usuario_id, $usuarioNombres, $this->getUsuarioIdentificacion(), $data, $this->getOficinaId(), $this->getEmpresaId(), $this->getConex());
            ...
            exit("Orden de cargue no encontrada");
    ...
    protected function setCampos()
    {
        ...
        $this->Campos[estado] = array(
            ...
            id => 'estado',
            type => 'select',
            options => array(array(value => 'E', text => 'ESPERA', selected => 'E'), array(value => 'R', text => 'REALIZADO'), array(value => 'A', text => 'ANULADO')),
            required => 'yes',
            disabled => 'yes',
            datatype => array(
                type => 'text',
        ...
        $this->Campos[generar_doc] = array(
            name => 'generar_doc',
            id => 'generar_doc',
            type => 'button',
            value => 'Generar Documentos',
            onclick => 'onclickGenerarDoc(this.form)',
        );
