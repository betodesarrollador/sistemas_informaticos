...
final class LiquidacionDescuDespaModel extends Db{

   public function selectDespachos($Conex){
  
    $select = "SELECT * FROM despachos_urbanos m WHERE  m.aprobacion_ministerio3 IS NULL
 	AND m.aprobacion_ministerio2 IS NOT NULL AND (m.estado='L' OR m.estado='M') AND m.fecha_du <='2015-08-25' 
	LIMIT 0,80";
   ...
   public function selectDespachosPropios($Conex){
  
    $select = "SELECT * FROM despachos_urbanos m, legalizacion_despacho l WHERE m.propio =1 AND m.estado='L' AND m.fecha_du<='2014-11-30' 
	AND l.despachos_urbanos_id=m.despachos_urbanos_id LIMIT 0,1";
   ...
   public function selectRemesasDespacho($despachos_urbanos_id,$Conex){
  ...
  public function selectLiquidacionDescuDespacho($despachos_urbanos_id,$Conex){
	 ...
	 $select = "SELECT dm.*,(SELECT descuento FROM tabla_descuentos WHERE descuento_id = dm.descuento_id)as nombre,d.calculo FROM descuentos_despacho dm, tabla_descuentos d WHERE dm.despachos_urbanos_id = $despachos_urbanos_id AND dm.descuento_id = d.descuento_id";
  	 $result = $this  -> DbFetchAll($select,$Conex);
  ...
  public function existeLiquidacionDescu($despachos_urbanos_id,$Conex){
  
     $select = "SELECT * FROM liquidacion_despacho_descu WHERE fuente_servicio_cod = 'DU' AND despachos_urbanos_id = $despachos_urbanos_id AND estado_liquidacion!='A'";
     $result = $this  -> DbFetchAll($select,$Conex,true);	    
  ...
  public function getEstadoLiquidacionDescu($despachos_urbanos_id,$Conex){
	  ...
	  if(!count($result) > 0){
		...
		exit();
		  ...
		  if(is_numeric($centro_de_costo_id)){	
			...
			,oficina_id,estado_liquidacion,valor_descuentos,despachos_urbanos_id,concepto,consecutivo,observaciones) VALUES ($liquidacion_despacho_descu_id,'$fecha',$tipo_documento_id,$tercero_id,$centro_de_costo_id,'$numero_despacho','$fuente_servicio_cod'
			,$oficina_id,'$estado_liquidacion',$valor_descuentos,$despachos_urbanos_id,'DU: $numero_despacho ',$consecutivo,$observaciones);";									
				...
				if(is_array($_REQUEST['descuentos'])){
				  ...
				  for($i = 0; $i < count($descuentos); $i++){
					...
					if(count($result) > 0){
				      ...
				      $insert = "INSERT INTO detalle_liquidacion_despacho_descu (detalle_liquidacion_despacho_descu_id,liquidacion_despacho_descu_id,puc_id,centro_de_costo_id,codigo_centro_costo,tercero_id,numero_identificacion,digito_verificacion,base,debito,credito,descuento,descuentos_despacho_id,valor,descripcion) 
				      VALUES ($detalle_liquidacion_despacho_descu_id,$liquidacion_despacho_descu_id,$puc_id,$centro_costo_liquidacion_id,$codigo_centro_costo_liquidacion,$tercero_liquidacion_id,$numero_identificacion_liquidacion,$digito_verificacion_liquidacion,NULL,$debito,$credito,1,$descuentos_despacho_id,$valor,'$descripcion');";
					 // echo $insert;					  							
				      $this -> query($insert,$Conex,true);					  
			...
			if($UtilidadesContables -> requiereCentroCosto($puc_id,$Conex)){ 
			  ...
			  $descripcion = "DESCUENTOS DU: $numero_despacho";
	 ...
	 if($tip_guardar_rem=='total'){
		 ...
		 for($i = 0; $i < count($remesas); $i++){
		   ...
		   if(!$remesas[$i]['valor_facturar'] > 0){
			  $this -> RollBack($Conex);
			  exit("<div align='center'>Debe liquidar todas las remesa del despacho primero!!</div>");
		   }
		 ...
		 if($valor_facturar_total > 0){
		   ...
		   for($i = 0; $i < count($remesas); $i++){
			 ...
			 if(count($remesas) > 0){
			  $this -> RollBack($Conex);
			  exit("<div align='center'>Debe liquidar las remesas de este despacho primero!!!</div>");	 
			 }
  ...
  public function Update($Campos,$empresa_id,$oficina_id,$Conex){
   ...
   $this -> Begin($Conex);
    ...
    $select                    = "SELECT oficina_id FROM despachos_urbanos WHERE despachos_urbanos_id = (SELECT despachos_urbanos_id FROM liquidacion_despacho_descu WHERE 
	...
	/*
	 ...
	 if($tip_guardar_rem=='total'){
		 ...
		 despachos_urbanos_id FROM liquidacion_despacho WHERE liquidacion_despacho_id = $liquidacion_despacho_id)) AND clase_remesa = 'NN'";
		 ...
		 for($i = 0; $i < count($remesas); $i++){
		   ...
		   if(!$remesas[$i]['valor_facturar'] > 0)  {
			 $this -> RollBack($Conex);
			 exit("<div align='center'>Debe liquidar todas las remesas antes de liquidar el despacho!!</div>");
		   }
			...
			for($i = 0; $i < count($remesa); $i++){	
				 ...
				 if($remesa[$i]['estado']=='LQ' && $remesa[$i]['valor_costo']!=''){
					 ...
					 $update = "UPDATE remesa SET valor_costo = ".$valor_costo.", 
								...
								peso_costo = ".$peso.",
								valor_unidad_costo = ".$valor_unidad_costo.",
								conductor_id = (SELECT conductor_id FROM despachos_urbanos WHERE despachos_urbanos_id = (SELECT despachos_urbanos_id FROM liquidacion_despacho WHERE liquidacion_despacho_id = $liquidacion_despacho_id))
								WHERE remesa_id = ".$remesa[$i]['remesa_id']."";
  ...
  public function getTiposDocumentoContable($Conex){
   ...
   public function selectLiquidacionDescu($liquidacion_despacho_descu_id,$Conex){
	...
	ld.liquidacion_despacho_descu_id,ld.encabezado_registro_id,ld.fecha,ld.oficina_id,ld.estado_liquidacion,m.despachos_urbanos_id,m.despacho,m.tenedor,m.tenedor_id,
	m.placa,m.placa_id,(SELECT nombre FROM ubicacion WHERE ubicacion_id = m.origen_id) AS origen,m.origen_id,(SELECT nombre FROM ubicacion WHERE ubicacion_id = m.destino_id) 
	 AS destino,m.destino_id,CONCAT('LIQUIDACION DU: ',m.despacho) AS concepto,ld.observaciones,ld.valor_descuentos FROM despachos_urbanos m, 
	 liquidacion_despacho_descu ld WHERE m.despachos_urbanos_id = ld.despachos_urbanos_id AND ld.liquidacion_despacho_descu_id = $liquidacion_despacho_descu_id";
	...
	//pedazo johnatan inicio
  	 $select = "SELECT SUM(IF(r.peso_costo>0,r.peso_costo,r.peso)) AS peso, SUM(IF(r.peso_volumen_costo>0,r.peso_volumen_costo,r.peso_volumen)) AS peso_volumen,SUM(IF(r.cantidad_costo>0,r.cantidad_costo,r.cantidad)) AS cantidad, SUM(r.valor_costo) AS valor_costos
	 FROM detalle_despacho dd, remesa r WHERE dd.despachos_urbanos_id = (SELECT despachos_urbanos_id FROM liquidacion_despacho_descu  WHERE liquidacion_despacho_descu_id = $liquidacion_despacho_descu_id) AND r.remesa_id=dd.remesa_id";	 
  	 $result = $this  -> DbFetchAll($select,$Conex);
	 /*	
	 ...
	 FROM detalle_despacho dd, remesa r WHERE dd.despachos_urbanos_id = (SELECT despachos_urbanos_id FROM liquidacion_despacho  WHERE liquidacion_despacho_id = $liquidacion_despacho_id) AND r.remesa_id=dd.remesa_id";	 
  	 $result = $this  -> DbFetchAll($select,$Conex); */
	 ...
	 (SELECT CONCAT_WS(' ',razon_social, primer_nombre,primer_apellido) FROM tercero WHERE tercero_id = (SELECT tercero_id FROM cliente WHERE cliente_id = t.cliente_id)) AS clientess,
	 (SELECT placa FROM vehiculo WHERE placa_id = t.placa_id) AS  placa
	 FROM  tiempos_clientes_remesas t WHERE t.despachos_urbanos_id = (SELECT despachos_urbanos_id FROM liquidacion_despacho_descu  WHERE liquidacion_despacho_descu_id = $liquidacion_despacho_descu_id) ";	 
  	 $result = $this  -> DbFetchAll($select,$Conex);
    ...
    $select = "SELECT dm.*,(SELECT descuento FROM tabla_descuentos WHERE descuento_id = dm.descuento_id)as nombre,d.calculo,dl.detalle_liquidacion_despacho_descu_id,dl.descuentos_despacho_id,dl.valor FROM detalle_liquidacion_despacho_descu 
	dl,descuentos_despacho dm,tabla_descuentos d WHERE dl.liquidacion_despacho_descu_id = $liquidacion_despacho_descu_id AND dl.descuento = 1 AND 
...
//      $select     = "SELECT DISTINCT dm.nombre FROM detalle_liquidacion_despacho dl,descuentos_despacho dm WHERE dl.descuentos_despacho_id = dm.descuentos_despacho_id";
  ...
  public function getContabilizarReg($liquidacion_despacho_descu_id,$empresa_id,$oficina_id,$usuario_id,$Conex){
	...
	$this -> Begin($Conex);
			...
			$result 	= $this -> DbFetchAll($select,$Conex,true); 
			if($result[0]['estado_liquidacion']=='C') exit('Esta liquidacion ya esta Contabilizada');
  ...
  //// GRID ////   
  public function getQueryDespachosGrid($oficina_id,$colsImpuestos,$colsDescuentos){
	 ...
	 FROM liquidacion_despacho ld, despachos_urbanos m WHERE ld.despachos_urbanos_id = m.despachos_urbanos_id";
