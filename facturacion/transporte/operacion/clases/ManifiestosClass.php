...
//LISTA MENU
	...
	$Attributes = array(
	  id=>'Manifiestos',
	  title=>'Manifiestos de Transporte',
	  sortname=>'fecha_mc',
	  sortorder=>'desc',
	  width=>'auto',
	  height=>'250'
	...
	$Cols = array(
	  ...
	  array(name=>'aprobacion_ministerio2',index=>'aprobacion_ministerio2',       sorttype=>'text', width=>'100', align=>'center')	  
	);
	

    $Titles = array('MANIFIESTO','PROPIO','ESTADO','FECHA','ORIGEN','DESTINO','CONDUCTOR','PLACA','REMOLQUE',
                    'FLETE','No ANTICIPOS','ANTICIPOS','IMPUESTOS','No APROB. MIN');
...
//BUSQUEDA
  ...
  protected function onclickPrint(){
    ...
    $print = new Imp_Manifiesto();

    $print$print -> printOut($this -> getConex());
  ...
  protected function comprobarManifiesto(){
    ...
    //echo 'aaaa'.$manifiesto;
    $data     = $Model -> selectConsecutivoManifiesto($this -> getConex(), $manifiesto);
  ...
  protected function calcularFlete(){
    ...
    $impuestos1        = $_REQUEST['impuestos1'];
	 ...
	 for($i = 0; $i < count($impuestos); $i++){	 
      ...
      $arrayResponse['impuestos1'][$i]['porcentaje']  = number_format($porcentaje,4,',','.');          
  ...
  protected function getContabilizar(){
    ...
    $fecha_mc 		= $_REQUEST['fecha_mc'];
	
    $empresa_id = $this -> getEmpresaId(); 
    $oficina_id = $this -> getOficinaId();	
    $usuario_id = $this -> getUsuarioId();		
	
    $mesContable     	= $Model -> mesContableEstaHabilitado($empresa_id,$oficina_id,$fecha_mc,$this -> getConex());
    $periodoContable 	= $Model -> PeriodoContableEstaHabilitado($this -> getConex());
	  $result_antic 		= $Model -> ValidaAnticipo($manifiesto_id,$this -> getConex()); 

	if(!$result_antic){
		if($mesContable && $periodoContable ){
	
			$return=$Model -> getContabilizarReg($manifiesto_id,$empresa_id,$oficina_id,$usuario_id,$mesContable,$periodoContable,$this -> getConex());
			if($return==true){
				exit("true");
			}else{
  ...
  protected function sendInformacionViaje(){
	  ...
	  $webService    = new WebServiceMinTransporte($this -> getConex());	 
	  $manifiesto_id = $this -> requestData('manifiesto_id');	
	  $estado       = $Model -> selectEstadoManifiesto($manifiesto_id,$this -> getConex());	
	  if($estado=='P') exit('El Manifiesto esta en estado Pendiente. <br />Por favor termine el proceso para Manifestar y Reportar al RNDC!!');
