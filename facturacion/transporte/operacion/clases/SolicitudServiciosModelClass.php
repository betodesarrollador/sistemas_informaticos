		...
		if($this -> GetNumError() > 0){
		  return false;
		    ...
		    if($paqueteo=='N'){
				...
				$valor_costo   = $this -> requestDataForQuery('valor_costo','numeric');
	    		$valores_solicitud_id = $this -> DbgetMaxConsecutive("solicitud_valores","valores_solicitud_id",$Conex,false,1);
				
				$insert = "INSERT INTO solicitud_valores (valores_solicitud_id,fecha,valor_facturar,valor_costo,solicitud_id) VALUES 
				($valores_solicitud_id,CURDATE(),$valor_facturar,$valor_costo,$solicitud_id)";
...
//LISTA MENU
  ...
  public function getClienteId($solicitud_id,$Conex){
	  
	  $select = "SELECT cliente_id 
	  FROM solicitud_servicio WHERE solicitud_id = $solicitud_id";
  ...
  public function GetHistorico($solicitud_id,$Conex){
  
    $select = "SELECT * 
				FROM solicitud_valores
...
//BUSQUEDA
  public function selectSolicitud($SeguimientoId,$Conex){
   ...
   $select = "SELECT s.*, 
   				(SELECT valor_facturar FROM solicitud_valores WHERE solicitud_id=$SeguimientoId ORDER BY fecha DESC, valores_solicitud_id DESC LIMIT 1) AS valor_facturar,
   				(SELECT valor_costo FROM solicitud_valores WHERE solicitud_id=$SeguimientoId ORDER BY fecha DESC, valores_solicitud_id DESC LIMIT 1) AS valor_costo,				
   				(SELECT CONCAT_WS(' ',razon_social,primer_nombre,segundo_nombre,primer_apellido,segundo_apellido) 
				FROM tercero WHERE tercero_id = (SELECT tercero_id FROM cliente WHERE cliente_id = s.cliente_id)) AS cliente,(SELECT nombre FROM oficina WHERE oficina_id = s.oficina_id) AS oficina 
  ...
  public function SelectContactosSeguimiento($solicitud_id,$Conex){
   ...
   public function getValueForeign($valor,$campo,$parametrosCampo,$cliente_id,$Conex){
	  ...
	  if($nombreTabla == 'destinatario'){
		...
		if($nombreTabla == 'origen_id'){
			...
			//validación del departamento concatenado con '-' 
			
			$separador   = '-';
			$pos = strpos($valor, $separador);
				 ...
				 //$select = "SELECT * FROM ubicacion WHERE TRIM(nombre) LIKE TRIM('$valor') AND tipo_ubicacion_id=3";
				  ...
				  if(count($result) > 0){
					 ...
					 return array(campos => "origen_id,", valores => "{$result[0]['ubicacion_id']},");		  
					   ...
					   return "1-No se encontro la ubicacion [ $valor ] parametrizada.";
				  
					}
			} else {
							
				$valor_temp = explode('-',$valor);
				
				$municipio = $valor_temp[0];
				$departamento = $valor_temp[1];
				
				$select = "SELECT * FROM ubicacion WHERE TRIM(nombre) LIKE TRIM('$municipio') AND tipo_ubicacion_id=3 AND ubi_ubicacion_id = (SELECT  ubicacion_id FROM ubicacion WHERE nombre LIKE TRIM('$departamento'))";
						  $result = $this -> DbFetchAll($select,$Conex,true);
				  ...
				  if(count($result) > 0){
					 ...
					 return array(campos => "origen_id,", valores => "{$result[0]['ubicacion_id']},");		  
					   ...
					   return "2-No se encontro la ubicacion [ $valor ] parametrizada.";
	      ...
	      if($nombreTabla == 'unidad_peso_id'){
		     ...
		     $select = "SELECT * FROM medida_cliente WHERE TRIM(medida) LIKE TRIM('$valor')";
             $result = $this -> DbFetchAll($select,$Conex,true);       			
   ...
   public function setInsertDetalleSolicitud($rowInsert,$solicitud_id,$cliente_id,$Conex){
	  ...
	  $tipo_identificacion_destinatario_id = $dataRemitente[0]['tipo_identificacion_id'];
	  
	  
      $insert = "INSERT INTO detalle_solicitud_servicio (detalle_ss_id,solicitud_id,remitente_id,tipo_identificacion_remitente_id,tipo_identificacion_destinatario_id,$campos) VALUES ($detalle_ss_id,$solicitud_id,$remitente_id,$tipo_identificacion_remitente_id,$tipo_identificacion_destinatario_id,$valores)";
	  //exit($insert);
