...
final class DetalleDevolucionPlacaModel extends Db{

   public function getDataplaca($placa_id,$Conex){
    
	  $select = "SELECT v.placa, v.propio, v.tenedor_id, v.conductor_id, v.placa_id,
	  (SELECT CONCAT_WS(' ',primer_nombre,segundo_nombre,primer_apellido,segundo_apellido,razon_social) FROM tercero WHERE tercero_id = (SELECT tercero_id FROM tenedor WHERE tenedor_id = v.tenedor_id)) AS tenedor,
	  ...
	  FROM vehiculo v WHERE v.placa_id = $placa_id";
   ...
   public function getAnticiposPlaca($placa_id,$Conex){
	 ...
	 FROM anticipos_placa a WHERE a.placa_id = $placa_id  AND devolucion=1  ORDER BY a.anticipos_placa_id desc";
   ...
   public function getAnticiposPlacaCruce($placa_id,$Conex){
   
     $select    = "SELECT a.*
	 FROM anticipos_placa a WHERE a.placa_id = $placa_id  AND devolucion=0  ORDER BY a.anticipos_placa_id desc";
     $anticipos = $this -> DbFetchAll($select,$Conex);
   ...
   public function getRegDataplaca($placa_id,$Conex){
   
     $select    = "SELECT (MAX(numero)+1) AS numeros FROM anticipos_placa WHERE placa_id = $placa_id AND devolucion=1 ";
  ...
  public function Anular($empresa_id,$oficina_id,$modifica,$usuario_id,$Conex){
    $this -> Begin($Conex);
     $anticipos_placa_id           = $this -> requestData('anticipos_placa_id');
	 $encabezado_registro_id       = $this -> requestData('encabezado_registro_id');
	 
	 $select1     = "SELECT COUNT(*) AS movi FROM relacion_anticipos_abono  WHERE anticipos_placa_id = $anticipos_placa_id";
	 ...
	 if(!$movimientos1>0){
		 ...
		 $update="UPDATE  anticipos_placa SET estado='A' WHERE anticipos_placa_id=$anticipos_placa_id";
  ...
  public function Delete($empresa_id,$oficina_id,$modifica,$usuario_id,$Conex){
    $this -> Begin($Conex);
	 ...
	 if(!$movimientos1>0){
		 $delete="DELETE FROM   anticipos_placa WHERE anticipos_placa_id=$anticipos_placa_id ";
  ...
  public function save($empresa_id,$oficina_id,$modifica,$usuario_id,$Conex){
    ...
    $this -> Begin($Conex);
	 ...
	 $anticipos_placa_id  	  = $this -> DbgetMaxConsecutive("anticipos_placa","anticipos_placa_id",$Conex,false,1);	  
	 ...
	 $sub_anticipos_placa_id  = $this -> requestData('sub_anticipos_placa_id');
	 ...
	 $select_totaldev     = "SELECT SUM(valor) AS valor_devolucion  FROM anticipos_placa 
	 WHERE sub_anticipos_placa_id = $sub_anticipos_placa_id AND estado!='A' AND devolucion=1";
     $result_totaldev     = $this -> DbFetchAll($select_totaldev,$Conex,true);	
	 $valor_devolucion=$result_totalant[0]['valor_devolucion']+$valor;
	 
	 $select_totalant     = "SELECT valor  FROM anticipos_placa 
	 WHERE anticipos_placa_id = $sub_anticipos_placa_id AND estado!='A' AND devolucion=0";
	...
	$id_documento_fuente     = $anticipos_placa_id;
	$anulado                 = "0";

	$select = "SELECT * FROM parametros_anticipo WHERE propio = $propio"; 
	$result = $this -> DbFetchAll($select,$Conex,true);			
   ...
   $insert = "INSERT INTO  anticipos_placa (anticipos_placa_id,consecutivo,fecha,propio,oficina_id,placa_id,numero,conductor,conductor_id,tenedor,tenedor_id,valor,estado,forma_pago_id,cuenta_tipo_pago_id,encabezado_registro_id,numero_soporte,fecha_egreso,observaciones,devolucion,sub_anticipos_placa_id)VALUES 
   ($anticipos_placa_id,$consecutivo,'$fecha_egreso',$propio,$oficina_id,$placa_id,$numero,'$nombre',$conductor_id,'$tenedor',$tenedor_id,$valor,'P',$forma_pago_id,$cuenta_tipo_pago,$encabezado_registro_id,'$numero_soporte','$fecha_registro','$observaciones',1,$sub_anticipos_placa_id)";
