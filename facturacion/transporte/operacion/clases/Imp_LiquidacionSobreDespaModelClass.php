...
final class Imp_LiquidacionSobreDespaModel extends Db{

  
  public function getEncabezado($liquidacion_despacho_sobre_id,$Conex){
  
	
	if(is_numeric($liquidacion_despacho_sobre_id)){
       ...
       $select = "SELECT (SELECT logo FROM empresa WHERE empresa_id = of.empresa_id) AS logo, (SELECT razon_social FROM tercero WHERE tercero_id = (SELECT tercero_id FROM empresa WHERE empresa_id = of.empresa_id)) AS razon_social_emp,
		...
		(SELECT nombre FROM ubicacion WHERE ubicacion_id=of.ubicacion_id) AS ciudad_ofi,IF(e.manifiesto_id IS NOT NULL,(SELECT placa FROM manifiesto WHERE manifiesto_id = e.manifiesto_id),(SELECT placa FROM despachos_urbanos WHERE despachos_urbanos_id = e.despachos_urbanos_id)) AS placa,IF(e.manifiesto_id IS NOT NULL,(SELECT nombre FROM manifiesto WHERE manifiesto_id = e.manifiesto_id),(SELECT nombre FROM despachos_urbanos WHERE despachos_urbanos_id = e.despachos_urbanos_id)) AS conductor,IF(e.manifiesto_id IS NOT NULL,(SELECT fecha_mc FROM manifiesto WHERE manifiesto_id = e.manifiesto_id),(SELECT fecha_du FROM despachos_urbanos WHERE despachos_urbanos_id = e.despachos_urbanos_id)) AS fecha_planilla,(
		
		
		SELECT nombre FROM ubicacion WHERE ubicacion_id = 
		
		IF(e.manifiesto_id IS NOT NULL,(SELECT origen_id FROM manifiesto WHERE manifiesto_id = e.manifiesto_id),(SELECT origen_id FROM despachos_urbanos WHERE 
		despachos_urbanos_id = e.despachos_urbanos_id))	)AS origen,
		(SELECT nombre FROM ubicacion WHERE ubicacion_id = 		
		IF(e.manifiesto_id IS NOT NULL,(SELECT destino_id FROM manifiesto WHERE manifiesto_id = e.manifiesto_id),(SELECT destino_id FROM despachos_urbanos WHERE 
		despachos_urbanos_id = e.despachos_urbanos_id))		) AS destino
        FROM  liquidacion_despacho_sobre  e, oficina of, tercero t WHERE e.liquidacion_despacho_sobre_id  = $liquidacion_despacho_sobre_id AND of.oficina_id=e.oficina_id AND t.tercero_id=e.tercero_id";			
  ...
  public function getimputaciones($liquidacion_despacho_sobre_id,$Conex){
	...
	if(is_numeric($liquidacion_despacho_sobre_id)){
        ...
        $select = "SELECT i.*,
         ...
         FROM   detalle_liquidacion_despacho_sobre i   WHERE i.liquidacion_despacho_sobre_id = $liquidacion_despacho_sobre_id ";		
  ...
  public function getTotal($liquidacion_despacho_sobre_id,$Conex){
  
    $liquidacion_despacho_sobre_id = $this -> requestDataForQuery('liquidacion_despacho_sobre_id','integer');
	
	if(is_numeric($liquidacion_despacho_sobre_id)){
				
        $select = "SELECT SUM(debito) AS total_debito, SUM(credito) AS total_credito		
         FROM  detalle_liquidacion_despacho_sobre    WHERE liquidacion_despacho_sobre_id = $liquidacion_despacho_sobre_id ";		
  ...
  public function getTotales($liquidacion_despacho_sobre_id,$Conex){
