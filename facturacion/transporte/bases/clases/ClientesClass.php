...
final class Clientes extends Controler
{
    ...
    public function Main()
    {
        ...
        $Layout->setEstado();
        $Layout->SetOficina($Model->GetOfi($this->getConex()));
        //// GRID ////
        $Attributes = array(
            id => 'terceros',
        ...
        $Cols = array(
            ...
            array(name => 'comercial', index => 'comercial', sorttype => 'text', width => '150', align => 'center'),
...
//--------------------------------------
    protected function validarClinton()
    {
        ...
        $apariciones = ($Model->Clinton($documento, $cliente_id, $this->getConex()));
        exit(' ' . $apariciones);
    ...
    protected function sendRemitenteDestinatarioMintransporte()
    {
        ...
        $data = array(
            ...
            nombre => $this->requestData('primer_nombre') . ' ' . $this->requestData('razon_social'),
            nombre_sede => $this->requestData('primer_nombre') . ' ' . $this->requestData('razon_social'),
...
//BUSQUEDA
    ...
    protected function onclickPrint()
    {
        ...
        $print = new Imp_Cliente($this->getConex());
        $print->printOut($this -> getConex());

    }

    protected function comprobar_obligaciones()
    {
        ...
        $Layout->setJsInclude("../../../transporte/bases/js/Clientes.js");
        $Layout->assign("CSSSYSTEM", $Layout->getCssInclude());
        $Layout->assign("JAVASCRIPT", $Layout->getJsInclude());

        $tercero_id = $_REQUEST['tercero_id'];

        $obligaciones = $Model->getObligacionesTercero($tercero_id, $this->getConex());
        $Layout->setVar('OBLIGACIONES', $obligaciones);
        $Layout->setVar('tercero_id', $tercero_id);

        $Layout->RenderLayout('ObligacionesResult.tpl');

    }

    protected function SaveObligacion()
    {
        ...
        $tercero_id = $_REQUEST['tercero_id'];
        $codigo_obligacion_id = $_REQUEST['codigo_obligacion_id'];
        $estado = $_REQUEST['estado'];
        $result = $Model->SaveObligacion($tercero_id, $codigo_obligacion_id, $estado, $this->getConex());
    ...
    protected function UpdateObligacion()
    {
        ...
        $tercero_id = $_REQUEST['tercero_id'];
        $tercero_obligacion_id = $_REQUEST['tercero_obligacion_id'];
        $codigo_obligacion_id = $_REQUEST['codigo_obligacion_id'];
        $estado = $_REQUEST['estado'];

        $result = $Model->UpdateObligacion($tercero_id, $tercero_obligacion_id, $codigo_obligacion_id, $estado, $this->getConex());
    ...
    protected function SetCampos()
    {
        ...
        $this -> Campos[tipo_comision] = array(
        name     =>'tipo_comision',
        id         =>'tipo_comision',
        type     =>'select',
        //required =>'yes',
        options     =>array(array(value => 'F',text => 'FACTURACION'),array(value => 'R', text => 'RECAUDO')),
        selected => 'F',
        datatype =>array(type=>'text'),
        transaction=>array(
        table    =>array('cliente'),
        ...
        $this->Campos[tipo_liquidacion] = array(
            ...
            options => array(array(value => 'A', text => 'AUTOMATICO'), array(value => 'M', text => 'MANUAL')),
            selected => 'M',
            transaction => array(
                table => array('cliente'),
                type => array('column')),
        ...
        $this->Campos[peso_unidad] = array(
            name => 'peso_unidad',
            id => 'peso_unidad',
            type => 'select',
            options => array(array(value => 'P', text => 'PESO'), array(value => 'U', text => 'UNIDAD')),
            selected => 'P',
            transaction => array(
                table => array('cliente'),
                type => array('column')),
        ...
        $this->Campos[ultima_clinton] = array(
            name => 'ultima_clinton',
            id => 'ultima_clinton',
            type => 'text',
            disabled => 'yes',
