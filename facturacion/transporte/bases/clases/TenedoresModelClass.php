...
final class TenedoresModel extends Db{
  ...
  public function Save($Campos,$Conex){
    ...
    $this -> Begin($Conex);
	  ...
	  $numcuenta_proveedor    = $this -> requestDataForQuery('numcuenta_proveedor','text');
	  $banco_id 	          = $this -> requestDataForQuery('banco_id','alphanum');
	
	
	  $insert = "INSERT INTO tercero (tercero_id,ubicacion_id,tipo_persona_id,tipo_identificacion_id,numero_identificacion,digito_verificacion,primer_apellido,segundo_apellido, primer_nombre,segundo_nombre,razon_social,sigla,telefono,movil,direccion,renta_proveedor,tipo_cta_id,numcuenta_proveedor,banco_id) 
	  VALUES 
($tercero_id,$ubicacion_id,$tipo_persona_id,$tipo_identificacion_id,$numero_identificacion,$digito_verificacion,$primer_apellido,$segundo_apellido,
	  $primer_nombre,$segundo_nombre,$razon_social,$sigla,$telefono,$movil,$direccion,$renta_proveedor,$tipo_cta_id,$numcuenta_proveedor,$banco_id)"; 
  ...
  public function Update($Campos,$Conex){
    ...
    //$this -> assignValRequest('numero_cuenta_tene',$_REQUEST['numcuenta_proveedor']);  	 
	
    if($this -> seRegistroTenedor($Conex)){
        $this -> DbUpdateTable("tenedor",$Campos,$Conex,true,false);
    }else{
          $this -> DbInsertTable("tenedor",$Campos,$Conex,true,false);    
     }
...
//BUSQUEDA
  public function selectTenedoresporId($Id,$Conex){
   ...
   t.tipo_cta_id,t.numcuenta_proveedor,t.banco_id,te.renta_proveedor,
   ...
   te.estado,te.documentos,te.rut,te.seguridad_social,te.venc_seguridad_social,te.estado AS estado
  ...
  public function selectTenedoresporNumId($Id,$Conex){
   ...
   (SELECT CONCAT_WS(' ',primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,razon_social,sigla) FROM tercero WHERE tercero_id=te.banco_id) AS banco,
   te.fecha_data_tene,te.calificacion_data_tene,
   te.estado,te.documentos,te.estado AS estado
   FROM tercero t  LEFT JOIN tenedor te ON t.tercero_id = te.tercero_id 
...
//// GRID ////
  public function getQueryTenedoresGrid(){
     ...
     $Query = "	SELECT (SELECT nombre FROM tipo_identificacion WHERE tipo_identificacion_id = t.tipo_identificacion_id) AS tipo_identificacion,
				...
				(SELECT CONCAT_WS('',primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,razon_social) FROM tercero WHERE tercero_id=t.banco_id) AS banco,
				(SELECT nombre_tipo_cuenta FROM tipo_cuenta WHERE tipo_cta_id=t.tipo_cta_id) AS nombre_tipo_cuenta,
				t.numcuenta_proveedor,te.fecha_data_tene,te.calificacion_data_tene,te.aprobacion_ministerio,
				IF(te.estado='B','BLOQUEADO','DISPONIBLE')AS estado
