...
final class RemitenteModel extends Db
{
    ...
    public function GetTipoId($Conex)
    {
        ...
        $select = "SELECT tipo_identificacion_id AS value,nombre AS text FROM tipo_identificacion WHERE ministerio = 1 ORDER BY nombre ASC";
    ...
    public function selectRemitente($Conex)
    {
        ...
        if ($remitente_destinatario_id != 'NULL') {
            ...
            $Query = "SELECT (SELECT CONCAT_WS(' ',razon_social,primer_nombre) FROM
                ...
                (SELECT nombre FROM ubicacion WHERE ubicacion_id = t.ubicacion_id) AS ubicacion,'R' AS tipo
    ...
    public function remitenteExiste($valor, $cliente_id, $ubicacion_id, $numero_identificacion,$Conex)
    {

        $select = "SELECT * FROM remitente_destinatario WHERE cliente_id = $cliente_id AND tipo = 'R' AND TRIM(nombre) LIKE TRIM('$valor') AND ubicacion_id = $ubicacion_id AND numero_identificacion = $numero_identificacion";
    ...
    public function getQueryRemitenteGrid()
    {
        ...
        $Query = "SELECT (SELECT concat_ws(' ',razon_social,primer_nombre,segundo_nombre,primer_apellido,segundo_apellido) FROM tercero WHERE tercero_id = (SELECT tercero_id FROM cliente WHERE cliente_id = T.cliente_id)) AS cliente,(SELECT nombre FROM tipo_identificacion WHERE tipo_identificacion_id = T.tipo_identificacion_id)
				...
				(SELECT nombre FROM ubicacion WHERE ubicacion_id = T.ubicacion_id) AS ubicacion,aprobacion_ministerio,estado
			   FROM remitente_destinatario T  WHERE tipo = 'R' ORDER BY T.nombre ASC";
