...
final class Imp_HV_VehiculoModel extends Db{
  ...
  public function getVehiculo($Conex){
	...
	if(is_numeric($placa_id)){
           ...
           $select = "SELECT v.*, 
		   CONCAT(v.archivo_imagen_frontal,'?',RAND()) as archivo_imagen_frontal,
		   CONCAT(v.archivo_imagen_lateral,'?',RAND()) as archivo_imagen_lateral,
		   CONCAT(v.archivo_imagen_trasera,'?',RAND()) as archivo_imagen_trasera,
...
(SELECT CONCAT_WS(' ', t.primer_apellido,t.segundo_apellido,t.primer_nombre,t.segundo_nombre) FROM tercero t, usuario u WHERE t.tercero_id=u.tercero_id AND u.usuario_id=v.usuario_id) AS nombre_usuario,
...
(SELECT email FROM tercero WHERE tercero_id = (SELECT tercero_id FROM tenedor WHERE tenedor_id = v.tenedor_id)) AS email_tenedor,
					...
					AS banco,
					(SELECT nombre_tipo_cuenta FROM tipo_cuenta tc, tenedor tn WHERE tc.tipo_cta_id=tn.tipo_cta_id AND tn.tenedor_id=v.tenedor_id) AS tipo_cuenta,
					(SELECT CONCAT_WS(' - ',configuracion, descripcion_config) AS configuracion FROM configuracion WHERE configuracion=v.configuracion) AS configuracion,
...
(SELECT concat_ws(' ',numero_identificacion,primer_nombre,segundo_nombre,primer_apellido,segundo_apellido) FROM tercero WHERE tercero_id = v.persona_aprobo_id) AS aprobo,v.responsable_verificacion1
  ...
  public function getPropietario($Conex){
	...
	if(is_numeric($placa_id)){
	  ...
	  $select  = "SELECT  
	  				(SELECT CONCAT_WS(' ', t.primer_apellido,t.segundo_apellido,t.primer_nombre,t.segundo_nombre,t.razon_social) FROM tercero t WHERE t.tercero_id=v.propietario_id) AS nombre_propietario,
					(SELECT CONCAT_WS(' ',ti.nombre,t.numero_identificacion)
						FROM tercero t, tipo_identificacion ti
						WHERE ti.tipo_identificacion_id=t.tipo_identificacion_id
						AND t.tercero_id=v.propietario_id) 
					AS identi_propietario,
					(SELECT nombre FROM tercero t, ubicacion u WHERE u.ubicacion_id=t.ubicacion_id AND t.tercero_id=v.propietario_id) AS ubicacion,
					(SELECT t.direccion FROM tercero t WHERE t.tercero_id=v.propietario_id) AS direccion,
						(SELECT CONCAT_WS('-',t.telefono,t.movil) FROM tercero t WHERE t.tercero_id=v.propietario_id) AS telefonos FROM vehiculo v WHERE v.placa_id=$placa_id";
