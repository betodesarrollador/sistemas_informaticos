...
final class Imp_FacturaAdjuntoModel extends Db{
 ...
 public function getParametros($Conex){
  ...
  public function getFactura($factura_id,$Conex){
	...
	if(is_numeric($factura_id)){
		...
		(SELECT email  FROM empresa WHERE empresa_id = of.empresa_id) AS email_empresa,
  ...
  public function getitemFactura($factura_id,$Conex){
	...
	if(is_numeric($factura_id)){
        ...
        IF((SELECT remesa_id FROM detalle_despacho WHERE remesa_id=d.remesa_id)>0,
			(IF((SELECT manifiesto_id FROM detalle_despacho WHERE remesa_id=d.remesa_id)>0,
			(SELECT placa FROM manifiesto WHERE manifiesto_id=(SELECT manifiesto_id FROM detalle_despacho WHERE remesa_id=d.remesa_id)),
			(SELECT placa FROM despachos_urbanos WHERE despachos_urbanos_id=(SELECT despachos_urbanos_id FROM detalle_despacho WHERE remesa_id=d.remesa_id))
			)),'N/A'
		)AS placa,

		 IF((SELECT remesa_id FROM detalle_despacho WHERE remesa_id=d.remesa_id)>0,
			(IF((SELECT manifiesto_id FROM detalle_despacho WHERE remesa_id=d.remesa_id)>0,
			(SELECT manifiesto FROM manifiesto WHERE manifiesto_id=(SELECT manifiesto_id FROM detalle_despacho WHERE remesa_id=d.remesa_id)),
			(SELECT despacho FROM despachos_urbanos WHERE despachos_urbanos_id=(SELECT despachos_urbanos_id FROM detalle_despacho WHERE remesa_id=d.remesa_id))
			)),'N/A'
		)AS manifiesto,

		IF((SELECT remesa_id FROM detalle_despacho WHERE remesa_id=d.remesa_id)>0,
			(IF((SELECT manifiesto_id FROM detalle_despacho WHERE remesa_id=d.remesa_id)>0,
			(SELECT pasador_vial FROM ruta WHERE ruta_id = (SELECT ruta_id FROM manifiesto WHERE manifiesto_id=(SELECT manifiesto_id FROM detalle_despacho WHERE remesa_id=d.remesa_id))),
			('N/A')
			)),'N/A'
		)AS pasador_vial
    ...
    public function get_pucFactura($factura_id,$fuente_facturacion_cod,$Conex){
	...
	if(is_numeric($factura_id)){
		...
		if ($fuente_facturacion_cod =='RM') {
			...
			(d.deb_item_factura+d.cre_item_factura) AS valor, 
			...
			WHERE o.factura_id = $factura_id AND d.factura_id=o.factura_id AND c.tipo_bien_servicio_factura_id=o.tipo_bien_servicio_factura_id AND c.ret_tercero_bien_servicio_factura = 0 AND c.tercero_bien_servicio_factura=0 AND c.puc_id=d.puc_id GROUP BY c.puc_id ORDER BY d.detalle_factura_puc_id ";	
		}else {
  ...
  public function get_valor_letras($factura_id,$Conex){
  ...
  public function get_valor_detalles($factura_id,$Conex){
  ...
  public function getImputacionesContables($factura_id,$Conex){
