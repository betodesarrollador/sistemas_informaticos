...
final class Imp_DocumentoModel extends Db{
  ...
  public function getEncabezado($encabezado_registro_id,$Conex){
	...
	if(is_numeric($encabezado_registro_id)){
		...
		(SELECT nombre FROM ubicacion WHERE ubicacion_id = (SELECT ubicacion_id FROM tercero WHERE tercero_id = (SELECT tercero_id FROM empresa WHERE empresa_id = of.empresa_id))) AS ciudad_emp,(SELECT a.observaciones FROM abono_factura a WHERE a.encabezado_registro_id=e.encabezado_registro_id)AS observaciones_abono,
