...
function suma_cantidad() {
	...
	$("input[name=cant_item_liquida_servicio]").keyup(function () {
		var Fila = $(this).parent().parent();
		var cant = $(Fila).find("input[name=cant_item_liquida_servicio]").val();
		var valoruni = $(Fila).find("input[name=valoruni_item_liquida_servicio]").val();
	...
	$("input[name=valoruni_item_liquida_servicio]").keyup(function () {
		var Fila = $(this).parent().parent();
		var cant = $(Fila).find("input[name=cant_item_liquida_servicio]").val();
		var valoruni = $(Fila).find("input[name=valoruni_item_liquida_servicio]").val();
		if (cant != '' && cant != 0 && valoruni != '' && valoruni != 0) {
...
function saveDetalle(obj) {
	...
	if (validaRequeridosDetalle(obj, row)) {
		...
		var item_liquida_servicio_id = $(Fila).find("input[name=item_liquida_servicio_id]").val();
		var orden_servicio_id = $("#orden_servicio_id").val();
		var cant_item_liquida_servicio = $(Fila).find("input[name=cant_item_liquida_servicio]").val();
		var desc_item_liquida_servicio = $(Fila).find("input[name=desc_item_liquida_servicio]").val();
		var valoruni_item_liquida_servicio = $(Fila).find("input[name=valoruni_item_liquida_servicio]").val();
		var remesa_id = $(Fila).find("input[name=remesa_id]").val();
		var checkProcesar = $(Fila).find("input[name=procesar]");



		if (!item_liquida_servicio_id.length > 0) {

			item_liquida_servicio_id = 'NULL';

			var QueryString = "ACTIONCONTROLER=onclickSave&orden_servicio_id=" + orden_servicio_id + "&item_liquida_servicio_id=" +
				item_liquida_servicio_id + "&cant_item_liquida_servicio=" + cant_item_liquida_servicio + "&desc_item_liquida_servicio="
				+ desc_item_liquida_servicio + "&valoruni_item_liquida_servicio=" + valoruni_item_liquida_servicio + "&remesa_id=" + remesa_id;
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (!isNaN(response)) {
						...
						$(Fila).find("input[name=item_liquida_servicio_id]").val(response);
						...
						$(newRow).find("input[name=cant_item_liquida_servicio]").focus();
						...
						$(Celda).removeClass("focusSaveRow");
						if (parent.getTotal)
							parent.getTotal(parent.document.getElementById("orden_servicio_id").value, 1);
			...
			var QueryString = "ACTIONCONTROLER=onclickUpdate&orden_servicio_id=" + orden_servicio_id + "&item_liquida_servicio_id=" +
				item_liquida_servicio_id + "&cant_item_liquida_servicio=" + cant_item_liquida_servicio + "&desc_item_liquida_servicio="
				+ desc_item_liquida_servicio + "&valoruni_item_liquida_servicio=" + valoruni_item_liquida_servicio + "&remesa_id=" + remesa_id;
...
function deleteDetalle(obj) {
	...
	var Fila = obj.parentNode.parentNode;
	var item_liquida_servicio_id = $(Fila).find("input[name=item_liquida_servicio_id]").val();
	var QueryString = "ACTIONCONTROLER=onclickDelete&item_liquida_servicio_id=" + item_liquida_servicio_id;
