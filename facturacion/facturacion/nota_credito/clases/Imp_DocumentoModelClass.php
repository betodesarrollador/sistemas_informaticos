...
final class Imp_DocumentoModel extends Db{
  ...
  public function getEncabezado($encabezado_registro_id,$Conex){
	...
	if(is_numeric($encabezado_registro_id)){
        ...
        $select = "SELECT 
		 (SELECT logo FROM empresa WHERE empresa_id = of.empresa_id) AS logo, 
		...
		(SELECT a.cufe FROM abono_factura a WHERE a.encabezado_registro_id=e.encabezado_registro_id) AS cufe,
		...
		(SELECT f.consecutivo_factura FROM factura f, relacion_abono r, abono_factura a WHERE f.factura_id = r.factura_id AND r.abono_factura_id=a.abono_factura_id AND a.encabezado_registro_id=e.encabezado_registro_id) AS consecutivo_factura,
		(SELECT nombre FROM ubicacion WHERE ubicacion_id=of.ubicacion_id) AS ciudad_ofi

        FROM encabezado_de_registro  e, oficina of, tercero t 
		WHERE e.encabezado_registro_id = $encabezado_registro_id AND of.oficina_id=e.oficina_id AND t.tercero_id=e.tercero_id";		
