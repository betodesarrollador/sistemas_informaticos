...
function saveDetalle(obj){
	...
	if(validaRequeridosDetalle(obj,row)){
	  ...
	  var acuerdo_id       			   = $(Fila).find("input[name=acuerdo_id]").val();	  	  
	  var compromiso                   = $(Fila).find("input[name=compromiso]").val();
	  var prioridad			= $(Fila).find("select[name=prioridad] option:selected").val();
	  var checkProcesar                = $(Fila).find("input[name=procesar]");
	  	
	  if(!acuerdo_id.length > 0){
	    	      	      
	      var QueryString = "ACTIONCONTROLER=onclickSave&acuerdo_id="+acuerdo_id+"&compromiso="+
		  compromiso+'&acta_id='+acta_id+'&prioridad='+prioridad;
	      
	      $.ajax({
		      url        : "TerceroActaClass.php",
		      data       : QueryString,
		      beforeSend : function(){
			    setMessageWaiting();
		      },
	      ...
	      var QueryString = "ACTIONCONTROLER=onclickUpdate&acuerdo_id="+acuerdo_id+"&compromiso="+
		  compromiso+"&acta_id="+acta_id;
...
function deleteDetalleSolicitud(obj){
	...
	var QueryString   = "ACTIONCONTROLER=onclickDelete&acta_id="+acta_id+"&acuerdo_id="+acuerdo_id;
