	  ...
	  if(ValidaRequeridos(formulario)){ 
	    ...
	    if(this.id == 'guardar'){
			...
			if(parseInt(numDetalle) > 0){
				...
				$.ajax({
				  url        : "ParametrosLegalizacionParticularClass.php?rand="+Math.random(),
				  data       : QueryString,
				  beforeSend : function(){
				    showDivLoading();
				  },
				  success    : function(resp){				   
					   ...
					   ParametrosLegalizacionParticularOnSaveOnUpdateonDelete(formulario);					  
...
function ParametrosLegalizacionParticularOnSaveOnUpdateonDelete(formulario,resp){

   $("#refresh_QUERYGRID_parametros_legalizacion").click();
   ParametrosLegalizacionParticularOnReset(formulario);
...
function ParametrosLegalizacionParticularOnReset(formulario){
