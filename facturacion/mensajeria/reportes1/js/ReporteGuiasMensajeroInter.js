...
function sologuia(){
	...
	if ($('#allguia').val()=='S'){
		...
		$('#guia_interconexion_id').attr('disabled','');
		$("#desde,#hasta,#origen_id,#destino_id,#oficina_id,#estado_id,#reexpedido").val('');
		$("#allreexpedido").val('N');
		$('#desde,#hasta,#estado_id,#all_estado,#all_oficina,#all_servicio,#tipo_servicio_mensajeria_id,#si_mensajero,#oficina_id,#mensajero,#origen,#destino,#allreexpedido,#reexpedido,#consolidado').attr('disabled','true');
		$('#desde,#hasta,#estado_id,#si_mensajero,#oficina_id,#allreexpedido,#reexpedido,#consolidado,#all_servicio,#tipo_servicio_mensajeria_id').removeClass('obligatorio');
		$('#guia_interconexion_id').addClass('obligatorio');

	};
	if ($('#allguia').val()=='N'){

		$('#guia_interconexion_id').attr('disabled','true');
		$("#allreexpedido").val('S');
		...
		$('#guia_interconexion_id').removeClass('obligatorio');
		$('#guia_interconexion_id').css('border-color','none');

		$("#guia_interconexion_id").val('');
	};
...
function soloreexpedido(){
	...
	if ($('#allreexpedido').val()=='S'){
		...
		$("#desde,#hasta,#origen_id,#destino_id,#oficina_id,#estado_id,#guia_interconexion_id").val('');
		$('#desde,#hasta,#estado_id,#all_estado,#all_oficina,#si_mensajero,#oficina_id,#mensajero,#origen,#destino,#guia_interconexion_id,#all_servicio,#tipo_servicio_mensajeria_id').attr('disabled','true');
		$('#desde,#hasta,#estado_id,#si_mensajero,#oficina_id,#allguia,#guia_interconexion_id,#all_servicio,#tipo_servicio_mensajeria_id').removeClass('obligatorio');
		$('#reexpedido,#consolidado').addClass('obligatorio');

	};
	if ($('#allreexpedido').val()=='N'){

		$('#reexpedido,#guia_interconexion_id,#consolidado').attr('disabled','true');
...
function OnclickGenerar(formulario){
	...
	if(ValidaRequeridos(formulario)){
		 var QueryString = "ReporteGuiasMensajeroInterClass.php?ACTIONCONTROLER=generateReport&"+FormSerialize(formulario);
...
function OnclickGenerarExcel(formulario){
     if(ValidaRequeridos(formulario)){
       var QueryString = "ReporteGuiasMensajeroInterClass.php?ACTIONCONTROLER=generateReport&download=true&"+FormSerialize(formulario);
