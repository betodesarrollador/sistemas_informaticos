  ...
  <body>
    ...
    <table border="1" width="90%" align="center" style="font-size:13px;">
	 ...
	 <tr>
	  ...
	  <td>{$r.valor_total|number_format:0:',':'.'}&nbsp;</td>		  	  
	  
	  {foreach name=impuestos from=$DATA.impuestos item=i}
	   <td>{$r[$i.nombre]|number_format:0:',':'.'}&nbsp;</td>
	  {/foreach}
	  
	  <td>{$r.anticipos|number_format:0:',':'.'}&nbsp;</td>		  	  	  	  
	  	  	  	
	  {foreach name=descuentos from=$DATA.descuentos item=d}
	   <td>{$r[$d.nombre]|number_format:0:',':'.'}&nbsp;</td>
	  {/foreach}				
				  	  	  
	  <td>{$r.saldo_por_pagar|number_format:0:',':'.'}&nbsp;</td>		  	  	  	  	  	  	  	  
	 </tr>	 	  	 
	 {/foreach}
