  ...
  public function getReporte1($oficina_id,$estado,$desde,$hasta,$consulta_cliente,$clase,$Conex){ 
	 ...
	 (SELECT GROUP_CONCAT(ld.fecha) FROM liquidacion_despacho ld WHERE  ld.manifiesto_id = m.manifiesto_id AND ld.estado_liquidacion='L') AS fecha_liquidacion  
	  ...
	  SELECT 
		(SELECT nombre FROM oficina WHERE oficina_id=m.oficina_id) AS oficina_planilla,
		'RE' AS tipo,
		m.reexpedido AS planilla, 
		IF(r.nacional=1,'SI','NO') AS nacional, 
		'N/A' AS propio, 
		'N/A' AS placa,
		'N/A' AS conductor, 
		m.fecha_rxp AS fecha_planilla,
		(SELECT nombre FROM oficina WHERE oficina_id=r.oficina_id) AS oficina_remesa, 
		r.numero_remesa,
		CASE r.estado WHEN 'P' THEN 'PENDIENTE' WHEN 'PD' THEN 'PENDIENTE' WHEN 'PC' THEN 'PROCESANDO' WHEN 'MF' THEN 'MANIFESTADO' 
		WHEN 'LQ' THEN 'LIQUIDADA' WHEN 'FT' THEN 'FACTURADA' WHEN 'ET' THEN 'ENTREGADA' WHEN 'AN' THEN 'ANULADA' ELSE 'PENDIENTE' END AS estado,
	  ...
	  (SELECT GROUP_CONCAT(f.fecha) FROM factura f,detalle_factura df WHERE r.remesa_id = df.remesa_id AND f.factura_id = df.factura_id AND f.estado!='I') AS fecha_factura,
	  (SELECT GROUP_CONCAT(df.valor) FROM factura f,detalle_factura df WHERE df.remesa_id = r.remesa_id AND f.factura_id=df.factura_id AND f.estado!='I') AS valor_facturado,
	  (SELECT GROUP_CONCAT(ld.consecutivo) FROM liquidacion_reexpedido ld WHERE  ld.reexpedido_id = m.reexpedido_id AND ld.estado_liquidacion_reexpedido='L') AS numero_liquidacion,
	  (SELECT GROUP_CONCAT(ld. 	fecha_liquidacion_reexpedido) FROM liquidacion_reexpedido ld WHERE ld.reexpedido_id = m.reexpedido_id AND ld.estado_liquidacion_reexpedido='L') AS fecha_liquidacion  
	  
	  FROM remesa r, reexpedido m, detalle_despacho d 
	  WHERE r.remesa_id = d.remesa_id AND d.reexpedido_id = m.reexpedido_id AND r.fecha_remesa BETWEEN '$desde' AND '$hasta' 
	  AND r.oficina_id IN($oficina_id) AND r.estado IN ('$estado') AND r.clase_remesa IN ('$clase') $consulta_cliente
	  ...
	  r.fecha_anulacion,
	  (SELECT CONCAT_WS(' ',t.primer_nombre,t.primer_apellido) FROM usuario u, tercero t  WHERE r.usuario_anulo_id = u.usuario_id  AND u.tercero_id  = t.tercero_id)  AS usuario_anulacion,
	  IF(r.estado='FT',(SELECT GROUP_CONCAT(f.consecutivo_factura) FROM factura f,detalle_factura df WHERE r.remesa_id = df.remesa_id AND f.factura_id = df.factura_id AND f.estado!='I'),'N/A') AS numero_factura,
	  IF(r.estado='FT',(SELECT GROUP_CONCAT(f.fecha) FROM factura f,detalle_factura df WHERE r.remesa_id = df.remesa_id AND f.factura_id = df.factura_id AND f.estado!='I'),'N/A') AS fecha_factura,
	  IF(r.estado='FT',(SELECT GROUP_CONCAT(df.valor) FROM factura f,detalle_factura df WHERE df.remesa_id = r.remesa_id AND f.factura_id=df.factura_id AND f.estado!='I'),'N/A') AS valor_facturado,
	  ...
	  $results = $this -> DbFetchAll($select,$Conex);
		  ...
		  foreach($results as $items){
			  ...
			  $result[$i]=array(
				...
				conductor=>$items[conductor],fecha_planilla=>$items[fecha_planilla],oficina_remesa=>$items[oficina_remesa],
				...
				valor_flete=>$items[valor_flete],valor_seguro=>$items[valor_seguro],valor_otros=>$items[valor_otros],valor_total=>$items[valor_total],
				valor_liq_flete=>$items[valor_liq_flete],valor_liq_seguro=>$items[valor_liq_seguro],valor_liq_otros=>$items[valor_liq_otros],valor_liq_total=>$items[valor_liq_total],				
