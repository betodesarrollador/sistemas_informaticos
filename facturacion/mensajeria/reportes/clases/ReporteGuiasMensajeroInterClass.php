<?php
	...
	final class ReporteGuiasMensajeroInter extends Controler{
		...
		public function Main(){
			...
			$this -> noCache();
			require_once("ReporteGuiasMensajeroInterLayoutClass.php");
			require_once("ReporteGuiasMensajeroInterModelClass.php");

			$Layout   = new ReporteGuiasMensajeroInterLayout($this -> getTitleTab(),$this -> getTitleForm());
			$Model    = new ReporteGuiasMensajeroInterModel();
		...
		protected function generateReport(){
			...
			$Conex					= $this -> getConex();
			$guia_interconexion_id	= str_replace(",","','",trim($_REQUEST['guia_interconexion_id']));
			...
			if ($allguia=='N' AND $allreexpedido=='N'){
				...
				if ($si_mensajero=='' || $si_mensajero=='NULL' || $si_mensajero==NULL || $si_mensajero=='ALL'){
					...
					$consulta_mensajero=" AND g.guia_interconexion_id IN (SELECT dd.guia_interconexion_id FROM reexpedido r, detalle_despacho_guia dd WHERE  r.proveedor_id =".$mensajero_id." AND dd.reexpedido_id=r.reexpedido_id)";
				}

				$arrayReporte=$Model -> getReporte1($desde,$hasta,$oficina,$estado_id,$tipo_servicio_mensajeria_id,$consulta_origen,$consulta_destino,$consulta_mensajero,$this -> getConex());

			}
			if ($allreexpedido=='S' || $allguia=='S') {

				if ($allguia=='S') {
					$documento=" g.numero_guia IN ('".$guia_interconexion_id."')";
					$arrayReporte=$Model -> getReporte3($documento,$Conex); 
				}elseif ($allreexpedido=='S') {
					if($consolidado=='S'){
						$documento="g.guia_interconexion_id=dg.guia_interconexion_id AND g.estado_mensajeria_id=4 AND r.reexpedido_id=dg.reexpedido_id AND r.reexpedido=".$reexpedido;
						$transito=$Model -> getReporte4($documento,$Conex);
						
						$documento="g.guia_interconexion_id=dg.guia_interconexion_id AND g.estado_mensajeria_id=6 AND r.reexpedido_id=dg.reexpedido_id AND r.reexpedido=".$reexpedido;
						$entregado=$Model -> getReporte4($documento,$Conex);
						
						$documento="g.guia_interconexion_id=dg.guia_interconexion_id AND g.estado_mensajeria_id=7 AND r.reexpedido_id=dg.reexpedido_id AND r.reexpedido=".$reexpedido;
						$devuelto=$Model -> getReporte4($documento,$Conex);
					}else{
						$documento="g.guia_interconexion_id=dg.guia_interconexion_id AND r.reexpedido_id=dg.reexpedido_id AND r.reexpedido=".$reexpedido;
			...
			$Layout -> setJsInclude("/velotax/mensajeria/reportes/js/ReporteGuiasMensajeroInter.js");
			...
			if($download == 'true'){
				$Layout -> exportToExcel('DetallesReporteGuiasMensajeroInter.tpl');
			}else{
				$Layout -> RenderLayout('DetallesReporteGuiasMensajeroInter.tpl');
			}
		}
		...
		protected function setCampos(){
			...
			$this -> Campos[guia_interconexion_id] = array(
				name=>'guia_interconexion_id',
				id=>'guia_interconexion_id',
				// required=>'yes',
				type=>'text',
				disabled=>'true'
			); 
	...
	$ReporteGuiasMensajeroInter = new ReporteGuiasMensajeroInter();
?>