...
function all_forma_pago(){
	
	if(document.getElementById('all_forma').checked==true){
		$('#all_forma').val('SI');
		
		var objSelect = document.getElementById('forma_pago_mensajeria_id'); 
		var numOp     = objSelect.options.length -1;	   
...
function OnclickGenerar(formulario){
	...
	if(ValidaRequeridos(formulario)){
		 var QueryString = "DetallesReporteGuiasEncomiendaClass.php?"+FormSerialize(formulario);
...
function OnclickGenerarExcel(formulario){
     if(ValidaRequeridos(formulario)){
       ...
       var QueryString = "DetallesReporteGuiasEncomiendaClass.php?ACTIONCONTROLER=generateFile&"+FormSerialize(formulario)
       +"&download=true&rand="+Math.random();
...
function OnclickGenerarExcelFormato(formulario){
     if(ValidaRequeridos(formulario)){
       ...
       var QueryString = "DetallesReporteGuiasEncomiendaClass.php?ACTIONCONTROLER=generateFileFormato&"+FormSerialize(formulario)
       +"&download=true&rand="+Math.random();
...
function OnclickGenerarExcelFiltrado(formulario){
     if(ValidaRequeridos(formulario)){
       ...
       var QueryString = "DetallesReporteGuiasEncomiendaClass.php?ACTIONCONTROLER=generateFileFormato1&"+FormSerialize(formulario)+"&download=true&rand="+Math.random();
...
function OnclickGenerarFotos(formulario){
     if(ValidaRequeridos(formulario)){
	   if($('#si_remitente').val()=='1'){
		   ...
		   var QueryString = "DetallesReporteGuiasEncomiendaClass.php?ACTIONCONTROLER=generateFotos&"+FormSerialize(formulario)+"&download=SI&rand="+Math.random();
