...
final class ProveedorModel extends Db{
  ...
  public function Save($Campos,$Conex){	
    ...
    $this -> Begin($Conex);
	  ...
	  $this -> assignValRequest('mensajero',1);
  ...
  public function GetTipoId($Conex){
   ...
   public function GetTipoRegimen($Conex){
	return $this -> DbFetchAll("SELECT regimen_id AS value,nombre AS text, '1' AS selected FROM regimen",$Conex,
	$ErrDb = false);
   }
   ...
   $Query = "SELECT 
		...
		WHERE t.tercero_id = p.tercero_id AND p.mensajero=1";
   return $Query;
   }
}
