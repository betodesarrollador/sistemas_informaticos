	...
	$Cols = array(
	  ...
	  array(name=>'estado',		                index=>'estado',					    sorttype=>'text',	width=>'100',	align=>'center')
        ...
        $Titles = array('PLACA',			'MARCA',			'LINEA','CONFIGURACION','CARROCERIA',			'MODELO',			'MODELO REPOTENCIADO',			'COLOR',
						...
						'N° SOAT','ASEGURADORA','VENCIMIENTO','PROPIO','ESTADO');
...
/*	}else{
  ...
  protected function sendVehiculoMintransporte(){
    ...
    $webService -> sendVehiculoMintransporte($data);	      
...
//BUSQUEDA
  ...
  protected function onclickPrint(){
    ...
    $print = new Imp_HV_Vehiculo($this -> getConex());
  ...
  protected function setCampos(){
	...
	$this -> Campos[propio] = array(
		...
		type	=>'select',
		required=>'yes',
		options => array(array(value => 1, text => 'SI', selected => 1),array(value => 0, text => 'NO', selected => 1)),
		datatype=> array(type=>'integer'),
		transaction=>array(
			table	=>array('vehiculo'),
			type	=>array('column'))
	...
	$this -> Campos[conductor_id] = array(
 	...
 	$this -> Campos[archivo_imagen_frontal] = array(
		...
		type	=>'file',
		value	=>'',
		path	=>'/velotax/imagenes/transporte/vehiculos/',
		size	=>'70',
		datatype=>array(
			type	=>'file'),
	 	transaction=>array(
