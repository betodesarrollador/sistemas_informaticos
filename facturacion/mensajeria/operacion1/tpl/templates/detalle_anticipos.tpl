...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN" "http://www.w3.org/TR/html4/strict.dtd">
	   ...
	   {if count($ANTICIPOS) > 0}
		 {foreach name=anticipos from=$ANTICIPOS item=a}
		   <tr class="rowAnticipo" align="center">
			 ...
			 <td>
			   <input type="text" name="numero_soporte" id="numero_soporte" value="{$f.numero_soporte}" />
			 </td>
			 <td><div id="divCuentaTipoPago"> {if count($a.cuentas_forma_pago) > 0}
  ...
  <select name="select" id="select" disabled>
    <option value="NULL">(... Seleccione ...)</option>
			 ...
			 {*<td><input type="checkbox" name="anular" {if not is_numeric($a.encabezado_registro_id) }disabled{/if} /></td>*}		   </tr>	     
		 {/foreach}
	   {/if}
