...
final class Imp_OrdenSeguimientoModel extends Db{
  ...
  public function getOrdenSeguimiento($Conex){
	...
	if(is_numeric($seguimiento_id)){
		...
		(SELECT nombre FROM ubicacion WHERE ubicacion_id=o.destino_id) AS destino,
		 ...
		 (SELECT TRIM(nombre) FROM ubicacion WHERE ubicacion_id=of.ubicacion_id) AS ciudad_ofi,o.estado,
		 (SELECT SUM(valor) FROM anticipos_particular WHERE seguimiento_id = o.seguimiento_id AND entrega = 'E') AS valor_anticipo_empresa,
		 (SELECT SUM(valor) FROM anticipos_particular WHERE seguimiento_id = o.seguimiento_id AND entrega = 'C') AS valor_anticipo_cliente,
		 o.valor_flete,o.saldo_por_pagar,o.valor_neto_pagar
  ...
  public function getImpuestos($seguimiento_id,$Conex){
