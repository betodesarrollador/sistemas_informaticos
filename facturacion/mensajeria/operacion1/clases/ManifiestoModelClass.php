 ...
 public function retirar_remesa_inter($guia_interconexion_id,$reexpedido_id,$Conex){
  ...
  public function Update($usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$Campos,$Conex){ 
    ...
    $this -> Begin($Conex);
	 ...
	 $update = "UPDATE reexpedido SET obser_rxp=$obser_rxp, fecha_rxp=$fecha_rxp, destino_id=$destino_id, proveedor_id=$proveedor_id WHERE reexpedido_id = $reexpedido_id";
	 $this -> query($update,$Conex,true);	
...
//BUSQUEDA
  public function selectReexpedidos($reexpedido_id,$Conex){
   ...
   $select = "SELECT r.*,
   (SELECT COUNT(dd.guia_id) FROM detalle_despacho_guia dd WHERE dd.reexpedido_id = $reexpedido_id) AS n_guias,
