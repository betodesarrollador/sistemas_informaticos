...
final class GuiaManualCreditoModel extends Db{
  ...
  public function getTipoEnvioMetro($origen_id,$destino_id,$Conex){ 
   ...
   public function setInsertDetalleSolicitud($rowInsert,$oficina_id,$usuario_id,$Conex){
	  ...
	  $select_guia = "SELECT numero_guia FROM guia WHERE numero_guia=$numero_guia AND prefijo='RC'";
	...
	if($this -> getTipoEnvioMetro($origen_id,$destino_id,$Conex) ) $tipo_envio_id=2;	
	  ...
	  $insert = "INSERT INTO guia (guia_id,oficina_id,tipo_servicio_mensajeria_id,tipo_envio_id,forma_pago_mensajeria_id,crm,
						...
						descripcion_producto,peso,medida_id,cantidad,valor,valor_flete,valor_seguro,valor_total,manual,proceso,pedido,unidad,usuario_id) 
	  VALUES ($guia_id,$oficina_id,5,$tipo_envio_id,1,0,
			  1,'EL','$fecha_guia',$numero_guia,'MP',$origen_id,$destino_id,
			  '$remitente','$doc_remitente','$direccion_remitente','$telefono_remitente',
			  '$destinatario','$doc_destinatario','$direccion_destinatario','$telefono_destinatario', 
			  '$descripcion_producto',$peso,38,$cantidad,$valor,$valor_flete,$valor_seguro,$valor_total,1,$PROCESO,$PEDIDO,$UNIDAD,$usuario_id);";
