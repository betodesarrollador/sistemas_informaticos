...
final class Imp_DocumentoLegalizacion{

  private $Conex;
  private $oficina_id;
  private $empresa_id;
  
  public function __construct($oficina_id,$empresa_id,$Conex){    
  ...
  public function printOut(){
      ...
      require_once("Imp_DocumentoLegalizacionLayoutClass.php");
      require_once("../../../framework/clases/fpdf/fpdf.php");		 
      require_once("Imp_DocumentoLegalizacionModelClass.php");
		
      $Layout = new Imp_DocumentoLegalizacionLayout();
      $Model  = new Imp_DocumentoLegalizacionModel();		
	  	  
	  $legalizacion_manifiesto_id = $_REQUEST['legalizacion_manifiesto_id'];
	  
      $encabezado   = $Model -> getEncabezado($this -> oficina_id,$legalizacion_manifiesto_id,$this -> Conex);	  
      $imputaciones = $Model -> getimputaciones($this -> empresa_id,$this -> oficina_id,$legalizacion_manifiesto_id,$this -> Conex);  
      $total        = $Model -> getTotal($legalizacion_manifiesto_id,$this -> Conex);  
      $valor_letras = $Model -> getTotales($legalizacion_manifiesto_id,$this -> Conex);
	  ...
	  $pdf->Cell(20,5,$encabezado[0]['manifiesto'],1,0,'C');		
	  ...
	  $pdf->Cell(25,5,'Origen - Dest ',1,0,'L');	
	  $pdf->Cell(60,5,$encabezado[0]['origen'].' - '.$encabezado[0]['destino'],1,0,'L');		 	 	 
	  ...
	  $pdf->Cell(60,5,'ANT / '.$encabezado[0]['observaciones'],1,0,'L');		  

	 /* $pdf->Ln(5);	
      $pdf->SetFont('Arial','B',10);	 	 	 	  
	  $pdf->Cell(25,5,'Forma Pago :',1,0,'L');	
	  $pdf->Cell(90,5,$encabezado[0]['formapago'],1,0,'L');	
	  ...
	  for($i=0; $i < count($imputaciones); $i++){
	   ...
	   $pdf->Cell(40,5,$imputaciones[$i]['tercero_imputacion'],1,0,'C');	
