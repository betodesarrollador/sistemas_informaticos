  ...
  public function getPermiso($ActividadId,$Permiso,$Conex){
	...
	public function Save($Campos,$oficina_id,$empresa_id,$modifica,$usuario_id,$Conex){
		...
		if(($remitente_id=='NULL' || $remitente_id=='') && $doc_remitente>0){
			 ...
			 $insert = "INSERT INTO remitente_destinatario (remitente_destinatario_id,numero_identificacion,tipo_identificacion_id,nombre,direccion,telefono,ubicacion_id,tipo,estado,guia)
						VALUES ($remitente_destinatario_id,$doc_remitente,1,'$remitente','$direccion_remitente','$telefono_remitente',$origen_id,'R','D',1)";
			 $this -> query($insert,$Conex,true);  
			 $this -> assignValRequest('remitente_id',$remitente_destinatario_id);

			
		}elseif($remitente_id>0){
			 $update = "UPDATE remitente_destinatario SET nombre='$remitente', direccion='$direccion_remitente',telefono='$telefono_remitente',ubicacion_id= $origen_id
		...
		if(($destinatario_id=='NULL' || $destinatario_id=='') && $doc_destinatario>0){
			 ...
			 $remitente_destinatario_id = $this -> DbgetMaxConsecutive("remitente_destinatario","remitente_destinatario_id",$Conex,false,1);
			 $insert = "INSERT INTO remitente_destinatario (remitente_destinatario_id,numero_identificacion,tipo_identificacion_id,nombre,direccion,telefono,ubicacion_id,tipo,estado,guia)
						VALUES ($remitente_destinatario_id,$doc_destinatario,1,'$destinatario','$direccion_destinatario','$telefono_destinatario',$destino_id,'D','D',1)";
			 $this -> query($insert,$Conex,true); 
			 $this -> assignValRequest('destinatario_id',$remitente_destinatario_id);
		}elseif($destinatario_id>0){
			 $update = "UPDATE remitente_destinatario SET nombre='$destinatario', direccion='$direccion_destinatario',telefono='$telefono_destinatario' ,ubicacion_id= $destino_id
		...
		if($_REQUEST['numero_guia']>0){
			...
			$select1 = "SELECT  guia_encomienda_id FROM guia_encomienda WHERE  numero_guia=$numero_guia AND prefijo='$prefijo'  AND manual=0 AND crm=1";//oficina_id = $oficina_id AND
			...
			$select = "SELECT MAX(numero_guia) AS numero_guia FROM guia_encomienda WHERE  manual=0 AND crm=1";//oficina_id = $oficina_id  AND
			$result = $this -> DbFetchAll($select,$Conex,true);
			$numero_guia = ($result[0]['numero_guia']+1);
		}
		
		$selectcom = "SELECT p.*, t.tipo_documento_id FROM rango_guia_encomienda p, tipo_bien_servicio_factura t WHERE p.tipo='C' AND  p.estado='A' AND t.tipo_bien_servicio_factura_id=p.tipo_bien_servicio_factura_id"; //p.oficina_id = $oficina_id AND
		...
		if(is_numeric($numero_guia)){
			$select = "SELECT rango_guia_encomienda_fin,prefijo FROM rango_guia_encomienda WHERE tipo='C' AND  estado = 'A'";//oficina_id = $oficina_id AND
			$result = $this -> DbFetchAll($select,$Conex,true);
			...
			$select = "SELECT rango_guia_encomienda_ini, prefijo FROM rango_guia_encomienda WHERE tipo='C' AND  estado = 'A'"; //oficina_id = $oficina_id AND
			$result = $this -> DbFetchAll($select,$Conex,true);
...
//////////////////////////////////////////
  ...
  public function getClientes($oficina_id,$Conex){
   ...
   public function GetTipoMedida($Conex){
	...
	$result = $this -> DbFetchAll("SELECT tipo_servicio_mensajeria_id AS value,nombre AS text FROM tipo_servicio_mensajeria WHERE guia_encomienda=1 ORDER BY nombre ASC",$Conex,false);
    return $result;
  }  
/*
