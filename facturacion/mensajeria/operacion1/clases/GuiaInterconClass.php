...
final class GuiaIntercon extends Controler{
  ...
  public function Main(){
    ...
    require_once("GuiaInterconLayoutClass.php");    
    require_once("GuiaInterconModelClass.php");
	
    $Layout   = new GuiaInterconLayout($this -> getTitleTab(),$this -> getTitleForm());
    $Model    = new GuiaInterconModel();
...
///////////////////////
	...
	$Attributes = array(
	  id		=>'GuiaOficinas',
	  title		=>'Listado de Guias Interconexion',
	  sortname	=>'guia_interconexion',
	  sortorder =>'ASC',
	  width		=>'auto',
	  height	=>'250',
	...
	$Cols = array(
	  array(name=>'guia_interconexion',		           index=>'guia_interconexion',	        sorttype=>'text',	width=>'60',	align=>'center' ),      	  
	...
	$Layout -> SetGridGuiaInterconOficinas($Attributes,$Titles,$Cols,$Model -> getQueryGuiaInterconOficinasGrid($this -> getOficinaId()));	
	$Layout -> RenderMain();    
  }
  ...
  protected function onclickValidateRow(){
    ...
    $Data = new ValidateRow($this -> getConex(),"guia_interconexion",$this ->Campos);
    print $Data  -> GetData();
  }
  ...
  protected function onclickUpdate(){  
	...
	if($clase_guia == 'CP' && $Model -> esGuiaInterconPrincipal($numero_guia,$this -> getConex())){	
  ...
  protected function onclickDelete(){
	...
	protected function OnClickAnular(){
		...
		$Model -> Anular($guia_interconexion_id,$this -> getConex());
		if($Model -> GetNumError() > 0){
  ...
  protected function onclickCancellation(){    
	 ...
	 $Model -> cancellation($guia_interconexion_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$this -> getConex());	
...
//BUSQUEDA
  protected function onclickFind(){
  	require_once("GuiaInterconModelClass.php");
    $Model = new GuiaInterconModel();	
    $Data =  $Model -> selectGuiaIntercon($this -> getConex());	
  ...
  protected function getGuiaInterconComplemento(){  
  	require_once("GuiaInterconModelClass.php");
    $Model = new GuiaInterconModel();	
	$numero_guia = $_REQUEST['numero_guia'];	
    $Data =  $Model -> selectGuiaInterconComplemento($numero_guia,$this -> getConex());		
	if(count($Data) > 0){
      $this -> getArrayJSON($Data);	
	}else{
        print 'false';
  ...
  protected function getOptionsTipoEnvio(){
	...
	$field[tipo_envio_id] = array(
		...
		datatype=>array(
			type	=>'integer'),
		transaction=>array(
			table	=>array('guia_interconexion'),
			type	=>array('column'))
	);		
...
////////// INFORMACION GENERAL
	...
	$this -> Campos[tipo_envio_id] = array(
		...
		required => 'yes',		
		datatype => array(type=>'integer'),
		transaction => array(table =>array('guia_interconexion'), type =>array('column'))
	);	
...
////////// FIN PARA IMPRESION DE FLETE
	$this -> Campos[guias_interconexion] = array(
		name	=>'guias_interconexion',
		id		=>'guias_interconexion',
		type	=>'text'
	);
	...
	$this -> Campos[formato] = array(
		...
		type	=>'select',
		options	=> array(
					array(value => 'SI', text => 'Impresion GuiaIntercon', selected => 'SI'),
    ...
    type=>array('column')),
   	...
   	$this -> Campos[busqueda] = array(
		...
		tabindex=>'1',
		suggest=>array(
			name	=>'busca_guia_interconexion',
			setId	=>'guia_interconexion_id',
...
//////////BOTONES
	...
	$this -> Campos[actualizar] = array(
  	...
  	$this -> Campos[borrar] = array(
    	...
    	property=>array(
			name	=>'delete_ajax',
			onsuccess=>'GuiaInterconOnDelete')
	);
...
$guia_interconexion = new GuiaIntercon();
