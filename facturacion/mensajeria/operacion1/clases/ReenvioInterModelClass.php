...
final class ReenvioInterModel extends Db{
...
/////  
  ...
  public function setLeerCodigobar($guia,$Conex){
	...
	AND g.guia_interconexion_id  IN (SELECT dd.guia_interconexion_id FROM detalle_devolucion dd, devolucion d WHERE  dd.devolucion_id=d.devolucion_id AND d.estado='D')";
    $result = $this-> DbFetchAll($select,$Conex,true);
    return $result;
 }
  ...
  public function cancellation($reenvio_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$Conex){ 
 	...
 	$select = "SELECT d.guia_interconexion_id FROM detalle_reenvio d 
	WHERE d.reenvio_id = $reenvio_id AND 
	 d.guia_interconexion_id IN (SELECT dd.guia_interconexion_id FROM detalle_entrega dd, entrega dv WHERE dv.estado='E' AND dd.entrega_id=dv.entrega_id )";
	$result = $this -> DbFetchAll($select,$Conex,true);	
   ...
   $this -> Begin($Conex);  
	 ...
	 $update = "UPDATE guia_interconexion SET  estado_mensajeria_id = 7 WHERE guia_interconexion_id IN (SELECT guia_interconexion_id FROM detalle_reenvio WHERE reenvio_id = $reenvio_id)";
	 $this -> query($update,$Conex,true);	
   ...
   public function Save($usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$Campos,$Conex){
	...
	if(is_array($_REQUEST['g'])){
	  ...
	  for($i = 0; $i < count($guias); $i++){
		...
		$select = "SELECT numero_guia FROM guia_interconexion  WHERE guia_interconexion_id = $guia_id AND estado_mensajeria_id!=7";
		$result = $this  -> DbFetchAll($select,$Conex,true);						
		...
		if($result[0]['numero_guia']>0){
		 ...
		 $insert = "INSERT INTO detalle_reenvio (detalle_reenvio_id, guia_interconexion_id,reenvio_id)
					VALUES ($detalle_reenvio_id,$guia_id,$reenvio_id)";

		$this -> query($insert,$Conex,true);						  

		 $update = "UPDATE guia_interconexion SET estado_mensajeria_id=1, desbloqueada=1 WHERE guia_interconexion_id=$guia_id ";
...
//BUSQUEDA
  public function selectReenvioInter($reenvio_id,$Conex){
...
//// GRID ////
  public function getQueryReenvioInterGrid(){
