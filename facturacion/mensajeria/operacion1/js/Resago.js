...
function onclickUpdate(formulario){
	...
	// var resago_id	= document.getElementById('resago_id').value;
	// var obser_ent		= document.getElementById('obser_ent').value;

	// // alertJquery(resago_id+"-"+obser_dev+"");
	// if(!isNaN(resago_id)){
	// 	var QueryString = "ACTIONCONTROLER=onclickUpdate&resago_id="+resago_id+"&obser_ent="+obser_ent;
	// 	$.ajax({
	// 		url        : "ResagoClass.php?rand="+Math.random(),
	// 		data       : QueryString,
	// 		beforeSend : function(){
	// 			showDivLoading();
	// 		},
		...
		if (e.which == 13) {	
			...
			if(document.getElementById('codigo_barras1').value!=''){						
				...
				if(guia == "NULL" || guia == ""){
					...
					$("#codigo_barras1").val('');
					$.ajax({
					   url        : 'ResagoClass.php?random='+Math.random(),
					   data       : QueryString,
					   beforeSend : function(){
						   showDivLoading();
					   },
						...
						try{
						   ...
						   var data       = $.parseJSON(resp);
							if(data[0]['estado_mensajeria_id']=='10'){
								...
								$("#mensaje_alerta").html("La guia "+guia1+",<br> No se encuentra en estado EN OFICINA.");
							}
						}catch(e){
...
function setDataFormWithResponse(){
	...
	// var QueryString = "ACTIONCONTROLER=onclickFind&resago_id="+$('#resago_id').val();
...
function EntregaOnSave(formulario,resago_id){		
   
	// if(parseInt(resago_id)>0){
...
function beforePrint(formulario,url,title,width,height){
	...
	// var resago_id = parseInt($("#resago_id").val());
...
function onclickCancellation(formulario){
 ...
 //       if(ValidaRequeridos(formulario)){
	...
	//      var QueryString = "ACTIONCONTROLER=onclickCancellation&"+FormSerialize(formulario)+"&resago_id="+$("#resago_id").val();
