...
function checkRow(obj){  
  ...
  if(numChecks > numMaxGuia){
   alertJquery('Solo puede Seleccionar un Maximo de <span style="color:#red">'+numMaxGuia+'</span> productos por guia !!!','Validacion Solicitud Servicio Guia')
   obj.checked = false;
  }  
}
...
function setSolicitud(){
    ...
    $(document).find("input[type=checkbox]:checked").each(function(){
      ...
      if(orden_despacho == '' && orden_despacho_tmp == ''){	
        orden_despacho   = parseInt($(this.parentNode.parentNode).find("td[aria-describedby=QUERYGRID_SolicServToGuiaPaqueteo_orden_despacho]").text());  	
	    orden_despacho_tmp = orden_despacho;
		
      }else{	
         orden_despacho_tmp = parseInt($(this.parentNode.parentNode).find("td[aria-describedby=QUERYGRID_SolicServToGuiaPaqueteo_orden_despacho]").text());  	
	...
	$.ajax({
	  ...
	  url     : "SolicServToGuiaClass.php?rand="+Math.random(),
	  data    : QueryString,
