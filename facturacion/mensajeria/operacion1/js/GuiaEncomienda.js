   ...
   $("#guardar,#actualizar").click(function(){
	...
	$("#peso").blur(function(){	  
	   if(parseInt(this.value)<5000){//5000
		   $("#peso").val('');
	   		alertJquery("No se permite valores inferiores a 5.000 Gramos","VALIDACION");
...
function GuiaOnSave(formulario,resp){
  ...
  if(guia_encomienda_numero > 0){
	...
	var url = "GuiaEncomiendaClass.php?ACTIONCONTROLER=onclickPrint&rango_desde="+guia_encomienda_numero+"&rango_hasta="+guia_encomienda_numero+"&formato=SI&random="+Math.random();	
...
function printOut(){		
	...
	var url = "GuiaEncomiendaClass.php?ACTIONCONTROLER=onclickPrint&rango_desde="+rango_desde+"&rango_hasta="+rango_hasta+"&formato="+formato+"&orden_servicio="+orden_servicio+"&fecha_guia_crea="+fecha_guia_crea+"&random="+Math.random();	
...
function updateGridGuia(){
	$("#refresh_QUERYGRID_guia_encomienda").click();
}
...
function recalculaValorvol(){
	...
	if(parseInt(largo)>0 && parseInt(ancho)>0 && parseInt(alto)>0){ 
		...
		if(parseFloat(pesovol)<5){
			alertJquery("El peso volumen no puede ser menor a 5 Kilogramos","Validacion");
			$("#peso_volumen").val(''); 
		}else{
