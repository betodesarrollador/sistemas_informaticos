...
function onclickUpdate(formulario){
	...
	// var entrega_oficina_id	= document.getElementById('entrega_oficina_id').value;
	// var obser_ent		= document.getElementById('obser_ent').value;

	// // alertJquery(entrega_oficina_id+"-"+obser_dev+"");
	// if(!isNaN(entrega_oficina_id)){
	// 	var QueryString = "ACTIONCONTROLER=onclickUpdate&entrega_oficina_id="+entrega_oficina_id+"&obser_ent="+obser_ent;
	// 	$.ajax({
	// 		url        : "EntregaOficinaClass.php?rand="+Math.random(),
	// 		data       : QueryString,
	// 		beforeSend : function(){
	// 			showDivLoading();
	// 		},
		...
		if (e.which == 13) {	
			...
			if(document.getElementById('codigo_barras1').value!=''){						
				...
				if(guia == "NULL" || guia == ""){
					...
					$("#codigo_barras1").val('');
					$.ajax({
					   url        : 'EntregaOficinaClass.php?random='+Math.random(),
					   data       : QueryString,
					   beforeSend : function(){
						   showDivLoading();
					   },
...
function setDataFormWithResponse(){
	...
	// var QueryString = "ACTIONCONTROLER=onclickFind&entrega_oficina_id="+$('#entrega_oficina_id').val();
 ...
 //       url        : 'EntregaOficinaClass.php?random='+Math.random(),
	...
	// 	   var data       = $.parseJSON(resp);
	// 	   var entrega = data[0]['entrega_oficina_id'];
...
function EntregaOnSave(formulario,entrega_oficina_id){	 	
   
	// if(parseInt(entrega_oficina_id)>0){
	...
	//   if($('#limpiar'))    $('#limpiar').attr("disabled","");
	//    $('#entrega_oficina_id').val(entrega_oficina_id);
...
function beforePrint(formulario,url,title,width,height){
	...
	// var entrega_oficina_id = parseInt($("#entrega_oficina_id").val());
...
function onclickCancellation(formulario){
 ...
 //       if(ValidaRequeridos(formulario)){
	...
	//      var QueryString = "ACTIONCONTROLER=onclickCancellation&"+FormSerialize(formulario)+"&entrega_oficina_id="+$("#entrega_oficina_id").val();
		
	//      $.ajax({
 //           url  : "EntregaOficinaClass.php",
	//        data : QueryString,
	//        beforeSend: function(){
	// 		   showDivLoading();
	//        },
 ...
 //    }else{
	...
	//  var entrega_oficina_id = $("#entrega_oficina_id").val();
