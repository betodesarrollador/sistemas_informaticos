...
final class LiquidacionGuiasIntercon extends Controler{
	...
	public function Main(){
		...
		$this -> noCache();
		require_once("LiquidacionGuiasInterconLayoutClass.php");
		require_once("LiquidacionGuiasInterconModelClass.php");

		$Layout   = new LiquidacionGuiasInterconLayout($this -> getTitleTab(),$this -> getTitleForm());
		$Model    = new LiquidacionGuiasInterconModel();
	...
	protected function OnClickSave(){
		...
		if($guias_interconexion_id=='todas'){
			...
			$result =  $Model -> getGuiasLiqUni($desde,$hasta,$cliente_id,$this -> getOficinaId(),$oficina_id1,$guias_interconexion_id,$this -> getConex());
		}
		$mensaje='';
		for($i=0;$i<count($result);$i++){
			$guia_interconexion_id1=$result[$i]['guia_interconexion_id'];
			...
			if($valor_total>0 && $total!=$valor_total){
				$resultado = $Model -> getActualizarValor($guia_interconexion_id1,$tipo_envio_id, $total,$valor_decla,$valorinicial,$manejo,$valor_otros,$this -> getConex());	
				if($resultado>0){
	...
	public function getConsecutivo(){
		...
		require_once("LiquidacionGuiasInterconModelClass.php");
		$Model = new LiquidacionGuiasInterconModel();
		$liquidacion_guias_interconexion_id			= $_REQUEST['liquidacion_guias_interconexion_id'];
		$result = $Model -> getConsecutivoReal($liquidacion_guias_interconexion_id,$this -> getConex());
		// echo "$result";
		exit("$result");
	}
  ...
  protected function onclickCancellation(){  
		...
		$result = $Model -> cancellation($liquidacion_guias_interconexion_id,$observacion_anulacion,$fecha_anulacion,$this -> getUsuarioId(),$this -> getConex());
...
//BUSQUEDA
	protected function onclickFind(){
		...
		$Model                   = new LiquidacionGuiasInterconModel();
		$liquidacion_guias_interconexion_id = $_REQUEST['liquidacion_guias_interconexion_id'];
		$Data  = $Model -> selectLiquidacionGuiasIntercon($liquidacion_guias_interconexion_id,$this -> getConex());
	...
	public function GetValorPorFacturado(){
		...
		if($_REQUEST[guias_interconexion_id]=='todas'){
			...
			$result = $Model -> getValorPorFacturadoInd($_REQUEST[guias_interconexion_id],$_REQUEST[cliente_id],$_REQUEST[desde],$_REQUEST[hasta],$this -> getConex());
		}
		exit("$result");
	}
	...
	public function GetUltimo(){
		...
		$result = $Model -> getUltimo($_REQUEST[cliente_id],$this -> getConex());
	...
	protected function onclickPrint(){
		...
		require_once("Imp_LiquidacionGuiasInterconClass.php");
		$print = new Imp_LiquidacionGuiasIntercon($this -> getConex());
		$print -> printOut();
	}
	...
	protected function setCampos(){
		...
		$this -> Campos[liquidacion_guias_interconexion_id] = array(
			name	=>'liquidacion_guias_interconexion_id',
			id	=>'liquidacion_guias_interconexion_id',
			type	=>'hidden',
			datatype=>array(
				type	=>'autoincrement',
				length	=>'20'),
			transaction=>array(
				table	=>array('liquidacion_guias_interconexion'),
		...
		$this -> Campos[guias_interconexion_id] = array(
			name	=>'guias_interconexion_id',
			id		=>'guias_interconexion_id',
			type	=>'hidden',
			required=>'yes',
			datatype=>array(
				type	=>'text',
		...
		$this -> Campos[UltimaLiquidacion] = array(
			name		=>'UltimaLiquidacion',
			id			=>'UltimaLiquidacion',
			readonly	=>'yes',
			size	=>'25',
			type		=>'text',
			datatype=>array(
		...
		$this -> Campos[busqueda] = array(
			...
			size	=>'85',
			suggest=>array(
				name	=>'liquidacion_guias_intercon_facturar',
				setId	=>'liquidacion_guias_interconexion_id',
...
$LiquidacionGuiasIntercon = new LiquidacionGuiasIntercon();
?>