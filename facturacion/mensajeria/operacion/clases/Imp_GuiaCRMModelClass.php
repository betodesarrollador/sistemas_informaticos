...
final class Imp_GuiaCRMModel extends Db{
  ...
  public function getGuia($oficina_id,$empresa_id,$Conex){
	...
	$select = "SELECT r.fecha_guia,r.valor_flete,r.valor_seguro,r.valor_otros,r.valor_total,r.peso_volumen, r.zona, r.correo_destinatario, r.correo_remitente,	(SELECT abreviatura FROM medida WHERE medida_id=r.medida_id) AS medida,
	...
	(SELECT numero_resolucion FROM rango_guia_crm WHERE tipo='C' AND ESTADO='A' ORDER BY fecha_rango_guia DESC LIMIT 1) AS resolucion_dian,
	(SELECT  fecha_rango_guia  FROM rango_guia_crm WHERE tipo='C' AND ESTADO='A' ORDER BY fecha_rango_guia DESC LIMIT 1) AS fecha_dian,	
	(SELECT rango_guia_crm_ini FROM rango_guia_crm WHERE tipo='C' AND ESTADO='A' ORDER BY fecha_rango_guia DESC LIMIT 1) AS rango_dian_ini,	
	(SELECT rango_guia_crm_fin FROM rango_guia_crm WHERE tipo='C' AND ESTADO='A' ORDER BY fecha_rango_guia DESC LIMIT 1) AS rango_dian_fin,
	...
	FROM guia r WHERE r.guia_id>0 AND r.crm=1 $guias $rangos  $ordensql $fechasql  ORDER BY r.guia_id ASC";	
