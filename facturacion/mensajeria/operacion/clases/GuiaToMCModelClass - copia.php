...
////////// APLICAR FILTRO
  public function aplicarFiltro($Campos,$Conex){
...
//// GRID ////
  public function getQueryGuiaToMCGrid($oficina_id){
	  ...
	  /*$Query = "(SELECT CONCAT('<input type=\"checkbox\" value=\"',r.guia_id,'\" />') AS link,
			  r.numero_guia AS guia,(SELECT CONCAT_WS(' ',razon_social,primer_nombre,segundo_nombre,primer_apellido,segundo_apellido)
			  FROM tercero WHERE tercero_id = (SELECT tercero_id FROM cliente c WHERE c.cliente_id = r.cliente_id)) AS cliente,
			  (SELECT nombre FROM ubicacion WHERE ubicacion_id=r.destino_id) AS destino,r.destinatario AS destinatario,r.fecha_guia AS fecha 
			  FROM guia r WHERE r.oficina_id = $oficina_id AND r.estado_mensajeria_id = 1 AND r.planilla = 1 AND r.desbloqueada = 0) UNION 
	  		 (SELECT CONCAT('<input type=\"checkbox\" value=\"',r.guia_id,'\" />') AS link,
			  r.numero_guia AS guia,(SELECT CONCAT_WS(' ',razon_social,primer_nombre,segundo_nombre,primer_apellido,segundo_apellido)
			  FROM tercero WHERE tercero_id = (SELECT tercero_id FROM cliente c WHERE c.cliente_id = r.cliente_id)) AS cliente,
			  (SELECT nombre FROM ubicacion WHERE ubicacion_id=r.destino_id) AS destino,r.destinatario AS destinatario,r.fecha_guia AS fecha 
