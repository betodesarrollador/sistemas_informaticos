...
final class Imp_GuiaEncomiendaModel extends Db{
  ...
  public function getGuia($oficina_id,$empresa_id,$Conex){
	...
	$consul_forma_pago = $forma_pago_mensajeria_id == 'NULL' ? '' : "AND r.forma_pago_mensajeria_id = $forma_pago_mensajeria_id";
	
	//exit('test'.$consul_forma_pago);
	...
	( SELECT f.nombre FROM forma_pago_mensajeria f WHERE f.forma_pago_mensajeria_id = r.forma_pago_mensajeria_id) AS tipo_guia,
	...
	FROM guia_encomienda r WHERE r.guia_encomienda_id>0 $consul_forma_pago $guias $rangos  $ordensql $fechasql  ORDER BY r.guia_encomienda_id ASC";	//echo $select;
