...
final class EntregaOficina extends Controler{
  ...
  public function Main(){
	...
	require_once("EntregaOficinaLayoutClass.php"); 
	require_once("EntregaOficinaModelClass.php");
	
    $Layout   = new EntregaOficinaLayout($this -> getTitleTab(),$this -> getTitleForm());
    $Model    = new EntregaOficinaModel();
  ...
  protected function onclickValidateRow(){
    ...
    $Data = new ValidateRow($this -> getConex(),"entrega_oficina",$this ->Campos);
...
/////
  protected function asignoGuiaEntrega(){
	...
	if($Model -> entregaTieneGuias($entrega_oficina_id,$this -> getConex())){		
	  exit('true');
	}else{
	     exit('false');
	  }  
  ...
  protected function onclickSave(){    
    ...
    $result = $Model -> Save($obser_ent,$fecha_ent,$hora_ent,$usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$this -> Campos,$this -> getConex());
...
//BUSQUEDA
  protected function onclickFind(){
    ...
    $Data =  $Model -> selectEntrega($entrega_oficina_id,$this -> getConex());	
 ...
 //    $proveedor_id = $_REQUEST['proveedor_id'];
	...
	if($Data[0][proveedor_id]>0){
 	...
 	//Solo se permiten guias en estado  TRANSITO,DEVUELTO
	if($Data[0]['numero_guia']!=NULL && $Data[0]['numero_guia']!='NULL' && $Data[0]['numero_guia']!=''){
		if($Data[0][estado_mensajeria_id]==4 || $Data[0][estado_mensajeria_id]==7){
			$usuario_id                    = $this -> getUsuarioId();
			$oficina_id                    = $this -> getOficinaId();
			$empresa_id                    = $this -> getEmpresaId();
			$usuarioNombres                = $this -> getUsuarioNombres();
		...
		if($Data1[0][estado_mensajeria_id]==4 || $Data1[0][estado_mensajeria_id]==7){
			$usuario_id                    = $this -> getUsuarioId();
			$oficina_id                    = $this -> getOficinaId();
			$empresa_id                    = $this -> getEmpresaId();
			$usuarioNombres                = $this -> getUsuarioNombres();
			$usuario_numero_identificacion = $this -> getUsuarioIdentificacion();
			$Model -> SaveInter($obser_ent,$fecha_ent,$hora_ent,$usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$Data1[0][guia_interconexion_id],$proveedor,$proveedor_id,$this -> Campos,$this -> getConex());
			$this -> getArrayJSON($Data1);			
		}		
	}elseif($Data2[0]['numero_guia']!=NULL && $Data2[0]['numero_guia']!='NULL' && $Data2[0]['numero_guia']!=''){		
		if($Data2[0][estado_mensajeria_id]==4 || $Data2[0][estado_mensajeria_id]==7){
			$usuario_id                    = $this -> getUsuarioId();
			$oficina_id                    = $this -> getOficinaId();
			$empresa_id                    = $this -> getEmpresaId();
			$usuarioNombres                = $this -> getUsuarioNombres();
  ...
  protected function onclickCancellation(){  
     ...
     $Model                 = new EntregaOficinaModel(); 
	 $entrega_oficina_id	= $this -> requestDataForQuery('entrega_oficina_id','integer');
	 ...
	 $Model -> cancellation($entrega_oficina_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$this -> getConex());
  ...
  protected function setCampos(){  
	//FORMULARIO
	$this -> Campos[entrega_oficina_id] = array(
		name	=>'entrega_oficina_id',
		id		=>'entrega_oficina_id',
		type	=>'hidden',
		required=>'no',
		datatype=>array(
			type	=>'autoincrement',
	...
	$this -> Campos[estado] = array(
		...
		required=>'yes',
		disabled=>'yes',
		options =>array(array(value => 'O', text => 'EN OFICINA', selected => 'O'), array(value => 'A', text => 'ANULADO' , selected => 'O')),
		datatype=>array(
			type	=>'alpha_upper')
	);			
	...
	//BUSQUEDA
   	$this -> Campos[busqueda] = array(
		...
		tabindex=>'1',
		suggest=>array(
			name	=>'busca_entrega_oficina',
			setId	=>'entrega_oficina_id',
...
$EntregaOficina = new EntregaOficina();
