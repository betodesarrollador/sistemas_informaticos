...
final class Disposicion extends Controler{
  ...
  public function Main(){
	...
	require_once("DisposicionLayoutClass.php"); 
	require_once("DisposicionModelClass.php");
	
    $Layout   = new DisposicionLayout($this -> getTitleTab(),$this -> getTitleForm());
    $Model    = new DisposicionModel();
	...
	$Layout -> setCausalesDisposicion($Model -> getCausalesDisposicion($this -> getConex()));			
	
	//// GRID ////
	$Attributes = array(
	  id		=>'Disposicion',
	  title		=>'Listado de disposiciones Mensajeria',
	  sortname	=>'fecha_dev',
	  width		=>'auto',
	  height	=>'250'
	);
	...
	$Layout -> SetGridDisposicion($Attributes,$Titles,$Cols,$Model -> getQueryDisposicionGrid());	
	$Layout -> RenderMain();    
  }

  protected function onclickValidateRow(){
    require_once("../../../framework/clases/ValidateRowClass.php");
    $Data = new ValidateRow($this -> getConex(),"disposicion",$this ->Campos);
    $this -> getArrayJSON($Data  -> GetData());
  }

/////
  protected function asignoGuiaDisposicion(){
	...
	if($Model -> disposicionTieneGuias($disposicion_id,$this -> getConex())){		
	  exit('true');
	}else{
	     exit('false');
	  }  
  ...
  protected function onclickDelete(){
	...
	if($Model -> GetNumError() > 0){
	  exit('Ocurrio una inconsistencia');
	    ...
	    exit('Se elimino correctamente la Disposicion');
	}
  }
  
  protected function onclickPrint(){  
    require_once("Imp_DisposicionClass.php");
    $print = new Imp_Disposicion($this -> getConex());
    $print -> printOut();  
  }  
...
//BUSQUEDA
  protected function onclickFind(){
    ...
    $Data =  $Model -> selectDisposicion($disposicion_id,$this -> getConex());	
 ...
 protected function setLeerCodigobar() {
	...
	// $disposicion_id = $Model -> SaveDisposicion($guia, $Conex);
	...
	if($Data[0][proveedor_id]>0){
 	...
 	// $estado = $Model -> getEstadoReex($disposicion_id, $this -> getConex());	
	// $estadodis = $Model -> getEstadoDisReex($disposicion_id, $this -> getConex());	
	// if($estado!='A'){
 	// print $Data[0][numero_guia];
	if($Data[0]['numero_guia']!=NULL && $Data[0]['numero_guia']!='NULL' && $Data[0]['numero_guia']!=''){
		
		if($Data[0][estado_mensajeria_id]==11){
		...
		if($Data1[0][estado_mensajeria_id]==11){
			$usuario_id                    = $this -> getUsuarioId();
			$oficina_id                    = $this -> getOficinaId();
			$empresa_id                    = $this -> getEmpresaId();
			$usuarioNombres                = $this -> getUsuarioNombres();
		...
		if($Data2[0][estado_mensajeria_id]==11){
	...
	// }elseif($estado=='A'){
	// 	exit('No se Puede adicionar Guias a Esta Disposicion,<br>ya que esta Anulada');
	// }elseif($estadodis>0){
	// 	exit('No se Puede adicionar Guias a Esta Disposicion,<br>ya que una de las guias Relacionadas a este Disposicion ha cambiado de estado');
  ...
  protected function onclickCancellation(){  
     ...
     $Model                 = new DisposicionModel(); 
	 $disposicion_id         = $this -> requestDataForQuery('disposicion_id','integer');
	 ...
	 $Model -> cancellation($disposicion_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$this -> getConex());
  ...
  protected function setCampos(){  
	//FORMULARIO
	$this -> Campos[disposicion_id] = array(
		name	=>'disposicion_id',
		id		=>'disposicion_id',
		type	=>'hidden',
		required=>'no',
	...
	$this -> Campos[estado] = array(
		...
		required=>'yes',
		disabled=>'yes',
		options =>array(array(value => '12', text => 'DISPOSICION FINAL', selected => '12')),
		datatype=>array(
			type	=>'alpha_upper')
	);			

	$this -> Campos[causal_disposicion_id] = array(
		name	=>'causal_disposicion_id',
		id		=>'causal_disposicion_id',
		type	=>'select',
		required=>'yes',
		options	=>array(),
		datatype=>array(
	...
	$this -> Campos[causal_disposicion_id1] = array(
		name	=>'causal_disposicion_id1',
		id		=>'causal_disposicion_id1',
		type	=>'select',
		disabled=>'yes',
		options	=>array(),
		datatype=>array(
    ...
    disabled=>'yes',
    value   =>'Imprimir',
	id_prin =>'disposicion_id',
	displayoptions => array(
		      form        => 0,
		      beforeprint => 'beforePrint',
      title       => 'Impresion Disposicion',
      width       => '900',
      height      => '600'
    )
    );
	...
	*/
	...
	//BUSQUEDA
   	$this -> Campos[busqueda] = array(
		...
		tabindex=>'1',
		suggest=>array(
			name	=>'busca_disposicion',
			setId	=>'disposicion_id',
