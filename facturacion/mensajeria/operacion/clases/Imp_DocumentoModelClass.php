...
final class Imp_DocumentoModel extends Db{
  ...
  public function getManifiesto($manifiesto_id,$Conex){
	...
	if(is_numeric($manifiesto_id)){
        ...
        $select = "SELECT nombre, numero_identificacion
        FROM manifiesto WHERE manifiesto_id = $manifiesto_id ";		
	    $result = $this -> DbFetchAll($select,$Conex);
  ...
  public function getUrbano($despachos_urbanos_id,$Conex){
  
	if(is_numeric($despachos_urbanos_id)){
				
        $select = "SELECT nombre, numero_identificacion
        FROM  despachos_urbanos WHERE despachos_urbanos_id = $despachos_urbanos_id ";		
	    $result = $this -> DbFetchAll($select,$Conex);
