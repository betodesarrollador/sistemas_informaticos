...
function onclickGenerarAuxiliar(formulario) {
	...
	if (ValidaRequeridos(formulario)) {
		...
		var QueryString = "ACTIONCONTROLER=validaCentroCosto&" + FormSerialize(formulario);

		$.ajax({
			url: "LibrosAuxiliaresClass.php?rand=" + Math.random(),
			data: QueryString,
			beforeSend: function () {
				showDivLoading();
			},
			success: function (resp) {
				try {
					...
					var url = "LibrosAuxiliaresClass.php?ACTIONCONTROLER=onclickGenerarAuxiliar&" + FormSerialize(formulario);
					...
					if(resp !='null'){
						...
						for (var i = 0; i < data.length; i++) {
							...
							codigo_puc.push(data[i]['codigo_puc']);
						
						}
						
						alertJquery("Las siguientes cuentas filtradas NO poseen centro de costo : <br><br> <b>"+codigo_puc+"</b>","ATENCION")
					}
