...
final class LibrosAuxiliaresModel extends Db{
	...
	public function getCentroCosto($cuenta_desde_id,$cuenta_hasta_id,$Conex){
		
		$select = "SELECT CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc FROM imputacion_contable i,puc p WHERE 
		i.puc_id=p.puc_id AND p.nivel=5 AND i.centro_de_costo_id IS NULL AND i.puc_id BETWEEN $cuenta_desde_id AND $cuenta_hasta_id 
		GROUP BY i.puc_id";
	...
	public function selectAuxiliarCentrosTerceros($empresa_id,$opciones_centros,$centro_de_costo_id,$documentos,$cuenta_desde_id,$cuenta_hasta_id,$desde,$hasta,$agrupar,$consul_estado,$Conex){
		...
		if($opciones_centros == 'T'){
			...
			IF(i.tercero_id>0,
			(SELECT CONCAT_WS(' - ',SUBSTR(CONCAT_WS(' ',primer_apellido,segundo_apellido,'  ',primer_nombre,segundo_nombre, razon_social),1,25),numero_identificacion) FROM tercero WHERE tercero_id=i.tercero_id),
			...
			e.consecutivo,
			CONCAT(SUBSTR(i.descripcion,1,15),' ',SUBSTR(i.descripcion,16,25))as descripcion,
			...
			WHERE e.tipo_documento_id  IN ($documentos) AND e.fecha BETWEEN '$desde' AND '$hasta' AND e.encabezado_registro_id = i.encabezado_registro_id   $consul_estado 
			...
			ORDER BY $consul_ordensql ASC ";
				...
				CONCAT_WS(' - ',SUBSTR(CONCAT_WS(' ',primer_apellido,segundo_apellido,'  ',primer_nombre,segundo_nombre,
				razon_social),1,25),	t.numero_identificacion) AS tercero,p.puc_id,CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc,
				e.fecha,o.codigo_centro AS oficina,td.codigo AS documento,cc.codigo AS centro_costo,e.consecutivo,CONCAT(SUBSTR(i.descripcion,1,15),' ',SUBSTR(i.descripcion,16,25))as descripcion,
				...
				WHERE e.tipo_documento_id IN ($documentos) AND e.fecha BETWEEN '$desde' AND '$hasta'  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND 
				...
				codigo_puc,e.fecha,o.codigo_centro AS oficina,td.codigo AS documento,cc.codigo AS centro_costo,e.consecutivo,CONCAT(SUBSTR(i.descripcion,1,15),' ',SUBSTR(i.descripcion,16,25))as descripcion,IF(e.forma_pago_id='4',CONCAT_WS(' ',' # CH #   ',e.numero_soporte),'') AS cheque, i.base,i.debito,i.credito 
				FROM encabezado_de_registro e, imputacion_contable i, tipo_de_documento td, centro_de_costo cc,puc p, oficina o 
				WHERE e.tipo_documento_id   IN ($documentos) AND e.fecha BETWEEN '$desde' AND '$hasta'  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.centro_de_costo_id 
			...
			if($agrupar == 'defecto'){ 
				...
				($documentos) 
				AND e.fecha BETWEEN '$desde' AND '$hasta'  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.centro_de_costo_id IN 
				($centro_de_costo_id)
				...
				if(strlen(trim($tercerosMovimiento)) > 0){
					...
					if($opciones_centros == 'T'){	 
						...
						$select = "(SELECT t.tercero_id,CONCAT_WS(' - ',CONCAT_WS(' ',t.primer_apellido,t.segundo_apellido,t.primer_nombre,t.segundo_nombre,t.razon_social),t.numero_identificacion) AS tercero,p.puc_id,CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc,(CASE WHEN  
						...
						p, tercero t WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.tercero_id NOT IN 
						...
						p.naturaleza = 'D' THEN SUM(debito - credito) ELSE  SUM(credito - debito) END) AS saldo  FROM encabezado_de_registro e, imputacion_contable i,puc 
						p WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.tercero_id IS NULL AND i.puc_id = 
						...
						(SELECT t.tercero_id,CONCAT_WS(' - ',CONCAT_WS(' ',t.primer_apellido,t.segundo_apellido,t.primer_nombre,t.segundo_nombre,t.razon_social),t.numero_identificacion) AS tercero,p.puc_id,CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc,(CASE WHEN  
						p.naturaleza = 'D' THEN SUM(debito - credito) ELSE  SUM(credito - debito) END) AS saldo  FROM encabezado_de_registro e, imputacion_contable i,puc 
						p, tercero t WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.tercero_id NOT IN 
						...
						UNION ALL (SELECT t.tercero_id,CONCAT_WS(' - ',CONCAT_WS(' ',t.primer_apellido,t.segundo_apellido,t.primer_nombre,t.segundo_nombre,t.razon_social),t.numero_identificacion) AS tercero,p.puc_id,CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc,(CASE WHEN  
						p.naturaleza = 'D' THEN SUM(debito - credito) ELSE  SUM(credito - debito) END) AS saldo  FROM encabezado_de_registro e, imputacion_contable i,puc 
						p, tercero t WHERE e.tipo_documento_id IN ($documentos)   $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.puc_id = p.puc_id AND 
						...
						p.naturaleza = 'D' THEN SUM(debito - credito) ELSE  SUM(credito - debito) END) AS saldo FROM encabezado_de_registro e, imputacion_contable i,puc 
						p WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.puc_id = p.puc_id AND 
						i.centro_de_costo_id IN ($centro_de_costo_id) AND (SELECT codigo_puc FROM puc WHERE puc_id =  i.puc_id)  BETWEEN $cuenta_desde_id AND $cuenta_hasta_id AND i.puc_id NOT IN ($cuentasMovimiento) 
			...
			if($opciones_centros == 'T'){	
				...
				$select = "(SELECT t.tercero_id,CONCAT_WS(' - ',CONCAT_WS(' ',t.primer_apellido,t.segundo_apellido,t.primer_nombre,t.segundo_nombre,t.razon_social),t.numero_identificacion) AS tercero,p.puc_id,CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc,
				...
				imputacion_contable i,puc p, tercero t WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id 
				...
				(SELECT t.tercero_id,CONCAT_WS(' - ',CONCAT_WS(' ',t.primer_apellido,t.segundo_apellido,t.primer_nombre,t.segundo_nombre,t.razon_social),t.numero_identificacion) AS tercero,p.puc_id,CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc,
				(CASE WHEN p.naturaleza = 'D' THEN SUM(debito - credito) ELSE SUM(credito - debito) END) AS saldo , e.fecha AS fecha_reg
				FROM encabezado_de_registro e, 
				imputacion_contable i,puc p, tercero t WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id 
				...
				p WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.puc_id = p.puc_id 
				AND i.tercero_id 
		...
		WHERE e.tipo_documento_id IN 	($documentos)  $consul_estado  AND e.fecha BETWEEN '$desde' AND '$hasta' AND e.encabezado_registro_id = i.encabezado_registro_id AND 
		...
		if($opciones_centros == 'T'){	
			...
			WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id
			...
			$cuenta_hasta_id AND i.puc_id NOT IN ($cuentasMovimiento) AND e.fecha BETWEEN $fechaDesdeSaldo AND 
		...
		if(is_array($saldoTercerosSinMov)){
			...
			if(is_array($arrayResult)){
				...
				$arrayResult = array_values(array_merge($arrayResult,$saldoTercerosSinMov));	  
			if($cuenta_desde_id!=$cuenta_hasta_id && $opciones_centros != 'T'){ //echo 'sss';
			$arrayResult = $this -> sortResultByField($arrayResult,'fecha',SORT_ASC);  	  	  
			$arrayResult = $this -> sortResultByField($arrayResult,'codigo_puc',SORT_ASC);  	  	  
		}
	}else{
...
public function selectSaldoCentrosTercero($tercero_id,$empresa_id,$opciones_centros,$centro_de_costo_id,$documentos,$puc_id,$desde,$consul_estado,$Conex){  
	...
	encabezado_de_registro e, imputacion_contable i,puc p, tercero t WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id 
	...
	i.puc_id = $puc_id AND e.fecha BETWEEN $fechaDesdeSaldo AND DATE_SUB('$desde',INTERVAL 1 DAY) AND i.tercero_id = t.tercero_id 
...
public function selectSaldoCentrosPuc($empresa_id,$opciones_centros,$centro_de_costo_id,$documentos,$puc_id,$desde,$consul_estado,$Conex){  
	...
	$select = "SELECT (CASE WHEN p.naturaleza = 'D' THEN SUM(debito - credito) ELSE  SUM(credito - debito) END) AS saldo FROM 
	encabezado_de_registro e, imputacion_contable i,puc p WHERE e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id 
	= i.encabezado_registro_id AND i.puc_id = p.puc_id AND i.centro_de_costo_id IN ($centro_de_costo_id) AND 
	i.puc_id = $puc_id AND e.fecha BETWEEN $fechaDesdeSaldo AND DATE_SUB('$desde',INTERVAL 1 DAY) GROUP BY i.puc_id";    
...
public function selectAuxiliarCentrosTercero($empresa_id,$opciones_centros,$centro_de_costo_id,$tercero_id,
	$documentos,$cuenta_desde_id,$cuenta_hasta_id,$desde,$hasta,$agrupar,$consul_estado,$Conex){
	...
	$arrayResult         = array();
	if($opciones_centros == 'T'){
		// exit("Ingresa al if".$opciones_centros);
		$select = "SELECT 
		e.encabezado_registro_id,
		t.tercero_id,
		CONCAT_WS(' - ',SUBSTR(CONCAT_WS(' ',primer_apellido,segundo_apellido,'  ',primer_nombre,segundo_nombre,razon_social),1,25), t.numero_identificacion) AS tercero,
		p.puc_id,
		CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc,
		e.fecha,
		o.codigo_centro AS oficina,
		...
		WHERE e.tipo_documento_id IN ($documentos)   $consul_estado  AND e.fecha BETWEEN '$desde' AND '$hasta' AND e.encabezado_registro_id = i.encabezado_registro_id 
	 ...
	 //echo $select;	 	   	 	
		...
		$select = "SELECT e.encabezado_registro_id,t.tercero_id,CONCAT_WS(' - ',SUBSTR(CONCAT_WS(' ',primer_apellido,segundo_apellido,'  ',primer_nombre,segundo_nombre,razon_social),1,25),
		t.numero_identificacion) AS tercero,p.puc_id,CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc,e.fecha,o.codigo_centro AS 
		oficina,td.codigo AS documento,cc.codigo AS centro_costo,e.consecutivo,CONCAT(SUBSTR(i.descripcion,1,15),' ',SUBSTR(i.descripcion,16,25))as descripcion,IF(e.forma_pago_id='4',CONCAT_WS(' ',' # CH #  ',e.numero_soporte),'') AS cheque, i.base,i.debito,i.credito FROM 
		...
		IN ($documentos)   $consul_estado  AND e.fecha BETWEEN '$desde' AND '$hasta' AND e.encabezado_registro_id = i.encabezado_registro_id AND i.tercero_id = $tercero_id
		...
		i.tercero_id = t.tercero_id AND e.tipo_documento_id = td.tipo_documento_id 	AND e.oficina_id = o.oficina_id AND i.centro_de_costo_id = 
		cc.centro_de_costo_id  ORDER BY p.codigo_puc,e.fecha,tercero,oficina,documento,centro_costo,e.consecutivo ASC";	 //echo $select
		
		
	}
	$arrayResult = $this -> DbFetchAll($select,$Conex,true);    	
	$select = "SELECT DISTINCT i.puc_id FROM encabezado_de_registro e, imputacion_contable i WHERE e.tipo_documento_id IN ($documentos)  $consul_estado 	AND e.fecha BETWEEN '$desde' AND '$hasta' AND e.encabezado_registro_id = i.encabezado_registro_id AND i.tercero_id = $tercero_id 
	...
	if(strlen(trim($cuentasMovimiento)) > 0){
		...
		if($opciones_centros == 'T'){
			...
			e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.tercero_id = $tercero_id AND i.puc_id 
			...
			AND $cuenta_hasta_id AND e.fecha BETWEEN $fechaDesdeSaldo AND DATE_SUB('$hastaSaldoTercero',INTERVAL 1 DAY) AND i.tercero_id 
	...
	if($opciones_centros == 'T'){
		...
		t.numero_identificacion) AS tercero,p.puc_id,CONCAT_WS(' - ',p.codigo_puc,p.nombre) AS codigo_puc,(CASE WHEN p.naturaleza = 'D' 
		THEN SUM(debito - credito) ELSE  SUM(credito - debito) END) AS saldo, e.fecha AS fecha_reg FROM encabezado_de_registro e, imputacion_contable i,puc p, tercero t WHERE 
		e.tipo_documento_id IN ($documentos)  $consul_estado  AND e.encabezado_registro_id = i.encabezado_registro_id AND i.tercero_id = $tercero_id AND 
