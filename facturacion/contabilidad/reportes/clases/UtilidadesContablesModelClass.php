<?php
  ...
  public function periodoContableEstaHabilitado($empresa_id,$fecha,$Conex){
	...
	return $result[0]['estado'] == 1? true : false;		 
	  ...
	  public function getPeriodoContableId($fecha,$Conex){
		...
		if(count($result) > 0){
		  return $result[0]['periodo_contable_id'];
			 ...
			 exit("No existe un periodo contable creado para el a�o [ $anio ] <br> Comuniquese con el administrador del sistema !!");
		  }	
	  ...
	  public function getMesContableTrece($periodo_contable_id,$Conex){
	  
		 $select = "SELECT mes_contable_id FROM mes_contable WHERE periodo_contable_id = $periodo_contable_id AND mes = 13 AND mes_trece=1";
		 $result = $this  -> DbFetchAll($select,$Conex,true);
		 
		 if(count($result) > 0){
		   return $result[0]['mes_contable_id'];
		 }else{
			 exit("No existe el mes 13 para el periodo contable seleccionado <br> Comuniquese con el administrador del sistema !!");
		  }		 
   ...
   public function getCentrosCosto($empresa_id,$Conex){
    ...
    public function getAreas($empresa_id,$Conex){
   
     $select = "SELECT area_id AS value, nombre AS text FROM area ";
	 $result = $this -> DbFetchAll($select,$Conex,$ErrDb = false);    
    ...
    public function getDepartamentos($empresa_id,$Conex){
   
     $select = "SELECT departamento_id AS value, nombre AS text FROM departamento ";
	 $result = $this -> DbFetchAll($select,$Conex,$ErrDb = false);    
   ...
   public function getUnidades($empresa_id,$Conex){
   
     $select = "SELECT unidad_negocio_id AS value, nombre AS text FROM unidad_negocio ";
	 $result = $this -> DbFetchAll($select,$Conex,$ErrDb = false);    
   ...
   public function getSucursalesTxt($centros_costo,$opciones_centros,$Conex){
  ...
  public function getCondicionSaldosAuxiliares($empresa_id){
    ...
    $condicion = "(CASE WHEN SUBSTRING(p.codigo_puc,1,1) IN (4,5,6,7) THEN (SELECT IF(MAX(ec.encabezado_registro_id) > 0, (SELECT fecha FROM  
	 encabezado_de_registro WHERE estado='C' AND encabezado_registro_id = ec.encabezado_registro_id),(SELECT MIN(fecha) FROM encabezado_de_registro WHERE estado='C' AND empresa_id =
