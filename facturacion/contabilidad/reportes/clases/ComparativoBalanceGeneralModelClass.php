...
final class ComparativoBalanceGeneralModel extends Db{
  private $Permisos;
...
//LISTA MENU
   ...
   public function getSaldoCuentasBalance($hasta1,$desde1,$centro_de_costo_id,$opciones_centros,$empresa_id,$utilidadesContables,$Conex){
	...
	$result1 = $this->DbFetchAll($select1, $Conex, $ErrDb = false);
      ...
      $select = "
	     (
			...
			imputacion_contable i,puc p WHERE e.fecha BETWEEN '$fechaSaldoBalance'	 
			AND '$hasta1' AND  e.encabezado_registro_id = i.encabezado_registro_id AND i.puc_id IN ($cuentasMovimiento1Nivel1) 
	   ...
	   puc p WHERE e.fecha BETWEEN '$fechaSaldoBalance'	 
	   AND '$hasta1' AND  e.encabezado_registro_id = i.encabezado_registro_id AND i.puc_id IN ($cuentasMovimiento2Nivel1) 
	   AND i.puc_id = p.puc_id AND i.centro_de_costo_id IN ($centro_de_costo_id) GROUP BY cuenta) saldo1  LEFT JOIN 
	  ...
	  (SELECT SUBSTRING(p.codigo_puc,1,1) AS cuenta,(CASE WHEN  
	   p.naturaleza = 'D' THEN SUM(debito - credito) ELSE  SUM(credito - debito) END) AS saldo FROM encabezado_de_registro e, imputacion_contable i,
	   puc p WHERE e.fecha BETWEEN '$fechaSaldoBalance'	 
	   AND '$hasta1' AND  e.encabezado_registro_id = i.encabezado_registro_id AND i.puc_id IN ($cuentasMovimiento3Nivel1) 
	   AND i.puc_id = p.puc_id AND i.centro_de_costo_id IN ($centro_de_costo_id) GROUP BY cuenta) saldo1 LEFT JOIN 
   ...
   public function getSaldoUtilidadPerdidaNivel1($hasta1,$desde1,$centro_de_costo_id,$opciones_centros,$empresa_id,$utilidadesContables,$Conex){
	...
	$result1 = $this->DbFetchAll($select1, $Conex, $ErrDb = false);
	  ...
	  if($opciones_centros == 'T'){ 
		  ...
		  if(strlen(trim($cuentasMovimiento4Nivel1)) > 0){
			...
			imputacion_contable i,puc p WHERE e.fecha BETWEEN '$fechaSaldoBalance' AND '$hasta1' AND  
			...
			AS saldo FROM encabezado_de_registro e,imputacion_contable i,puc p WHERE e.fecha BETWEEN '$fechaSaldoBalance' AND '$hasta1' AND  
			e.encabezado_registro_id = i.encabezado_registro_id AND i.puc_id IN ($cuentasMovimiento4Nivel1) 
		  ...
		  if(strlen(trim($cuentasMovimiento5Nivel1)) > 0){
			...
			encabezado_de_registro e,imputacion_contable i,puc p WHERE e.fecha BETWEEN '$fechaSaldoBalance' AND '$hasta1' AND  
		  ...
		  if(strlen(trim($cuentasMovimiento6Nivel1)) > 0){
			...
			e,imputacion_contable i,puc p WHERE e.fecha BETWEEN '$fechaSaldoBalance' AND '$hasta1' AND  
   ...
   public function getSaldoCuenta($puc_id,$hasta1,$desde1,$centro_de_costo_id,$opciones_centros,$empresa_id,$Model,$utilidadesContables,$Conex){
  ...
  public function selectSaldoTercerosBalance($empresa_id,$puc_id,$opciones_centros,$centro_de_costo_id,$hasta1,$desde1,$utilidadesContables,$Conex){  
     ...
     if($opciones_centros == 'T'){
	  ...
	  SELECT t.tercero,(CASE WHEN p.naturaleza = 'D' THEN SUM(debito - credito) ELSE  SUM(credito - debito) END) AS saldo FROM
	  
      ((SELECT i.puc_id,CONCAT_WS(' - ',CONCAT_WS(' ',primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,razon_social),
	  ...
	  BETWEEN '$fechaSaldoBalance' AND '$hasta1' AND i.tercero_id = t.tercero_id) 
	  
	  UNION ALL 
	  
	  (SELECT i.puc_id,'SIN TERCERO' AS tercero,i.debito,i.credito FROM 
	  encabezado_de_registro e, imputacion_contable i, puc p WHERE e.empresa_id = $empresa_id AND e.encabezado_registro_id 
	  = i.encabezado_registro_id AND i.puc_id = $puc_id AND i.puc_id = p.puc_id AND i.centro_de_costo_id IN ($centro_de_costo_id) AND e.fecha 
	  BETWEEN '$fechaSaldoBalance' AND '$hasta1' AND i.tercero_id IS NULL)
	  ...
	  encabezado_de_registro e, imputacion_contable i, puc p WHERE e.empresa_id = $empresa_id AND e.encabezado_registro_id 
	  = i.encabezado_registro_id AND i.puc_id = $puc_id AND i.puc_id = p.puc_id AND i.centro_de_costo_id IS NULL AND e.fecha 
	  BETWEEN '$fechaSaldoBalance' AND '$hasta1' AND i.tercero_id IS NULL)) t, puc p WHERE t.puc_id = p.puc_id GROUP BY tercero ORDER BY tercero ASC
      ...
      $select = "SELECT t.tercero,(CASE WHEN p.naturaleza = 'D' THEN SUM(debito - credito) ELSE  SUM(credito - debito) END) AS saldo FROM
