  ...
  {if count($REPORTE) > 0}
	...
	<table border="0" width="80%" align="center">
	  <tr>
		...
		<td align="right" class="subtitulo" >Saldo Anterior : &nbsp;{$r.saldo}</td>
	  </tr>  
		...
		<td colspan="3">
			...
			{foreach name=registros from=$r.registros item=rg}
			 <tr bgcolor="{cycle values="#eeeeee,#d0d0d0"}">
               ...
               {if strlen(trim($rg.base)) > 0} 
			     <td class="borderTop borderRight" align="right">{$rg.base} </td>
			   {else}
			   ...
			   {if trim($rg.credito) > 0}      
			     <td class="borderTop borderRight" align="right">{$rg.credito} </td>
			   {else}
			     <td class="borderTop borderRight" align="right">{$rg.debito} </td>
			   {/if}		   					               
			...
			{/foreach}
			  ...
			  <tr class="subtitulo">
			   ...
			   <td class="borderTop borderRight borderBottom" align="right">{$r.total_base}</td>  
			    <td class="borderTop borderRight borderBottom" align="center"></td>             
			    {if trim($r.total_credito) > 0}      
			     <td class="borderTop borderRight borderBottom" align="right">{$r.total_credito}</td>
			   {else}
			     <td class="borderTop borderRight borderBottom" align="right">{$r.total_debito}</td>
			   {/if}			
			  </tr>  
            </table>
	  </td>
