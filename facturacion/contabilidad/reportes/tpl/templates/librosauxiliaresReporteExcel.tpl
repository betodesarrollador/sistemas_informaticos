<html>
  ...
  {if count($REPORTE) > 0}
	...
	{assign var="numdecimales" value=$MOSTRARDECIMALES}
	...
	<table border="0" width="80%" align="center">
	  <tr>
	    <td colspan="3" >&nbsp;{$r.tercero}</td>
	  </tr>
	  <tr>
	    <td width="72%" >&nbsp;{$r.codigo_puc}</td>
		<td align="right" class="subtitulo" >Saldo Anterior : &nbsp;{$r.saldo|number_format:$numdecimales:",":""}</td>
	  </tr>  
	  ...
	  <tr>
		<td colspan="3">
			<table border="0" width="100%" id="registros">
			  <tr align="center">
              <th class="borderLeft borderTop borderRight">Cuenta</th>
               ...
               <th class="borderTop borderRight" align="right">Base</th>
			...
			{foreach name=registros from=$r.registros item=rg}
			 <tr bgcolor="{cycle values="#eeeeee,#d0d0d0"}">
               <td class="borderLeft borderTop borderRight">{$rg.codigo_puc}</td>
             	<td class="borderTop borderRight">{$rg.tercero}</td>
			   ...
			   {if strlen(trim($rg.descripcion)) > 0} 
			     <td class="borderTop borderRight">{$rg.descripcion}&nbsp;<b>{$rg.cheque}</b> </td>
			   {else}
			     <td class="borderTop borderRight" align="center">-------</td>
			   {/if}
			   {if strlen(trim($rg.base)) > 0 && trim($rg.base)!=0}      
			   ...
			   {if strlen(trim($rg.debito)) > 0}      
			     <td class="borderTop borderRight" align="right">{$rg.debito|number_format:$numdecimales:",":""} </td>
			   {else}
			     <td class="borderTop borderRight" align="center">-------</td>
			   {/if}		   					               
			   {if strlen(trim($rg.credito)) > 0}     
			     <td class="borderTop borderRight" align="right">{$rg.credito|number_format:$numdecimales:",":""}</td>
			   {else}
			     <td class="borderTop borderRight" align="center">-------</td>
			   {/if}		   						               
			   {if strlen(trim($rg.saldo)) > 0}       
			     <td class="borderTop borderRight" align="right">{$rg.saldo|number_format:$numdecimales:",":""}  </td>
			   {else}
			...
			{/foreach}
			  <tr class="subtitulo">
			   <td class="borderLeft borderTop borderRight borderBottom" colspan="8" align="center">TOTAL</td>
			   <td class="borderTop borderRight borderBottom" align="right">&nbsp;{$r.total_base|number_format:$numdecimales:",":""}</td>
			   <td class="borderTop borderRight borderBottom" align="right">&nbsp;{$r.total_debito|number_format:$numdecimales:",":""}</td>
			   <td class="borderTop borderRight borderBottom" align="right">&nbsp;{$r.total_credito|number_format:$numdecimales:",":""}</td>
			   <td class="borderTop borderRight borderBottom" align="right">&nbsp;{$r.saldo_total|number_format:$numdecimales:",":""}</td>								
			  </tr>  
            </table>
	  </td>
	</tr>
