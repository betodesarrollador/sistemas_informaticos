<html>
  ...
  <body>
    ...
    <table width="80%" align="center" id="encabezado" border="0">
	  ...
	  <tr><td align="center" colspan="3" class="header">BALANCE GENERAL</td></tr>	 	 	   
	  <tr><td align="center" colspan="3" class="header">A CORTE : {$HASTA}</td></tr>	 	   
	...
	<tbody>
	 {section name=reporte loop=$arrayResult}
	  ...
	  <tr align="left" bgcolor="{cycle values="#eeeeee,#d0d0d0"}">
	    ...
	    {if $NIVEL > 4 }
		  <td align="right" >
		    {if $arrayResult[reporte].tipo eq 'AUX'}{$arrayResult[reporte].saldo|number_format:0:",":""}{else}&nbsp;{/if}
		   </td>								
		{/if}
	    {if $NIVEL > 3 }
		  <td align="right" >
		    {if $arrayResult[reporte].tipo eq 'SUBCUENTA'}{$arrayResult[reporte].saldo|number_format:0:",":""}{else}&nbsp;{/if}
		  </td>			
		{/if}
	    {if $NIVEL > 2 }
		  <td align="right" >
		    {if $arrayResult[reporte].tipo eq 'CUENTA'}{$arrayResult[reporte].saldo|number_format:0:",":""}{else}&nbsp;{/if}
		 </td>			
		{/if}
	    {if $NIVEL > 1 }
		 <td align="right" >
		  {if $arrayResult[reporte].tipo eq 'GRUPO'}{$arrayResult[reporte].saldo|number_format:0:",":""}{else}&nbsp;{/if}
		 </td>
		 {/if}
	    {if $NIVEL > 0 }
		 <td align="right" >
		   {if $arrayResult[reporte].tipo eq 'CLASE'}{$arrayResult[reporte].saldo|number_format:0:",":""}{else}&nbsp;{/if}
		 </td>		
		{/if} 
	  </tr>		  
