...
function size(){
    ...
    $("#scan_documento").blur(function(){
						 
  	var input = document.getElementById('scan_documento');
    ...
    if($('#actualizar')) $('#actualizar').attr("disabled","true");
		...
		if($('#encabezado_registro_id').val()>0){
			if($('#guardar'))    $('#guardar').attr("disabled","true");
		    if($('#actualizar')) $('#actualizar').attr("disabled","");
		}else{
      ...
      if($.trim(resp) == 'true' || parseInt(resp) > 0){
		...
		alertJquery("Se actualizo exitosamente",'Registros Contables');
		  ...
		  if (parseInt(resp) > 0){
			  ...
			  if (encabezado_registro_id > 0 && consecutivo > 0) {

				  alertJquery("Se actualizo exitosamente", 'Registros Contables');

			  }else{
				  console.log("ocurrio una inconsistencia.")
			  }

		  }else{

			  alertJquery(resp, ' Contables');
...
function MovimientosContablesOnReset(){ 
  ...
  $("#movimientosContables").attr("src","../../../framework/tpl/blank.html");						   
  $("#msj_anulado").css("display","none"); 
...
$(document).ready(function(){		
  ...
  $("#generar_excel").click(function(){
	 ...
	 if(ValidaRequeridos(formulario)){
	   ...
	   var QueryString = "MovimientosContablesClass.php?ACTIONCONTROLER=generateFileexcel&encabezado_registro_id="+encabezado_registro_id+"&rand="+Math.random();
