...
final class ParametrosLiquidacionModel extends Db{
  ...
  public function Save($Campos,$oficina_id,$Conex){	    
	$this -> DbInsertTable("parametros_liquidacion_comisiones",$Campos,$Conex,true,false);	
  }
	
  public function Update($Campos,$Conex){	
    $this -> DbUpdateTable("parametros_liquidacion_comisiones",$Campos,$Conex,true,false);
  }
	
  public function Delete($Campos,$Conex){  
  	$this -> DbDeleteTable("parametros_liquidacion_comisiones",$Campos,$Conex,true,false);	
  }	
  ...
  public function getTiposDocumentoContable($Conex){
   ...
   public function selectParametrosLiquidacion($parametros_liquidacion_id,$Conex){
    ...
    $select = "SELECT p.*, (SELECT CONCAT(codigo_puc,' - ',nombre) FROM puc WHERE puc_id = p.valor_comision_id) AS valor_comision,(SELECT CONCAT(codigo_puc,' - ',nombre) 
	FROM puc WHERE puc_id = p.sobre_flete_id) AS sobre_flete,
	(SELECT CONCAT(codigo_puc,' - ',nombre) FROM puc WHERE puc_id = p.reteica_id) AS reteica, (SELECT CONCAT(codigo_puc,' - ',nombre) FROM puc WHERE 
	puc_id = p.comisiones_por_pagar_id) AS comisiones_por_pagar FROM parametros_liquidacion_comisiones p WHERE parametros_liquidacion_id = $parametros_liquidacion_id";				
	$result = $this -> DbFetchAll($select,$Conex,$ErrDb = false);
