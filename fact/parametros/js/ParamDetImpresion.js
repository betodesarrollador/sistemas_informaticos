...
function linkImputaciones() {
    ...
    $("a[name=saveParamDetImpresion]").attr("href", "javascript:void(0)");

    $("a[name=saveParamDetImpresion]").focus(function() {
        var celda = this.parentNode;
        $(celda).addClass("focusSaveRow");
    });

    $("a[name=saveParamDetImpresion]").blur(function() {
        var celda = this.parentNode;
        $(celda).removeClass("focusSaveRow");
    });

    $("a[name=saveParamDetImpresion]").click(function() {
    ...
    if (validaRequeridosDetalle(obj, row)) {
        ...
        if (!codpuc_bien_servicio_factura_id.length > 0) {
            ...
            var QueryString = "ACTIONCONTROLER=onclickSave&tipo_bien_servicio_factura_id=" + tipo_bien_servicio_factura_id + "&codpuc_bien_servicio_factura_id=" +
                ...
                "&ret_tercero_bien_servicio_factura=" + ret_tercero_bien_servicio_factura + "&reteica_bien_servicio_factura=" + reteica_bien_servicio_factura + "&aplica_ingreso=" + aplica_ingreso;


            $.ajax({

                url: "ParamDetImpresionClass.php",
                data: QueryString,
                beforeSend: function() {
                    showDivLoading();
                },
                success: function(response) {
                    ...
                    if (!isNaN(response)) {
                        ...
                        var Table = document.getElementById('tableParamDetImpresion');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if ($.trim(response) == 'true') {
                        checkProcesar.attr("checked", "");
                        $(Fila).find("a[name=saveParamDetImpresion]").parent().addClass("focusSaveRow");
                        parent.document.getElementById("refresh_QUERYGRID_tiposervicio").click();
                    } else {
                        alertJquery(response);
                    }
...
function saveParamDetImpresion() {
...
function deleteParamDetImpresion() {
