...
function setDataFormWithResponse(){
	...
	var controlador = 'EjecutivoClass.php';
...
FindRow([{campos:"tercero_id",valores:$('#tercero_id').val()}],document.forms[0],'EjecutivoClass.php');
...
function EjecutivoOnSaveOnUpdateonDelete(formulario,resp){
	Reset(formulario);
   clearFind();
...
function EjecutivoOnReset(formulario){
...
$(document).ready(function(){
  ...
  $("#numero_identificacion").blur(function(){
	 ...
	 if(!tercero_id.length > 0){
       ...
       validaRegistro(this,params,"EjecutivoClass.php",null,function(resp){    
																  																  
         if(parseInt(resp) != 0 ){
           var params     = new Array({campos:"numero_identificacion",valores:numero_identificacion});
           var formulario = document.forms[0];
           var url        = 'EjecutivoClass.php';
  ...
  $("#guardar,#actualizar").click(function(){
	  ...
	  var formulario = document.getElementById('EjecutivosForm');
	  
	  if(ValidaRequeridos(formulario) && ValidaOtrosTercero(formulario)){ 
	    if(this.id == 'guardar'){
         Send(formulario,'onclickSave',null,EjecutivoOnSaveOnUpdateonDelete)
		}else{
            Send(formulario,'onclickUpdate',null,EjecutivoOnSaveOnUpdateonDelete)
		  }
	  }
...
function showTable(){
  ...
  var frame_grid =  document.getElementById('frame_grid');
    ...
    $.ajax({
      url        : "EjecutivoClass.php?rand="+Math.random(),
      data       : QueryString,
       async     : false,
      beforeSend : function(){
      showDivLoading();
