...
final class ClientesModel extends Db{
  ...
  public function selectDatosClientesTerceroId($tercero_id,$Conex){
     ...
     $select    = "SELECT 
						c.*,						
	 					t.*,
						(SELECT CONCAT_WS(' ',codigo,descripcion) FROM codigo_ciiu WHERE codigo_ciiu_id=t.codigo_ciiu_id) AS codigo_ciiu,
						c.estado AS estado,
						...
						(SELECT CONCAT_WS(' ',te.razon_social,te.primer_nombre,te.segundo_nombre,te.primer_apellido,te.segundo_apellido)  FROM tercero te, comercial co WHERE co.comercial_id=c.comercial_id AND te.tercero_id=co.tercero_id) AS comercial,
						(SELECT CONCAT_WS(' ',te.razon_social,te.primer_nombre,te.segundo_nombre,te.primer_apellido,te.segundo_apellido)  FROM tercero te, ejecutivo co WHERE co.ejecutivo_id=c.ejecutivo_id AND te.tercero_id=co.tercero_id) AS ejecutivo
				   	FROM tercero t 
  ...
  public function Save($Campos,$Conex){	
    ...
    $this -> Begin($Conex);
	  ...
	  $codigo_ciiu_id 	          = $this -> requestDataForQuery('codigo_ciiu_id','text');
	  ...
	  if(!$_REQUEST['tercero_id']>0 || $_REQUEST['tercero_id'] == 'NULL'){
		  $insert = "INSERT INTO tercero (tercero_id,ubicacion_id,tipo_persona_id,tipo_identificacion_id,numero_identificacion,digito_verificacion,
										  primer_apellido,segundo_apellido, primer_nombre,segundo_nombre,razon_social,sigla,telefono,movil,direccion,email,regimen_id,zona_postal,codigo_ciiu_id) 
						VALUES 
  ...
  public function getObligacionesTercero($tercero_id,$Conex){
   ...
   $Query = "SELECT 
			...
			(SELECT CONCAT_WS(' ',t.razon_social,t.primer_nombre,t.segundo_nombre,t.primer_apellido,t.segundo_apellido) AS ejecutivo  FROM ejecutivo co, tercero t WHERE co.ejecutivo_id=c.ejecutivo_id AND t.tercero_id=co.tercero_id) AS ejecutivo,						
