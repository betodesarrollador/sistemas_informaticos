...
final class MandoFinalModel extends Db{
...
//// GRID ////
  ...
  public function getQueryMandoTraficoGrid(){
	...
	$Query = "SELECT 
	CONCAT('<a href=\"javascript:popPup(\'/roa/seguimiento/seguimiento_monitoreo/clases/RegistroNovedadesClass.php?trafico_id=',t.trafico_id,'\',\'Trafico\',\'1200\',\'650\');\">',
	...
	IF (t.manifiesto_id>0,(SELECT s.placa FROM manifiesto s, oficina o WHERE s.manifiesto_id=t.manifiesto_id AND o.oficina_id=s.oficina_id), (SELECT ('VER')) ))),'</a>') AS placa,
	...
	IF(t.manifiesto_id>0,   (SELECT CONCAT_WS('','<div title=\"Observaciones: ',observaciones,'- Fecha Estimada Salida:',fecha_estimada_salida,'- Hora Estimada Salida:',hora_estimada_salida,'\">','MC No ',manifiesto,'</div>') AS mani FROM manifiesto WHERE manifiesto_id=t.manifiesto_id),
	 ...
	 (SELECT CONCAT_WS('','<div title=\"Observaciones: \">','REX No ',reexpedido,'</div>') AS reex FROM reexpedido WHERE reexpedido_id=t.reexpedido_id)))) AS trafico_id,
	...
	IF (t.manifiesto_id>0,(SELECT CONCAT('<div title=\"Movil ',tr.movil,'\">',s.nombre,'</div>') AS nombre FROM manifiesto s, conductor c, tercero tr, oficina o WHERE s.manifiesto_id=t.manifiesto_id AND o.oficina_id=s.oficina_id AND c.conductor_id=s.conductor_id AND tr.tercero_id=c.tercero_id),(SELECT CONCAT_WS(' ',primer_nombre,segundo_nombre,primer_apellido,segundo_apellido) AS nom FROM tercero WHERE tercero_id=(SELECT tercero_id FROM proveedor WHERE proveedor_id=(SELECT proveedor_id FROM reexpedido WHERE reexpedido_id=t.reexpedido_id)))))) AS conductor,
	...
	FROM trafico t WHERE  t.estado='F' OR t.estado='A' ORDER BY (SELECT nombre FROM ubicacion WHERE ubicacion_id=t.destino_id)   ASC";
