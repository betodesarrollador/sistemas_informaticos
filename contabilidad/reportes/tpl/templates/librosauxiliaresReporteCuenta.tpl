		...
		<td colspan="3">
			<table border="0" width="100%" id="registros">
			  <tr align="center">
			   ...
			   <th class="borderTop borderRight">Of</th>
			   <th class="borderTop borderRight">C.C.</th>		   
			   <th class="borderTop borderRight">Doc</th>
			   <th class="borderTop borderRight">Cons</th>
			...
			{foreach name=registros from=$r.registros item=rg}
			 <tr bgcolor="{cycle values="#eeeeee,#d0d0d0"}">
			   ...
			   {if strlen(trim($rg.descripcion)) > 0} 
			     <td class="borderTop borderRight celdaFija">{$rg.descripcion}&nbsp;<b>{$rg.cheque}</b> </td>
			   {else}
			     <td class="borderTop borderRight" align="center">-------</td>
			   {/if}		   
			   {if strlen(trim($rg.base)) > 0 && trim($rg.base)!=0}      
			     <td class="borderTop borderRight" align="right">{$rg.base|number_format:$numdecimales:",":"."} </td>
			   {else}
			     <td class="borderTop borderRight" align="center">-------</td>
			   {/if}		   					               
