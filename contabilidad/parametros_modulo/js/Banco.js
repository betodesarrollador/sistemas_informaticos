...
RequiredRemove();
FindRow([{campos:"banco_id",valores:$('#banco_id').val()}],document.forms[0],'BancoClass.php',null, 
  function(resp){
...
function showTable(){
  ...
  var frame_grid =  document.getElementById('frame_grid');
    ...
    $.ajax({
      url        : "BancoClass.php?rand="+Math.random(),
      data       : QueryString,
       async     : false,
      beforeSend : function(){
      showDivLoading();
...
function BancoOnSaveOnUpdateonDelete(formulario,resp){
   Reset(formulario);
   clearFind();
   $("#refresh_QUERYGRID_banco").click();
   ...
   alertJquery(resp,"Banco");
   
}
function BancoOnReset(formulario){
