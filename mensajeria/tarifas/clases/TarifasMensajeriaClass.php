<?php
	...
	error_reporting(1);
		...
		final class TarifasMensajeria extends Controler{
			...
			public function Main(){
				...
				require_once("TarifasMensajeriaLayoutClass.php");
				require_once("TarifasMensajeriaModelClass.php");
				$Layout   = new TarifasMensajeriaLayout($this -> getTitleTab(),$this -> getTitleForm());
				...
				//// GRID ////
				$Attributes = array(
					id			=>'TarifasMensajeria',
					title		=>'Listado de Tarifas Mensajeria',
					sortname	=>'periodo',
					width		=>'1000',
					height		=>'250'
				);
				...
				$Cols = array(
					array(name=>'tipo_servicio_mensajeria_id',	index=>'tipo_servicio_mensajeria_id',	sorttype=>'int',	width=>'150',	align=>'left'),
					array(name=>'tipo_envio_id',				index=>'tipo_envio_id',					sorttype=>'int',	width=>'150',	align=>'left'),
					array(name=>'vr_min_declarado',				index=>'vr_min_declarado',				sorttype=>'int',	width=>'140',	align=>'left'),
					array(name=>'vr_min_declarado_paq',			index=>'vr_min_declarado_paq',			sorttype=>'int',	width=>'140',	align=>'left'),
					array(name=>'vr_max_declarado',				index=>'vr_max_declarado',				sorttype=>'int',	width=>'140',	align=>'left'),
					array(name=>'vr_max_declarado_paq',			index=>'vr_max_declarado_paq',			sorttype=>'int',	width=>'140',	align=>'left'),
					array(name=>'vr_kg_inicial_min',			index=>'vr_kg_inicial_min',				sorttype=>'int',	width=>'140',	align=>'center'),
					array(name=>'vr_kg_adicional_min',			index=>'vr_kg_adicional_min',			sorttype=>'int',	width=>'140',	align=>'center'),
					array(name=>'periodo',						index=>'periodo',						sorttype=>'int',	width=>'100',	align=>'center'),
					array(name=>'porcentaje_seguro',			index=>'porcentaje_seguro',				sorttype=>'int',	width=>'100',	align=>'center')
				);
				...
				$Layout -> SetGridTarifasMensajeria($Attributes,$Titles,$Cols,$Model -> getQueryTarifasMensajeriaGrid());
				$Layout -> RenderMain();
			}

			protected function onclickValidateRow(){
				require_once("../../../framework/clases/ValidateRowClass.php");
				$Data = new ValidateRow($this -> getConex(),"tarifas_mensajeria",$this ->Campos);
				print $Data  -> GetData();
			}
		  ...
		  protected function onclickDuplicar(){
			...
			protected function OnClickFind(){
				...
				$Data  = $Model -> selectTarifasMensajeria($tarifas_mensajeria_id,$this -> getConex());
			...
			protected function setCampos(){
				...
				//campos formulario
				$this -> Campos[tarifas_mensajeria_id] = array(
					name	=>'tarifas_mensajeria_id',
					id		=>'tarifas_mensajeria_id',
					type	=>'hidden',
					datatype=>array(
						type	=>'autoincrement',
						length	=>'20'),
					transaction=>array(
						table	=>array('tarifas_mensajeria'),
				...
				$this -> Campos[busqueda] = array(
					...
					size	=>'85',
					suggest=>array(
						name	=>'tarifas_mensajeria',
						setId	=>'tarifas_mensajeria_id',
						onclick	=>'setDataFormWithResponse')
				);	
				$this -> SetVarsValidate($this -> Campos);
			}
		}
	new TarifasMensajeria();
?>