  ...
  <table id="tableSolicitudServicios" width="100%" >
    <thead>
      ...
      <tr>	
        ...
        <td>{$d.dir_dest_rm}</td>  	
        <td style="display:none">{$d.tel_dest_rm}            <input type="hidden" name="detalle_despacho_guia_id" id="detalle_despacho_guia_id" value="{$d.detalle_despacho_guia_id}"/></td>  	
        <td><input type="text" name="valor_flete" id="valor_flete" class="numeric" value="{$d.valor_flete}"/></td>   
        <td>{if $d.estado_mensajeria_id eq '4' && $d.estado_man eq 'P'}<input type="checkbox" onClick="deleteDetalleReexpedido(this,{$d.detalle_despacho_guia_id})" />{/if}</td>         
      </tr>          
      {/foreach}      
      </tbody>        
    </table>
