...
final class ResagoModel extends Db{
...
/////  
  public function Delete($Campos,$Conex){
	$this -> Begin($Conex);
 	  ...
 	  $this -> DbDeleteTable("resago_id",$Campos,$Conex,true,false);	
  	  $this -> DbDeleteTable("resago",$Campos,$Conex,true,false);	
	
	$this -> Commit($Conex);
  } 
  
  ////
  public function seleccionar_remesa($guia_id,$resago_id,$Conex){
  	$this -> Begin($Conex);
	$detalle_resago_id = $this -> DbgetMaxConsecutive("detalle_resago","detalle_resago_id",$Conex,true,1);	
	$insert = "INSERT INTO detalle_resago (detalle_resago_id, guia_id,resago_id)
				VALUES ($detalle_resago_id,$guia_id,$resago_id)";
	$this -> query($insert,$Conex,true);						  

	$update = "UPDATE guia SET estado_mensajeria_id=11 WHERE guia_id=$guia_id ";
	$this -> query($update,$Conex,true);
	$this -> Commit($Conex);
  }

  public function getEstadoReex($resago_id,$Conex){
    $select = "SELECT 
	estado
	FROM resago r 
	WHERE r.resago_id=$resago_id";

    $result = $this-> DbFetchAll($select,$Conex,true);
    return $result[0][estado];
 }

  public function getEstadoDisReex($resago_id,$Conex){
    $select = "SELECT 
	COUNT(*) AS movimientos
	FROM resago r, detalle_resago dg, guia g
	WHERE r.resago_id=$resago_id AND dg.resago_id=r.resago_id 
	AND g.guia_id=dg.guia_id AND g.estado_mensajeria_id != 11 ";
  ...
  public function resagoTieneGuias($resago_id,$Conex){  
    $select = "SELECT COUNT(*) AS num_guia FROM detalle_resago WHERE resago_id = $resago_id";
	$result = $this -> DbFetchAll($select,$Conex,true);	
	...
	public function cancellation($resago_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$Conex){ 

		$select = "SELECT d.guia_id FROM detalle_resago d 
		WHERE d.resago_id = $resago_id AND 
		(d.guia_id IN (SELECT dd.guia_id FROM detalle_devolucion dd, devolucion dv WHERE dv.estado='D' AND dd.devolucion_id=dv.devolucion_id )
		 OR d.guia_id IN (SELECT dd.guia_id FROM detalle_resago dd, resago dv WHERE dv.estado='E' AND dd.resago_id=dv.resago_id ))";
		$result = $this -> DbFetchAll($select,$Conex,true);	
		if($result[0][guia_id]>0){
			exit('No puede Anular la Entrega, <br>debido a que una o varias de las guias estan relacionadas<br> en una devoluci&oacute;n o resago Activa');
		}
		...
		$update = "UPDATE resago SET estado = 'A',causal_anulacion_id = $causal_anulacion_id,observacion_anulacion = $observacion_anulacion,
		fecha_anulacion = NOW(),usuario_anulo_id = $usuario_anulo_id WHERE resago_id = $resago_id";
		$this -> query($update,$Conex,true);

		$update = "UPDATE guia SET estado_mensajeria_id = 4 WHERE guia_id IN (SELECT guia_id FROM detalle_resago WHERE resago_id = $resago_id)";
		$this -> query($update,$Conex,true);
	...
	public function Update($Conex){ 
		...
		if($this -> GetNumError() > 0){
			...
			return array(resago_id => $resago_id);
		}
   ...
   public function Save($obser_ent,$fecha_ent,$hora_ent,$usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$guia_id,$proveedor,$proveedor_id,$Campos,$Conex){
	    ...
	    $resago_id = $this -> DbgetMaxConsecutive("resago","resago_id",$Conex,true,1);	
	    ...
	    $this -> assignValRequest('resago_id',$resago_id);
					
	    $this -> Begin($Conex);    
		  
		$this -> assignValRequest('estado','E');		    					
		$this -> DbInsertTable("resago",$Campos,$Conex,true,false);

		$detalle_resago_id = $this -> DbgetMaxConsecutive("detalle_resago","detalle_resago_id",$Conex,false,1);					
 ...
 public function SaveInter($obser_ent,$fecha_ent,$usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$guia_interconexion_id,$proveedor,$proveedor_id,$Campos,$Conex){
		...
		$insert = "INSERT INTO detalle_resago (detalle_resago_id, guia_interconexion_id,resago_id)
		VALUES ($detalle_resago_id,$guia_interconexion_id,$resago_id)";

		$this -> query($insert,$Conex,true);

		$update = "UPDATE guia_interconexion SET estado_mensajeria_id=11 WHERE guia_interconexion_id=$guia_interconexion_id ";
...
//BUSQUEDA
  public function selectEntrega($resago_id,$Conex){
    				
   $select = "SELECT r.*, 
   r.fecha_ent,r.proveedor_id,proveedor
   FROM resago r WHERE r.resago_id=$resago_id";
...
//// GRID ////
  public function getQueryEntregaGrid(){
	...
	CASE r.estado WHEN 'E' THEN 'EN RESAGO' WHEN 'A' THEN 'ANULADO' ELSE 'ANULADO' END AS estado,
	obser_ent
	FROM resago r";
	
//(SELECT re.numero_guia FROM guia re, detalle_resago dd WHERE dd.resago_id = r.resago_id AND re.guia_id = dd.guia_id) AS numero_guia	
