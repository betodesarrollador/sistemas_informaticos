...
final class GuiaEncomienda extends Controler{
	...
	public function Main(){
		...
		require_once("GuiaEncomiendaLayoutClass.php");

		require_once("GuiaEncomiendaModelClass.php");
		$Layout   = new GuiaEncomiendaLayout($this -> getTitleTab(),$this -> getTitleForm());
		$Model    = new GuiaEncomiendaModel();
		...
		$Layout ->  SetPrefijo   ($Model -> GetPrefijo(1,$this -> getConex()));
		...
		$Cols = array(
			array(name=>'guia_encomienda',						index=>'guia_encomienda',					sorttype=>'text',	width=>'80',	align=>'center' ),
	...
	protected function onclickValidateRow(){
		...
		$Data = new ValidateRow($this -> getConex(),"guia_encomienda",$this ->Campos);
		print $Data  -> GetData();
	}
	...
	protected function onclickSave(){    
		require_once("GuiaEncomiendaModelClass.php");
		$Model = new GuiaEncomiendaModel();  
		if($_REQUEST['peso']<=5000 && $_REQUEST['peso_volumen']<=5 ) exit('El peso o peso volumen debe ser mayor a 5.1 KG (5100 gramos).');
	...
	protected function getPrefijo(){
		...
		$forma_pago_mensajeria_id = $_REQUEST['forma_pago_mensajeria_id'];
		
		$prefijo = $Model -> getPrefijo($forma_pago_mensajeria_id,$this -> getConex());
	...
	protected function onclickUpdate(){
		...
		$clase_guia_encomienda  = $_REQUEST['clase_guia_encomienda'];
		if($_REQUEST['peso']<=5000 && $_REQUEST['peso_volumen']<=5 ) exit('El peso o peso volumen debe ser mayor a 5.1 KG (5100 gramos).');
		...
		if($clase_guia_encomienda == 'CP' && $Model -> esGuiaPrincipal($numero_guia,$this -> getConex())){
			print "<p align='center'>Esta guia_encomienda es principal para otros complemento ingresados<br>No se puede marcar como guia_encomienda de complemento!!!</p>";
		}else{
	...
	protected function onclickDelete(){
		...
		if($Model -> GetNumError() > 0){
			...
			exit('Se elimino correctamente la guia_encomienda');
		}
	}
	...
	protected function onclickCancellation(){    
		...
		$Model -> cancellation($guia_encomienda_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$this -> getConex());	
	...
	protected function onclickPrint(){      
		require_once("Imp_GuiaEncomiendaClass.php");	
		$print   = new Imp_GuiaEncomienda($this -> getConex());  	
	...
	protected function CalcularTarifa(){  
		...
		if($peso<=5 && $peso_volumen<=5){ exit('El peso o peso volumen debe ser mayor a 5.1 Kg'); }
		...
		if($origen_id==$destino_id){ 
			...
			$tiene_tipo_envio=$Model -> getTipoEnvio1($origen_id,$destino_id,$this -> getConex());
			if($tiene_tipo_envio>0){
				$tipo_envio_id=$tiene_tipo_envio;
			}
		...
		if($tabla_esc[0]['tabla_encomienda']=='tarifas_encomienda'){
			$resultado = $Model -> getCalcularTarifa($tipo_servicio_mensajeria_id,$tabla_esc[0]['tabla_encomienda'],$tipo_envio_id,date('Y'),$this -> getConex(),$this -> getOficinaId());	
			
			//$resultado_esp = $Model -> getCalcularTarifaEspecial($origen_id,$destino_id,$this -> getConex(),$this -> getOficinaId());	
			...
			$resultado = $Model -> getCalcularTarifaMasivo($tipo_servicio_mensajeria_id,$tabla_esc[0]['tabla_encomienda'],$tipo_envio_id,date('Y'),$this -> getConex(),$this -> getOficinaId());	
			//}
		...
		if($valor <$resultado[0]['vr_min_declarado_paq']){
	...
	protected function setCampos(){
		...
		////////////////////////////////
		////////// INFORMACION GENERAL
		$this -> Campos[guia_encomienda_id] = array(
		name	=>'guia_encomienda_id',
		...
		required => 'yes',		
		datatype => array(type=>'integer'),
		transaction => array(table =>array('guia_encomienda'), type =>array('column'))
		);	
	...
	/*	
		...
		$this -> Campos[tipo_guia] = array(
		name	=>'tipo_guia',
		id		=>'tipo_guia',
		type	=>'select',
		options	=> array(
		array(value => '1', text => 'CONTADO (EC)'),
		array(value => '3', text => 'CONTRA-ENTREGA(ECC)')),
		selected=> 1,
		...
		id    =>'foto_cumplido',
		type  =>'file',
		path	=>'/velotax/imagenes/mensajeria/guia_encomienda/',	
		...
		tabindex=>'1',
		suggest=>array(
		name	=>'busca_guia_encomienda',
		setId	=>'guia_encomienda_id',
		...
		title       => 'Impresion Guia Encomienda',
		width       => '700',
		height      => '600'
		)
		);
...
$guia_encomienda = new GuiaEncomienda();
