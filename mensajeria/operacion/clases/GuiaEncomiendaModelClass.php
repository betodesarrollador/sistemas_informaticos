...
final class GuiaEncomiendaModel extends Db{
  ...
  public function getPermiso($ActividadId,$Permiso,$Conex){
	...
	public function Save($Campos,$oficina_id,$empresa_id,$modifica,$usuario_id,$Conex){
		...
		$this -> assignValRequest('guia_encomienda_id',$guia_encomienda_id);
		...
		if(($remitente_id=='NULL' || $remitente_id=='') && $doc_remitente>0){
			 ...
			 $insert = "INSERT INTO remitente_destinatario (remitente_destinatario_id,numero_identificacion,tipo_identificacion_id,nombre,direccion,telefono,correo_remitente,ubicacion_id,tipo,estado,guia)
						VALUES ($remitente_destinatario_id,$doc_remitente,1,'$remitente','$direccion_remitente','$telefono_remitente',
						'$correo_remitente',$origen_id,'R','D',1)";
			 $this -> query($insert,$Conex,true);  
			 $this -> assignValRequest('remitente_id',$remitente_destinatario_id);

			
		}elseif($remitente_id>0){
			 $update = "UPDATE remitente_destinatario SET nombre='$remitente', direccion='$direccion_remitente',telefono='$telefono_remitente',
		...
		if(($destinatario_id=='NULL' || $destinatario_id=='') && $doc_destinatario>0){
			 ...
			 $insert = "INSERT INTO remitente_destinatario (remitente_destinatario_id,numero_identificacion,tipo_identificacion_id,nombre,direccion,telefono,correo_destinatario,ubicacion_id,tipo,estado,guia)
						VALUES ($remitente_destinatario_id,$doc_destinatario,1,'$destinatario','$direccion_destinatario','$telefono_destinatario',
						'$correo_destinatario',$destino_id,'D','D',1)";
			 $this -> query($insert,$Conex,true); 
			 $this -> assignValRequest('destinatario_id',$remitente_destinatario_id);
		}elseif($destinatario_id>0){
			 $update = "UPDATE remitente_destinatario SET nombre='$destinatario', direccion='$direccion_destinatario',telefono='$telefono_destinatario' , 
		...
		if($_REQUEST['numero_guia']>0){
			...
			$select1 = "SELECT  guia_encomienda_id FROM guia_encomienda WHERE  numero_guia=$numero_guia AND prefijo='$prefijo'  AND manual=0 AND crm=1";
			
			$result1 = $this -> DbFetchAll($select1,$Conex,true);
			
			if($result1[0]['guia_encomienda_id']>0){ exit('Ya existe una la Guia CRM '.$numero_guia.' para esta oficina'); }
			
		}else{
			  
			$select = "SELECT IF(MAX(numero_guia) IS NULL,1,MAX(numero_guia)+1) AS numero_guia FROM guia_encomienda WHERE  manual=0 AND crm=1 AND forma_pago_mensajeria_id = ".$_REQUEST['forma_pago_mensajeria_id'];
			
			$result = $this -> DbFetchAll($select,$Conex,true);
			$numero_guia = $result[0]['numero_guia'];
		}
		
		$selectcom = "SELECT p.*, t.tipo_documento_id FROM rango_guia_encomienda p, tipo_bien_servicio_factura t WHERE p.tipo='C' AND  p.estado='A' AND p.forma_pago_mensajeria_id = ".$_REQUEST['forma_pago_mensajeria_id']." AND t.tipo_bien_servicio_factura_id=p.tipo_bien_servicio_factura_id"; 
		...
		if(is_numeric($numero_guia)){
			...
			$select = "SELECT rango_guia_encomienda_fin,prefijo FROM rango_guia_encomienda 
			WHERE estado = 'A' AND tipo='C' AND forma_pago_mensajeria_id = ".$_REQUEST['forma_pago_mensajeria_id'];
			
			$result = $this -> DbFetchAll($select,$Conex,true);
			
			$rango_guia_encomienda_fin = $result[0]['rango_guia_encomienda_fin'];
			
			$prefijo = $result[0]['prefijo'];

			
			if($numero_guia > $rango_guia_encomienda_fin){
			...
			$rango_guia_encomienda_ini = $result[0]['rango_guia_encomienda_ini'];
			
			$prefijo = $result[0]['prefijo'];
			
			if(is_numeric($rango_guia_encomienda_ini)){
				$numero_guia = $rango_guia_encomienda_ini;
			}else{
		...
		//exit('prefijo : '.$prefijo.'<br>numero guia : '.$numero_guia);
		...
		$this -> assignValRequest('hora_guia',date('H:i:s'));
			...
			$this -> DbInsertTable("guia_encomienda",$Campos,$Conex,true,false);
			if($this -> GetNumError() > 0){
				...
				if($resultcom[0]['tipo_bien_servicio_factura_id']>0){
					...
					if($result_com[0][num_cuentas]>0){
						exit('No se permite parametrizar la guia_encomienda contado/contraentrega con un tipo de servicio configurado para Tercero');	
					}else{
						...
						foreach($result as $resultado){
							 ...
							 if($resultado[tercero_bien_servicio_factura]!=1 && $resultado[ret_tercero_bien_servicio_factura]!=1  && $ingresa==1 &&  $resultado[contra_bien_servicio_factura]!=1 ){
								 ...
								 $detalle_guia_encomienda_puc_id 	= $this -> DbgetMaxConsecutive("detalle_guia_encomienda_puc","detalle_guia_encomienda_puc_id",$Conex,true,1);
								 ...
								 $insert = "INSERT INTO detalle_guia_encomienda_puc (detalle_guia_encomienda_puc_id,guia_encomienda_id,puc_id,tercero_id,numero_identificacion,digito_verificacion,centro_de_costo_id,codigo_centro_costo,base_factura,porcentaje_factura,formula_factura,desc_factura,debito,credito,valor_liquida,contra)
											VALUES ($detalle_guia_encomienda_puc_id,$guia_encomienda_id,$resultado[puc_id],$tercero,IF($tercero>0,(SELECT numero_identificacion FROM tercero WHERE tercero_id=$tercero),NULL),IF($tercero>0,(SELECT digito_verificacion FROM tercero WHERE tercero_id=$tercero),NULL),$centro_costo,IF($centro_costo>0,(SELECT codigo  FROM centro_de_costo  WHERE centro_de_costo_id=$centro_costo),NULL),$base,$porcentaje,'$formula','$descripcion','$debito','$credito','$valor_liqui',$resultado[contra_bien_servicio_factura])";
								 $this -> query($insert,$Conex,true); 
						...
						$insert="INSERT INTO encabezado_de_registro (encabezado_registro_id,empresa_id,oficina_id,tipo_documento_id,valor,numero_soporte,tercero_id,periodo_contable_id,
											...
											$mes_contable_id,$consecutivo,'$fecha','$concepto',NULL,'C','$fecha_registro','$modifica',$usuario_id,'$numero_guia',$guia_encomienda_id)"; 
						$this -> query($insert,$Conex,true);  
						

						$select_item      = "SELECT detalle_guia_encomienda_puc_id FROM detalle_guia_encomienda_puc WHERE guia_encomienda_id=$guia_encomienda_id";
						...
						foreach($result_item as $result_items){
						  ...
						  FROM detalle_guia_encomienda_puc WHERE guia_encomienda_id=$guia_encomienda_id AND detalle_guia_encomienda_puc_id=$result_items[detalle_guia_encomienda_puc_id]"; 
						  $this -> query($insert_item,$Conex);
						}
						$update = "UPDATE guia_encomienda SET encabezado_registro_id=$encabezado_registro_id
  ...
  public function Update($oficina_id,$Campos,$Conex){
	$this -> Begin($Conex);	
	    $guia_encomienda_id = $_REQUEST['guia_encomienda_id'];
		$this -> assignValRequest('valor_manejo',0);
		$this -> DbUpdateTable("guia_encomienda",$Campos,$Conex,true,false);
		if($this -> GetNumError() > 0){
			return false;
		}else{		   
		    if($this -> GetNumError() > 0){
  ...
  public function Delete($Campos,$Conex){	  
	$this -> Begin($Conex);	
		$guia_encomienda_id  = $this -> requestDataForQuery('guia_encomienda_id','integer');		
//		$delete     = "DELETE FROM contacto_guia_encomienda WHERE guia_encomienda_id = $guia_encomienda_id"; 
//		$this -> query($delete,$Conex);		
		$delete     = "DELETE FROM detalle_guia_encomienda WHERE guia_encomienda_id = $guia_encomienda_id"; 
		$this -> query($delete,$Conex);		
		$this -> DbDeleteTable("guia_encomienda",$Campos,$Conex,true,false);		
	$this -> Commit($Conex);
  }
  
  public function cancellation($guia_encomienda_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$Conex){  

	$select = "SELECT estado_mensajeria_id FROM guia_encomienda WHERE guia_encomienda_id = $guia_encomienda_id";
	$result = $this -> DbFetchAll($select,$Conex,true);
	if($result[0]['estado_mensajeria_id']!=1) exit('No se puede Anular Guias que no esten en Estado Alistamniento');

   $this -> Begin($Conex);  
     $update = "UPDATE guia_encomienda SET estado_mensajeria_id = 8,causal_anulacion_id = $causal_anulacion_id,observacion_anulacion = $observacion_anulacion,
	 fecha_anulacion = NOW(),usuario_anulo_id = $usuario_anulo_id WHERE guia_encomienda_id = $guia_encomienda_id AND estado_mensajeria_id=1";	 
	 $this -> query($update,$Conex,true);	 
   $this -> Commit($Conex);  
  }    
...
//LISTA MENU
  ...
  public function GetPrefijo($forma_pago_mensajeria_id,$Conex){ 
  	
		$select = "SELECT rango_guia_encomienda_fin, prefijo FROM rango_guia_encomienda WHERE forma_pago_mensajeria_id =$forma_pago_mensajeria_id AND  estado = 'A'";
  ...
  public function getCalcularTarifaCliente($cliente_id,$tipo_servicio_mensajeria_id,$tabla,$tipo_envio_id,$anio,$Conex,$oficina_id){ 
     ...
     $select = "SELECT * FROM tarifas_encomienda_cliente
  ...
  public function getCalcularTarifa($tipo_servicio_mensajeria_id,$tabla,$tipo_envio_id,$anio,$Conex,$oficina_id){ 
     ...
     $select = "SELECT * FROM tarifas_encomienda
  ...
  public function getCalcularTarifaMasivo($tipo_servicio_mensajeria_id,$tabla,$tipo_envio_id,$anio,$Conex,$oficina_id){ 
   ...
   public function getCalcularTarifaEspecial($origen_id,$destino_id,$Conex,$oficina_id){ 
  	
     $select = "SELECT * FROM tarifas_especiales WHERE   origen_id=$origen_id AND destino_id = $destino_id";
...
//BUSQUEDA
  public function selectGuia($Conex){  
	...
	(SELECT nombre FROM ubicacion WHERE ubicacion_id=r.destino_id) AS destino FROM guia_encomienda r WHERE r.guia_encomienda_id = $guia_encomienda_id";				
	$result = $this -> DbFetchAll($select,$Conex,$ErrDb = false); 
	$dataGuia[0]['guia_encomienda'] = $result;	
	return $dataGuia;
  }
  ...
  public function selectGuiaComplemento($numero_guia,$Conex){	  
	...
	(SELECT tercero_id FROM remitente_destinatario WHERE remitente_destinatario_id = r.remitente_id AND remitente_destinatario_id = r.destinatario_id)) AS cliente,
	(SELECT nombre FROM ubicacion WHERE ubicacion_id=r.origen_id) AS origen,
	(SELECT nombre FROM ubicacion WHERE ubicacion_id=r.destino_id) AS destino FROM guia_encomienda r WHERE TRIM(r.numero_guia) = TRIM('$numero_guia')";				
	$result = $this -> DbFetchAll($select,$Conex,$ErrDb = false);
	$guia_encomienda_id                         = $result[0]['guia_encomienda_id'];	
	$result[0]['guia_encomienda_id']            = null;
...
//////////////////////////////////////////
  ...
  public function getClientes($oficina_id,$Conex){
   ...
   public function GetTipoMedida($Conex){
	...
	$result = $this -> DbFetchAll("SELECT tipo_servicio_mensajeria_id AS value,nombre AS text FROM tipo_servicio_mensajeria WHERE guia_encomienda=1 AND tipo_servicio_mensajeria_id IN(5,7) ORDER BY nombre ASC",$Conex,false);
    return $result;
  }  
/*
   ...
   public function getTabla($tipo_servicio_mensajeria_id,$Conex){	   
	  $select = "SELECT tabla_encomienda FROM  tipo_servicio_mensajeria   WHERE tipo_servicio_mensajeria_id=$tipo_servicio_mensajeria_id";//accaaa
	  $result = $this -> DbFetchAll($select,$Conex);	
	  return $result;
...
//// GRID ////
  public function getQueryGuiaOficinasGrid($oficina_id){
     $Query = "SELECT CONCAT_WS('',r.prefijo,r.numero_guia) AS guia_encomienda,
	 			r.orden_despacho,
				r.fecha_guia,
				(SELECT nombre FROM ubicacion WHERE ubicacion_id=r.origen_id) AS origen,
			  ...
			  FROM guia_encomienda r,  oficina o 
