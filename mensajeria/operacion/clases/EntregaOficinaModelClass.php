...
final class EntregaOficinaModel extends Db{
...
/////  
  public function Delete($Campos,$Conex){
	$this -> Begin($Conex);
 	  ...
 	  $this -> DbDeleteTable("entrega_oficina_id",$Campos,$Conex,true,false);	
  	  $this -> DbDeleteTable("entrega_oficina",$Campos,$Conex,true,false);	
	
	$this -> Commit($Conex);
  } 
  
  ////
  public function seleccionar_remesa($guia_id,$entrega_oficina_id,$Conex){
  	$this -> Begin($Conex);
	$detalle_entrega_oficina_id = $this -> DbgetMaxConsecutive("detalle_entrega_oficina","detalle_entrega_oficina_id",$Conex,true,1);	
	$insert = "INSERT INTO detalle_entrega_oficina (detalle_entrega_oficina_id, guia_id,entrega_oficina_id)
				VALUES ($detalle_entrega_oficina_id,$guia_id,$entrega_oficina_id)";
	$this -> query($insert,$Conex,true);						  

	$update = "UPDATE guia SET estado_mensajeria_id=10 WHERE guia_id=$guia_id ";
	$this -> query($update,$Conex,true);
	$this -> Commit($Conex);
  }

  public function getEstadoReex($entrega_oficina_id,$Conex){
    $select = "SELECT 
	estado
	FROM entrega_oficina r 
	WHERE r.entrega_oficina_id=$entrega_oficina_id";

    $result = $this-> DbFetchAll($select,$Conex,true);
    return $result[0][estado];
 }

  public function getEstadoDisReex($entrega_oficina_id,$Conex){
    $select = "SELECT 
	COUNT(*) AS movimientos
	FROM entrega_oficina r, detalle_entrega_oficina dg, guia g
	WHERE r.entrega_oficina_id=$entrega_oficina_id AND dg.entrega_oficina_id=r.entrega_oficina_id 
  ...
  public function entregaTieneGuias($entrega_oficina_id,$Conex){  
    $select = "SELECT COUNT(*) AS num_guia FROM detalle_entrega_oficina WHERE entrega_oficina_id = $entrega_oficina_id";
	$result = $this -> DbFetchAll($select,$Conex,true);	
	...
	public function cancellation($entrega_oficina_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$Conex){ 

		$select = "SELECT d.guia_id FROM detalle_entrega_oficina d 
		WHERE d.entrega_oficina_id = $entrega_oficina_id AND 
		(d.guia_id IN (SELECT dd.guia_id FROM detalle_devolucion dd, devolucion dv WHERE dv.estado='D' AND dd.devolucion_id=dv.devolucion_id )
		 OR d.guia_id IN (SELECT dd.guia_id FROM detalle_entrega_oficina dd, entrega_oficina dv WHERE dv.estado='E' AND dd.entrega_oficina_id=dv.entrega_oficina_id ))";
		$result = $this -> DbFetchAll($select,$Conex,true);	
		...
		$update = "UPDATE entrega_oficina SET estado = 'A',causal_anulacion_id = $causal_anulacion_id,observacion_anulacion = $observacion_anulacion,
		fecha_anulacion = NOW(),usuario_anulo_id = $usuario_anulo_id WHERE entrega_oficina_id = $entrega_oficina_id";
		$this -> query($update,$Conex,true);

		$update = "UPDATE guia SET estado_mensajeria_id = 4 WHERE guia_id IN (SELECT guia_id FROM detalle_entrega_oficina WHERE entrega_oficina_id = $entrega_oficina_id)";
		$this -> query($update,$Conex,true);
	...
	public function Update($Conex){ 
		...
		$entrega_oficina_id=$_REQUEST['entrega_oficina_id'];
		...
		$update="UPDATE
				...
				WHERE
					d.entrega_oficina_id=$entrega_oficina_id
				";
		...
		if($this -> GetNumError() > 0){
			...
			return array(entrega_oficina_id => $entrega_oficina_id);
		}
   ...
   public function Save($obser_ent,$fecha_ent,$hora_ent,$usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$guia_id,$proveedor,$proveedor_id,$Campos,$Conex){
	    ...
	    $entrega_oficina_id = $this -> DbgetMaxConsecutive("entrega_oficina","entrega_oficina_id",$Conex,true,1);	
	    ...
	    $this -> assignValRequest('entrega_oficina_id',$entrega_oficina_id);
					
	    $this -> Begin($Conex);    
		  
		$this -> assignValRequest('estado','E');		    					
		$this -> DbInsertTable("entrega_oficina",$Campos,$Conex,true,false);

		$detalle_entrega_oficina_id = $this -> DbgetMaxConsecutive("detalle_entrega_oficina","detalle_entrega_oficina_id",$Conex,false,1);					
	    ...
	    return $entrega_oficina_id;
	}

 public function SaveInter($obser_ent,$fecha_ent,$hora_ent,$usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$guia_interconexion_id,$proveedor,$proveedor_id,$Campos,$Conex){
		...
		$insert = "INSERT INTO detalle_entrega_oficina (detalle_entrega_oficina_id, guia_interconexion_id,entrega_oficina_id)
		VALUES ($detalle_entrega_oficina_id,$guia_interconexion_id,$entrega_oficina_id)";

		$this -> query($insert,$Conex,true);

		$update = "UPDATE guia_interconexion SET estado_mensajeria_id=10 WHERE guia_interconexion_id=$guia_interconexion_id ";
   ...
   public function SaveEncomienda($obser_ent,$fecha_ent,$hora_ent,$usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$guia_encomienda_id,$proveedor,$proveedor_id,$Campos,$Conex){
		...
		$insert = "INSERT INTO detalle_entrega_oficina (detalle_entrega_oficina_id, guia_encomienda_id,entrega_oficina_id)
		VALUES ($detalle_entrega_oficina_id,$guia_encomienda_id,$entrega_oficina_id)";

		$this -> query($insert,$Conex,true);

		$update = "UPDATE guia_encomienda SET estado_mensajeria_id=10 WHERE guia_encomienda_id=$guia_encomienda_id ";
...
//BUSQUEDA
  public function selectEntrega($entrega_oficina_id,$Conex){
    				
   $select = "SELECT r.*, 
   r.fecha_ent,r.proveedor_id,proveedor
   FROM entrega_oficina r WHERE r.entrega_oficina_id=$entrega_oficina_id";
...
//(SELECT re.numero_guia FROM guia re, detalle_entrega_oficina dd WHERE dd.entrega_oficina_id = r.entrega_oficina_id AND re.guia_id = dd.guia_id) AS numero_guia	
