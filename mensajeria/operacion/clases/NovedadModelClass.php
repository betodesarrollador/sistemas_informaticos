...
/////  
  public function Delete($Campos,$Conex){
	$this -> Begin($Conex);
 	  ...
 	  $this -> DbDeleteTable("novedad_id",$Campos,$Conex,true,false);	
  	  $this -> DbDeleteTable("novedad",$Campos,$Conex,true,false);	
	
	$this -> Commit($Conex);
  } 
  
  ////
  public function seleccionar_remesa($guia_id,$novedad_id,$causal_novedad_id,$Conex){
  	$this -> Begin($Conex);
	$detalle_novedad_id = $this -> DbgetMaxConsecutive("detalle_novedad","detalle_novedad_id",$Conex,true,1);	
	$insert = "INSERT INTO detalle_novedad (detalle_novedad_id, guia_id,novedad_id,causal_novedad_id)
				VALUES ($detalle_novedad_id,$guia_id,$novedad_id,$causal_novedad_id)";
  ...
  public function getEstadoReex($novedad_id,$Conex){
    $select = "SELECT 
	estado
	FROM novedad r 
  ...
  public function getEstadoDisReex($novedad_id,$Conex){
    $select = "SELECT 
	COUNT(*) AS movimientos
	FROM novedad r, detalle_novedad dg, guia g
	WHERE r.novedad_id=$novedad_id AND dg.novedad_id=r.novedad_id 
  ...
  public function novedadTieneGuias($novedad_id,$Conex){  
    $select = "SELECT COUNT(*) AS num_guia FROM detalle_novedad WHERE novedad_id = $novedad_id";
	$result = $this -> DbFetchAll($select,$Conex,true);	
  ...
  public function cancellation($novedad_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$Conex){ 
 
	$select = "SELECT d.guia_id FROM detalle_novedad d 
	WHERE d.novedad_id = $novedad_id AND 
	(d.guia_id IN (SELECT dd.guia_id FROM detalle_novedad dd, novedad dv WHERE dv.estado='D' AND dd.novedad_id=dv.novedad_id )
	...
	if($result[0][guia_id]>0){
		exit('No puede Anular la Novedad, <br>debido a que una o varias de las guias estan relacionadas<br> en una devoluci&oacute;n,entrega o reenvio Activo');
	}
	
	$select = "SELECT d.guia_interconexion_id FROM detalle_novedad d 
	WHERE d.novedad_id = $novedad_id AND 
	(d.guia_interconexion_id IN (SELECT dd.guia_interconexion_id FROM detalle_novedad dd, novedad dv WHERE dv.estado='D' AND dd.novedad_id=dv.novedad_id )
   ...
   $this -> Begin($Conex);
	...
	$update = "UPDATE guia SET estado_mensajeria_id = 11 WHERE guia_id IN (SELECT guia_id FROM detalle_novedad WHERE novedad_id = $novedad_id)";
	$this -> query($update,$Conex,true);
	
	$update = "UPDATE guia_interconexion SET estado_mensajeria_id = 11 WHERE guia_interconexion_id IN (SELECT guia_interconexion_id FROM detalle_novedad WHERE novedad_id = $novedad_id)";
	$this -> query($update,$Conex,true);
   ...
   $this -> Commit($Conex);
	...
	public function getCausalesNovedad($Conex){		
		$select = "SELECT causal_novedad_id AS value,causal AS text FROM causal_novedad ORDER BY causal";
		$result = $this -> DbFetchAll($select,$Conex);		
		return $result;			
    }	  
	...
	public function Update($Conex){ 
		...
		if($this -> GetNumError() > 0){
			...
			return array(novedad_id => $novedad_id);
		}
   ...
   public function Save($obser_dev,$fecha_dev,$usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$guia_id,$proveedor,$proveedor_id,$Campos,$Conex){
    ...
    $novedad_id = $this -> DbgetMaxConsecutive("novedad","novedad_id",$Conex,true,1);
	...
	$this -> DbInsertTable("novedad",$Campos,$Conex,true,false);
  
	$detalle_novedad_id = $this -> DbgetMaxConsecutive("detalle_novedad","detalle_novedad_id",$Conex,false,1);
	...
	if($result[0][numero_guia]>0){
	 ...
	 $insert = "INSERT INTO detalle_novedad (detalle_novedad_id, guia_interconexion_id,novedad_id,causal_novedad_id)
				VALUES ($detalle_novedad_id,$guia_interconexion_id,$novedad_id,$causal_novedad_id)";
   ...
   public function SaveEncomienda($obser_dev,$fecha_dev,$usuario_id,$oficina_id,$empresa_id,$usuarioNombres,$usuario_numero_identificacion,$guia_encomienda_id,$proveedor,$proveedor_id,$Campos,$Conex){
	...
	if($result[0][numero_guia]>0){
	 ...
	 $insert = "INSERT INTO detalle_novedad (detalle_novedad_id, guia_encomienda_id,novedad_id,causal_novedad_id)
				VALUES ($detalle_novedad_id,$guia_encomienda_id,$novedad_id,$causal_novedad_id)";
...
//BUSQUEDA
  public function selectNovedad($novedad_id,$Conex){
    				
   $select = "SELECT r.*, 
   r.fecha_dev,r.proveedor
   FROM novedad r WHERE r.novedad_id=$novedad_id";
...
//(SELECT re.numero_guia FROM guia re, detalle_novedad dd WHERE dd.novedad_id = r.novedad_id AND re.guia_id = dd.guia_id) AS numero_guia	
