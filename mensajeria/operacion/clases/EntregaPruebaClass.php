...
//BUSQUEDA
 ...
 protected function setLeerCodigobar() {
	...
	$oficina_id  = $this -> getOficinaId();
 	...
 	$Data2 = $Model -> setLeerCodigobar2($guia, $oficina_id,$this -> getConex());	
	...
	if(count($Data)>0){
		...
		$destino_id 	   = $Data2[0]['destino_id'];
		$ciudad_oficina    = $Data2[0]['ciudad_oficina'];
		$nombre_destino    = $Data2[0]['nombre_destino'];
		$ubicacion_oficina = $Data2[0]['ubicacion_oficina'];
	...
	if($Data[0][proveedor_id]>0){
	 ...
	 //Se leen guias en estado OFICINA,DEVUELTO
	...
	if($Data[0]['numero_guia']!=NULL && $Data[0]['numero_guia']!='NULL' && $Data[0]['numero_guia']!=''){
		...
		if($Data1[0][estado_mensajeria_id]==10 || $Data1[0][estado_mensajeria_id]==7){
			$usuario_id                    = $this -> getUsuarioId();
			$oficina_id                    = $this -> getOficinaId();
			$empresa_id                    = $this -> getEmpresaId();
			$usuarioNombres                = $this -> getUsuarioNombres();
		...
		if($Data2[0][estado_mensajeria_id]==10 || $Data2[0][estado_mensajeria_id]==7){
			$usuario_id                    = $this -> getUsuarioId();
			$oficina_id                    = $this -> getOficinaId();
			$empresa_id                    = $this -> getEmpresaId();
			$usuarioNombres                = $this -> getUsuarioNombres();
