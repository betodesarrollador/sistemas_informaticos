...
final class GuiaInterconModel extends Db{
  ...
  public function Anular($guia_interconexion_id,$Conex){
  	$sql ="UPDATE guia_interconexion SET estado_mensajeria_id = '8' WHERE guia_interconexion_id = $guia_interconexion_id";
  ...
  public function Save($Campos,$oficina_id,$Conex){  	
	...
	$this -> assignValRequest('guia_interconexion_id',$guia_interconexion_id);	
	$this -> assignValRequest('oficina_id',$oficina_id);	
	$this -> assignValRequest('estado_mensajeria_id',1);
	$select = "SELECT MAX(numero_guia) AS numero_guia FROM guia_interconexion WHERE oficina_id = $oficina_id  AND manual=0 AND crm=0";
	...
	$this -> Begin($Conex);	
		$this -> DbInsertTable("guia_interconexion",$Campos,$Conex,true,false);
  ...
  public function Update($Campos,$Conex){	
	$this -> Begin($Conex);	
	    $guia_interconexion_id = $_REQUEST['guia_interconexion_id'];
		$oficina_id  = $this -> requestDataForQuery('oficina_id','integer');
		//$this -> assignValRequest('oficina_id',$oficina_id);	
		$this -> DbUpdateTable("guia_interconexion",$Campos,$Conex,true,false);
		$peso   = $_REQUEST['peso'];	
  ...
  public function Delete($Campos,$Conex){	  
	$this -> Begin($Conex);	
		$guia_interconexion_id  = $this -> requestDataForQuery('guia_interconexion_id','integer');		
//		$delete     = "DELETE FROM contacto_guia WHERE guia_id = $guia_id"; 
//		$this -> query($delete,$Conex);		
		$delete     = "DELETE FROM detalle_guia_interconexion WHERE guia_interconexion_id = $guia_interconexion_id"; 
		$this -> query($delete,$Conex);		
		$this -> DbDeleteTable("guia_interconexion",$Campos,$Conex,true,false);		
	$this -> Commit($Conex);
  }
  
	public function cancellation($guia_interconexion_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$Conex){
		$this -> Begin($Conex);
			$update = "UPDATE guia_interconexion SET estado_mensajeria_id = '8',causal_anulacion_id = $causal_anulacion_id,observacion_anulacion = $observacion_anulacion,
			fecha_anulacion = NOW(),usuario_anulo_id = $usuario_anulo_id WHERE guia_interconexion_id = $guia_interconexion_id";
...
//LISTA MENU
  ...
  public function getGuiaNumero($Conex,$oficina_id){ 
     $select = "SELECT guia_interconexion_id AS value,numero_guia AS text,(SELECT MAX(guia_interconexion_id) FROM guia_interconexion WHERE oficina_id = $oficina_id) AS selected 
	 FROM guia_interconexion WHERE oficina_id = $oficina_id ORDER BY numero_guia ASC";
...
//BUSQUEDA
  public function selectGuiaIntercon($Conex){  
    $guia_interconexion_id  = $this -> requestDataForQuery('guia_interconexion_id','integer');
	$dataGuiaIntercon = array();    				
    $select = "SELECT r.*,
	...
	(SELECT nombre FROM ubicacion WHERE ubicacion_id=r.destino_id) AS destino FROM guia_interconexion r WHERE r.guia_interconexion_id = $guia_interconexion_id";				
	$result = $this -> DbFetchAll($select,$Conex,$ErrDb = false); 
	$dataGuiaIntercon[0]['guia_interconexion'] = $result;	
	return $dataGuiaIntercon;
  }
  
  public function selectGuiaInterconComplemento($numero_guia,$Conex){	  
	$dataGuiaIntercon = array();    				
	...
	(SELECT tercero_id FROM remitente_destinatario WHERE remitente_destinatario_id = r.remitente_id AND remitente_destinatario_id = r.destinatario_id)) AS cliente,
	(SELECT nombre FROM ubicacion WHERE ubicacion_id=r.origen_id) AS origen,
	(SELECT nombre FROM ubicacion WHERE ubicacion_id=r.destino_id) AS destino FROM guia_interconexion r WHERE TRIM(r.numero_guia) = TRIM('$numero_guia')";				
	$result = $this -> DbFetchAll($select,$Conex,$ErrDb = false);
	$guia_interconexion_id                         = $result[0]['guia_interconexion_id'];	
	$result[0]['guia_interconexion_id']            = null;
...
//////////////////////////////////////////
//// GRID ////
  public function getQueryGuiaInterconOficinasGrid($oficina_id){
     $Query = "SELECT r.numero_guia AS guia_interconexion,
	 			r.orden_despacho,
				r.fecha_guia,
				(SELECT nombre FROM ubicacion WHERE ubicacion_id=r.origen_id) AS origen,
			  ...
			  FROM guia_interconexion r,  oficina o 
