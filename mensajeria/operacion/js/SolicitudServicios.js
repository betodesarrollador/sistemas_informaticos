...
$(document).ready(function(){
	...
	/*
    $('#fileInput').uploadify({
        'uploader'  : '/velotax/framework/js/jquery-uploader/uploadify.swf',
        'script'    : 'SolicitudServiciosClass.php?ACTIONCONTROLER=uploadFile',
	'scriptData' : {'ACTIONCONTROLER': 'uploadFile'},
        'cancelImg' : '/velotax/framework/js/jquery-uploader/cancel.png',
        'auto'      : true,
        'folder'    : '/velotax/framework/js/jquery-uploader/uploads',
...
function validaRango(){
  if ($("#solicitud_id").val()!="") {
    var QueryString = "ACTIONCONTROLER=validaRango&solicitud_id="+$("#solicitud_id").val()+'&oficina_id='+$("#oficina_id").val();
    ...
    $.ajax({
      ...
      success : function(response){
        if (response!="true") {
          alertJquery(response,"Error en consecutivo");
          $("#solicitud_id").val("");
        }
      }
    }); 
