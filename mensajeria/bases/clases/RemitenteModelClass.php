...
final class RemitenteModel extends Db{
  ...
  public function getClientes($Conex){
   ...
   public function selectRemitente($Conex){
      ...
      elseif($numero_identificacion!='NULL'){
	      ...
	      tercero WHERE tercero_id = t.tercero_id) AS nombre,numero_identificacion,digito_verificacion,tipo_identificacion_id,direccion,telefono,ubicacion_id,
				(SELECT nombre FROM ubicacion WHERE ubicacion_id = t.ubicacion_id) AS ubicacion,'R' AS tipo FROM tercero t WHERE 
   ...
   public function destinatarioExists($valor,$cliente_id,$Conex){
     ...
     $select = "SELECT * FROM remitente_destinatario WHERE cliente_id = $cliente_id AND tipo = 'R' AND TRIM(nombre) LIKE TRIM('$valor')";
