...
function autocompleteEstadoContacto(){
	...
	$("input[name=estado_contacto]").autocomplete("/velotax/framework/clases/ListaInteligente.php?consulta=estado_contacto",{
		width: 260,
		selectFirst: false
	});
...
function saveContacto(obj){
	...
	if(validaRequeridosDetalle(obj,row)){
		...
		var estado_contacto_id = $(Fila).find("select[name=estado_contacto_id] option:selected").val();
...
function deleteContacto(obj){
	...
	if(contacto_id.length > 0 && $('#borrar',parent.document).length > 0 ){
