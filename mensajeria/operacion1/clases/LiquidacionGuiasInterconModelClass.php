...
final class LiquidacionGuiasInterconModel extends Db{
	...
	public function getConsecutivo($OficinaId,$Conex){
		...
		$select="SELECT MAX(consecutivo) AS consecutivo FROM liquidacion_guias_interconexion WHERE oficina_id=$OficinaId";
		$result = $this -> DbFetchAll($select,$Conex);
	...
	public function getConsecutivoReal($liquidacion_guias_interconexion_id,$Conex){

		$select="SELECT consecutivo FROM liquidacion_guias_interconexion WHERE liquidacion_guias_interconexion_id=$liquidacion_guias_interconexion_id";
		$result = $this -> DbFetchAll($select,$Conex);
		$consecutivo = $result[0][consecutivo];
		return $consecutivo;
	}
	...
	public function getGuiasLiqtodas($desde,$hasta,$cliente_id,$OficinaId,$oficina_id1,$Conex){
		...
		$select = "SELECT s.numero_guia,
			s.guia_interconexion_id,s.valor_total,s.valor_flete,s.valor_otros,s.valor_seguro,
	...
	public function getGuiasLiqUni($desde,$hasta,$cliente_id,$OficinaId,$oficina_id1,$guias_interconexion_id,$Conex){
		...
		$select = "SELECT s.numero_guia,
			...
			s.origen_id,s.destino_id,s.peso,s.peso_volumen,	s.tipo_servicio_mensajeria_id,s.tipo_envio_id
		FROM guia_interconexion s
		WHERE  s.cliente_id=$cliente_id AND s.fecha_guia BETWEEN '$desde' AND '$hasta' AND s.facturado = 0 AND s.guia_interconexion_id IN ($guias_interconexion_id) AND  s.estado_mensajeria_id IN (1,4,6,7) $oficina";
		$result = $this -> DbFetchAll($select,$Conex);	
		$result = $this -> DbFetchAll($select,$Conex,true);	
		return $result;
	}
  ...
  public function getActualizarValor($guia_interconexion_id,$tipo_envio_id, $total,$valor_decla,$valorinicial,$manejo,$valor_otros,$Conex){ 
     $update = "UPDATE guia_interconexion SET valor_flete=$valorinicial,valor_seguro=$valor_decla, valor_otros=$valor_otros,
	 			valor_total=$total, tipo_envio_id=$tipo_envio_id 
	 			WHERE guia_interconexion_id = $guia_interconexion_id";

	 $this -> query($update,$Conex,true);
	 
	 return $guia_interconexion_id;    
  }  


	public function Save($Campos,$usuario_id,$oficina_id,$consecutivo,$estado,$Conex){

		$liquidacion_guias_interconexion_id = $this -> DbgetMaxConsecutive('liquidacion_guias_interconexion','liquidacion_guias_interconexion_id',$Conex,true,1);
		$fecha_registro  = date('Y-m-d H:m');	
		$this -> assignValRequest('fecha_registro',$fecha_registro);			
		$this -> assignValRequest('liquidacion_guias_interconexion_id',$liquidacion_guias_interconexion_id);
		...
		$guias_interconexion_id  = $_REQUEST['guias_interconexion_id'];	
		$cliente_id  = $_REQUEST['cliente_id'];	
		$desde  = $_REQUEST['fecha_inicial'];				
		$hasta  = $_REQUEST['fecha_final'];							
		
		$this -> Begin($Conex);
		$this -> DbInsertTable("liquidacion_guias_interconexion",$Campos,$Conex,true,false);
		
		if($guias_interconexion_id=='todas'){

			$select = "SELECT 
				s.guia_interconexion_id,
				s.valor_total
			FROM guia_interconexion s
			WHERE  s.cliente_id=$cliente_id AND s.fecha_guia BETWEEN '$desde' AND '$hasta' AND s.facturado = 0 AND s.estado_mensajeria_id IN (1,4,6,7)";
			$result = $this -> DbFetchAll($select,$Conex);	
			$valor_total = 0;
			for($i=0;$i<count($result);$i++){
				$guia_interconexion_id=$result[$i]['guia_interconexion_id'];
				$valor_total = $valor_total+$result[$i]['valor_total'];
				$detalle_liq_guias_interconexion_id = $this -> DbgetMaxConsecutive('detalle_liq_guias_interconexion','detalle_liq_guias_interconexion_id',$Conex,true,1);
				$insert = "INSERT INTO detalle_liq_guias_interconexion (detalle_liq_guias_interconexion_id,liquidacion_guias_interconexion_id,guia_interconexion_id) 
						VALUES(	$detalle_liq_guias_interconexion_id,$liquidacion_guias_interconexion_id,$guia_interconexion_id)";
				$this -> query($insert,$Conex,true);
				
				$update = "UPDATE guia_interconexion SET facturado = 1 WHERE guia_interconexion_id=$guia_interconexion_id";
				$this -> query($update,$Conex,true);
			}
		}else{
			...
			$select = "SELECT 
				...
				s.valor_total
			FROM guia_interconexion s
			WHERE  s.cliente_id=$cliente_id AND s.fecha_guia BETWEEN '$desde' AND '$hasta' AND s.facturado = 0 AND s.guia_interconexion_id IN ($guias_interconexion_id) AND s.estado_mensajeria_id IN (1,4,6,7)";
			$result = $this -> DbFetchAll($select,$Conex);	
			$valor_total = 0;
			for($i=0;$i<count($result);$i++){
				$guia_interconexion_id=$result[$i]['guia_interconexion_id'];
		...
		$this -> Commit($Conex);
		$update = "UPDATE liquidacion_guias_interconexion SET valor = $valor_total WHERE liquidacion_guias_interconexion_id=$liquidacion_guias_interconexion_id";
		$this -> query($update,$Conex,true);//ojo ultimo adicionado verificar
		
		return $liquidacion_guias_interconexion_id;
	}


	public function cancellation($liquidacion_guias_interconexion_id,$observacion_anulacion,$fecha_anulacion,$usuario_anulo_id,$Conex){ 

		$this -> Begin($Conex);


			$anular = "UPDATE guia_interconexion SET facturado = 0 
			WHERE guia_interconexion_id IN (SELECT guia_interconexion_id FROM detalle_liq_guias_interconexion WHERE liquidacion_guias_interconexion_id=$liquidacion_guias_interconexion_id) 
			AND facturado = 1";
			$this -> query($anular,$Conex,true);//verificar que no este en otra


			$anula="UPDATE liquidacion_guias_interconexion SET estado = 'A', fecha_anulacion='$fecha_anulacion', observacion_anulacion='$observacion_anulacion', usuario_anulo_id = $usuario_anulo_id 
			WHERE liquidacion_guias_interconexion_id = $liquidacion_guias_interconexion_id";
	...
	public function selectLiquidacionGuiasIntercon($liquidacion_guias_interconexion_id,$Conex){
		...
		$select="SELECT
			...
			ls.oficina_id AS oficina_id1
			FROM liquidacion_guias_interconexion ls
			WHERE $liquidacion_guias_interconexion_id = ls.liquidacion_guias_interconexion_id
		";
		$result = $this -> DbFetchAll($select,$Conex);
		return $result;
	}
	...
	public function getValorPorFacturado($cliente_id,$desde,$hasta,$Conex){
		...
		$select="SELECT CONCAT(fecha_inicial,' ENTRE ',fecha_final) AS ultima
			FROM liquidacion_guias_interconexion 
	...
	public function getValorPorFacturadoInd($guias_interconexion_id,$cliente_id,$desde,$hasta,$Conex){

		$select="SELECT  SUM(valor_total) AS valor
			FROM guia_interconexion s
			WHERE  s.cliente_id=$cliente_id AND  s.fecha_guia BETWEEN '$desde' AND '$hasta' AND s.facturado = 0 AND s.guia_interconexion_id IN ($guias_interconexion_id) AND (s.estado_mensajeria_id=6 OR s.estado_mensajeria_id=4)";
		// echo $select;
		$result = $this -> DbFetchAll($select,$Conex,true);
		if($result[0][valor]>0){
			return $result[0][valor];			
