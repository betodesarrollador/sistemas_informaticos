...
final class SolicServToGuiaModel extends Db{
  ...
  public function SelectSolicitud($detalles_ss_id,$solicitud_id,$Conex){
	...
	(SELECT nombre FROM ubicacion WHERE ubicacion_id = d.origen_id) AS origen,d.origen_id,(SELECT nombre FROM ubicacion WHERE ubicacion_id = d.destino_id) AS destino,
	d.destino_id,d.remitente_id,d.remitente,d.doc_remitente,d.tipo_identificacion_remitente_id,d.direccion_remitente,d.telefono_remitente,d.destinatario_id,d.destinatario,d.doc_destinatario,d.tipo_identificacion_destinatario_id,d.direccion_destinatario,
	d.telefono_destinatario,(SELECT SUM(cantidad) FROM detalle_solicitud_servicio WHERE solicitud_id = s.solicitud_id) AS cantidad,
	(SELECT SUM(peso) FROM detalle_solicitud_servicio WHERE solicitud_id = s.solicitud_id) AS peso,(SELECT SUM(peso_volumen) FROM detalle_solicitud_servicio WHERE solicitud_id = s.solicitud_id) AS peso_volumen,
	(SELECT SUM(valor_unidad) FROM detalle_solicitud_servicio WHERE solicitud_id = s.solicitud_id) AS valor FROM solicitud_servicio s,detalle_solicitud_servicio d WHERE s.solicitud_id = $solicitud_id AND s.solicitud_id = d.solicitud_id AND detalle_ss_id = $detalle_ss_id";

    $result = $this -> DbFetchAll($select,$Conex,false);
    $arraySolicitud[0]['solicitud'] = $result;

    $select = "SELECT detalle_ss_id,referencia_producto,descripcion_producto AS descripcion_producto_detalle,cantidad AS cantidad_detalle,peso AS peso_detalle,
	valor_unidad AS valor_detalle,peso_volumen AS peso_volumen_detalle,orden_despacho AS guia_cliente FROM detalle_solicitud_servicio WHERE solicitud_id = $solicitud_id AND detalle_ss_id IN ($detalles_ss_id) ORDER BY detalle_ss_id ASC ";
...
//// GRID ////
  public function getQuerySolicServToGuiaGrid($oficina_id){	
