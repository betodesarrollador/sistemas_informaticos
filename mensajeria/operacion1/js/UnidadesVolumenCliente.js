...
function getDetalleCamposArchivoCliente(){
  ...
  $("#cliente_id").change(function(){
      ...
      document.getElementById('detalleCamposArchivoCliente').src = "DetalleUnidadesVolumenClass.php?cliente_id="+this.value;
...
function setDetalleCamposArchivoCliente(){
  ...
  if(form){
    ...
    var QueryString = "ACTIONCONTROLER=setDetalleUnidadesVolumen&cliente_id="+cliente_id+"&"+FormSerialize(form);
      
    $.ajax({
      url        : "UnidadesVolumenClass.php?rand="+Math.random(),
      data       : QueryString,
      beforeSend : function(){
        showDivLoading();
      },
