...
function checkRow(obj){
  ...
  if(numChecks > numMaxRemesas){
   alertJquery('Solo puede Seleccionar un Maximo de <span style="color:#red">'+numMaxRemesas+'</span> productos por remesa !!!','Validacion OrdenCargue Servicio Remesa')
   obj.checked = false;
  }
...
function setOrdenCargue(){
	...
	$.ajax({
	  ...
	  url     : "OrdenCargueToRemesaClass.php?rand="+Math.random(),
	  data    : QueryString,
