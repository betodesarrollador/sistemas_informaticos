...
function onclickUpdate(formulario){
	...
	// var entrega_parcial_id	= document.getElementById('entrega_parcial_id').value;
	// var obser_ent		= document.getElementById('obser_ent').value;

	// // alertJquery(entrega_parcial_id+"-"+obser_dev+"");
	// if(!isNaN(entrega_parcial_id)){
	// 	var QueryString = "ACTIONCONTROLER=onclickUpdate&entrega_parcial_id="+entrega_parcial_id+"&obser_ent="+obser_ent;
	// 	$.ajax({
	// 		url        : "BodegaPuenteClass.php?rand="+Math.random(),
	// 		data       : QueryString,
	// 		beforeSend : function(){
	// 			showDivLoading();
	// 		},
		...
		if (e.which == 13) {	
			...
			if(document.getElementById('codigo_barras1').value!=''){						
				...
				if(guia == "NULL" || guia == ""){
					...
					$("#codigo_barras1").val('');
					$.ajax({
					   url        : 'BodegaPuenteClass.php?random='+Math.random(),
					   data       : QueryString,
					   beforeSend : function(){
						   showDivLoading();
					   },
...
function setDataFormWithResponse(){
	...
	// var QueryString = "ACTIONCONTROLER=onclickFind&entrega_parcial_id="+$('#entrega_parcial_id').val();
 ...
 //       url        : 'BodegaPuenteClass.php?random='+Math.random(),
	...
	// 	   var data       = $.parseJSON(resp);
	// 	   var entrega = data[0]['entrega_parcial_id'];
...
function EntregaOnSave(formulario,entrega_parcial_id){		
   
	// if(parseInt(entrega_parcial_id)>0){
	...
	//   if($('#limpiar'))    $('#limpiar').attr("disabled","");
	//    $('#entrega_parcial_id').val(entrega_parcial_id);
...
function beforePrint(formulario,url,title,width,height){
	...
	// var entrega_parcial_id = parseInt($("#entrega_parcial_id").val());
...
function onclickCancellation(formulario){
 ...
 //       if(ValidaRequeridos(formulario)){
	...
	//      var QueryString = "ACTIONCONTROLER=onclickCancellation&"+FormSerialize(formulario)+"&entrega_parcial_id="+$("#entrega_parcial_id").val();
		
	//      $.ajax({
 //           url  : "BodegaPuenteClass.php",
	//        data : QueryString,
	//        beforeSend: function(){
	// 		   showDivLoading();
	//        },
 ...
 //    }else{
	...
	//  var entrega_parcial_id = $("#entrega_parcial_id").val();
