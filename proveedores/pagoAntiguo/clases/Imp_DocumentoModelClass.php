...
final class Imp_DocumentoModel extends Db{
  ...
  public function getEncabezado($encabezado_registro_id,$Conex){
	...
	//(SELECT nombre FROM forma_pago WHERE forma_pago_id=e.forma_pago_id) AS  formapago,	
		...
		(SELECT codigo FROM tipo_de_documento WHERE tipo_documento_id=e.tipo_documento_id) AS texto_tercero,		
		...
		(SELECT CONCAT_WS(' - ',(SELECT nombre FROM forma_pago WHERE forma_pago_id=e.forma_pago_id ),(SELECT nombre FROM puc WHERE puc_id=e.puc_id )))as formapago,
			
		e.*,
		t.*,
		of.nombre AS nom_oficina,
		YEAR(DATE(e.fecha)) AS anio,
		MONTH(DATE(e.fecha)) AS mes,
		DAY(DATE(e.fecha)) AS dia,
		of.codigo_centro AS prefijo_ofi,
