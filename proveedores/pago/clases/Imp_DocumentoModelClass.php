...
final class Imp_DocumentoModel extends Db
{
    ...
    public function getEncabezado($encabezado_registro_id, $Conex)
    {
		...
		(SELECT observaciones FROM abono_factura_proveedor WHERE encabezado_registro_id=e.encabezado_registro_id) AS  observaciones,
		e.*,
		t.*,
    ...
    public function getTotales($encabezado_registro_id, $Conex)
    {
        ...
        if (is_numeric($encabezado_registro_id)) {
         ...
         FROM  imputacion_contable    WHERE encabezado_registro_id = $encabezado_registro_id AND puc_id NOT IN (SELECT puc_id FROM cuenta_traslado ) ";
