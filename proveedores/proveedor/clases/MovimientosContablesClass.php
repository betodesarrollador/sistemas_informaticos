	...
	if($estado == 'C' && $encabezado_registro_id == 'NULL'){
	  exit('<b>'.$nombreModifica[0].'</b> no se permite Contabilizar este registro aun ');				
         ...
         if($mesContable && $periodoContable){
           ...
           $return = $Model -> Save($this -> getOficinaId(),$this -> Campos,$this -> getConex());
  ...
  protected function setCampos(){
	...
	*****************************************/  
	...
	$this -> Campos[tipo_documento_id] = array(
		...
		Boostrap=>'si',
		required=>'yes',
		options	=>array(array(value=>'4',text=>'123 - LEGALIZACION DE VIAJES'),array(value=>'15',text=>'121 - REEMBOLSO DE CAJA')),
		datatype=>array(
			type	=>'integer'),
		transaction=>array(
			table	=>array('encabezado_de_registro'),
    ...
    $nombre_documento = 'scan_documento_'.rand();		
 	...
 	$this -> Campos[scan_documento] = array(
		...
		type	=>'file',
		value	=>'',
		path	=>'/rotterdan/imagenes/contabilidad/movimientos/',
		size	=>'70',
//		required=>'yes',		
		datatype=>array(
			type	=>'file'),
	...
	$this -> Campos[actualizar] = array(
  	...
  	$this -> Campos[borrar] = array(
   	...
   	$this -> Campos[limpiar] = array(
		...
		type	=>'reset',
		value	=>'Limpiar',
		onclick => 'MovimientosContablesOnReset(this.form)'
	);	
