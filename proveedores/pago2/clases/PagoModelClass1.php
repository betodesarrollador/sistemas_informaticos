 ...
 /* public function Save($empresa_id,$Campos,$Conex){	
	 ...
	 foreach($causacion_id as $causaciones){
		 if($causaciones>0){
			...
			$select_contra 	= "SELECT c.puc_id,
									f.consecutivo_factura_proveedor,
									IF(d.cre_item_factura_proveedor>0,'C','D') AS natu_bien_servicio,
									...
									WHERE i.puc_id=c.puc_id AND i.empresa_id=$empresa_id AND i.estado='A' AND pc.anio=Year(CURDATE()) AND pc.empresa_id=$empresa_id 
									AND ipc.periodo_contable_id=pc.periodo_contable_id 	AND ipc.impuesto_id=i.impuesto_id AND io.impuesto_id=ipc.impuesto_id AND io.empresa_id=$empresa_id AND io.oficina_id=$oficina_id ) AS for_emp					  
									FROM factura_proveedor f,   puc c, detalle_factura_proveedor_puc  d  
									WHERE f.factura_proveedor_id=$causaciones AND d.factura_proveedor_id=f.factura_proveedor_id  AND d.contra_factura_proveedor=1  AND c.puc_id=d.puc_id "; 
			...
			$nombre_contra 	= 'CANC. FACT: '.$result_contra[0][consecutivo_factura_proveedor];
			$fact_comp.= $result_contra[0][consecutivo_factura_proveedor].',';
			...
			$insert_item = "INSERT INTO relacion_abono (relacion_abono_id,factura_proveedor_id,abono_factura_proveedor_id,rel_valor_abono,rel_valor_descu) 
						VALUES ($relacion_abono_id,$causaciones,$abono_factura_proveedor_id,'$valor_pago_ind',$rel_valor_descu)"; 
				...
				foreach($valor_des as $valor_descu){
					if($valor_descu!=''){
						...
						$select_des 	= "SELECT p.naturaleza, p.puc_id, 
											...
											WHERE p.parametros_descuento_factura_proveedor_id=$id_param "; 
			
						$result_des 	= $this -> DbFetchAll($select_des,$Conex);	


						$puc_descu=$result_des[0][puc_id];
						$puc_descu_nom='DESC. FACT: '.$result_contra[0][consecutivo_factura_proveedor];
	...
	$utilidadesContables = new UtilidadesContablesModel(); 
 	...
 	$this -> Begin($Conex);
	  ...
	  $puc_id        = $this -> requestData('cuenta_tipo_pago_id');
	...
	*/
     ...
     $insert_encabezado ="INSERT INTO encabezado_de_registro(encabezado_de_registro_id,empresa_id,oficina_id,tipo_documento_id,valor,tercero_id,periodo_contable_id,mes_contable_id,consecutivo,fecha,concepto,puc_id,estado,fecha_registro,modifica,usuario_id)
	 VALUES ($encabezado_de_registro_id,$empresa_id,$oficina_id,$tipo_documento_id,$valor,$tercero_id,$periodo_contable_id,$mes_contable_id,$consecutivo,'$fecha','$descripcion ',$puc_id,'A',$fecha_registro,'$modifica',$usuario_id)";
	   ...
	   if(is_numeric($puc_id)){
		 ...
		 if(($utilidadesContables -> requiereTercero($puc_id,$Conex) && !is_numeric($tercero_id)) || ($utilidadesContables -> requiereCentroCosto($puc_id,$Conex) && 
	        ...
	        $this -> Commit($Conex);		 
			 ...
			 $puc = "SELECT c.puc_id,c.natu_bien_servicio_factura_proveedor FROM codpuc_bien_servicio_factura_proveedor c WHERE c.tipo_bien_servicio_factura_proveedor_id= (SELECT tipo_bien_servicio_factura_proveedor_id FROM factura_proveedor WHERE factura_proveedor_id=$factura_proveedor_id) ";
			 $result = $this -> DbFetchAll($select,$Conex); 
			...
			foreach($result as $result_items){
				...
				$select   = "SELECT requiere_centro_costo,requiere_tercero,requiere_sucursal,area,departamento,unidadnegocio 
				 ...
				 $select = "SELECT * FROM impuesto_oficina WHERE empresa_id = (SELECT empresa_id FROM encabezado_de_registro WHERE 
				  ...
				  if(count($impuesto) > 0){
						...
						$select_impuesto = "SELECT porcentaje,formula FROM impuesto_periodo_contable WHERE periodo_contable_id=$periodo_contable_id AND impuesto_id= $impuesto[0]['impuesto_id'] ";
						$result_impuesto = $this -> DbFetchAll($select_impuesto,$Conex,true);	
						
						$select_descripcion ="SELECT descripcion FROM impuesto WHERE impuesto_id =  $impuesto[0]['impuesto_id']";
	         ...
	         $this -> Commit($Conex);		 
  	         return $encabezado_de_registro_id;			 
  ...
  public function getTotalDebitoCredito($abono_factura_proveedor_id,$Conex){
	  ...
	  $select = "SELECT SUM(deb_item_abono) AS debito,SUM(cre_item_abono) AS credito FROM item_abono   WHERE abono_factura_proveedor_id=$abono_factura_proveedor_id 	";
      $result = $this -> DbFetchAll($select,$Conex);

	  $selectv = "SELECT valor_abono_factura_proveedor FROM abono_factura_proveedor   WHERE abono_factura_proveedor_id=$abono_factura_proveedor_id";
      $resultv = $this -> DbFetchAll($selectv,$Conex);
	  $valor_total	= $result[0][debito] > $result[0][credito] ? $result[0][debito] : $result[0][credito];				  
	  
	  if($resultv[0][valor_abono_factura_proveedor]!=$valor_total){
		  $update = "UPDATE abono_factura_proveedor SET valor_abono_factura_proveedor='$valor_total' WHERE abono_factura_proveedor_id=$abono_factura_proveedor_id";
