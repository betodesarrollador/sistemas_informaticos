...
final class PagoModel extends Db{
 ...
 /* public function Save($empresa_id,$Campos,$Conex){	
	...
	$insert = "INSERT INTO abono_factura (abono_factura_id,cuenta_tipo_pago_id,num_cheque,tipo_documento_id,cliente_id,fecha,valor_abono_factura,valor_descu_factura,valor_neto_factura,concepto_abono_factura,causaciones_abono_factura,descuentos_items,valores_abono_factura,usuario_id,oficina_id,ingreso_abono_factura,estado_abono_factura,consecutivo_documento) 
				VALUES ($abono_factura_id,$cuenta_tipo_pago_id,$num_cheque,$tipo_documento_id,$cliente_id,$fecha,'$valor_tot_pago',$valor_descu_factura,$valor_neto_factura,$concepto_abono_factura,$causaciones_abono_factura,$descuentos_items,$valores_abono_factura,$usuario_id,$oficina_id,$ingreso_abono_factura,$estado_abono_factura,$consecutivo_documento)"; 
				////echo$insert;
	   ...
	   if(is_numeric($puc_id)){
		 ...
		 if(($utilidadesContables -> requiereTercero($puc_id,$Conex) && !is_numeric($tercero_id)) || ($utilidadesContables -> requiereCentroCosto($puc_id,$Conex) && 
	        ...
	        $this -> Commit($Conex);		 
			 ...
			 $factura_id= $this -> requestData('facturas');	  
			 
			 $puc = "SELECT c.puc_id,c.natu_bien_servicio_factura FROM codpuc_bien_servicio_factura c WHERE c.tipo_bien_servicio_factura_id= (SELECT tipo_bien_servicio_factura_id FROM factura WHERE factura_id=$factura_id) ";
			 $result = $this -> DbFetchAll($select,$Conex); 
	
			foreach($result as $result_items){
				
				$naturaleza = $result_items['natu_bien_servicio_factura'];
