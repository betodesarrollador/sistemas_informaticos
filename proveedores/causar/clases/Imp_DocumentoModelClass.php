...
final class Imp_DocumentoModel extends Db{
  ...
  public function getEncabezado($encabezado_registro_id,$Conex){
	...
	if(is_numeric($encabezado_registro_id)){
		...
		(SELECT pe.prefijo FROM parametros_equivalente pe, factura_proveedor fp WHERE fp.encabezado_registro_id=$encabezado_registro_id AND  pe.parametros_equivalente_id=fp.parametros_equivalente_id) AS prefijo,
		(SELECT CONCAT_WS(' ',pe.resolucion_dian,' Fecha:',pe.fecha_resolucion_dian, ' Rango Autorizado:',pe.rango_inicial,'-' , pe.rango_final, 'Prefijo:',pe.prefijo) FROM parametros_equivalente pe, factura_proveedor fp WHERE fp.encabezado_registro_id=$encabezado_registro_id AND  pe.parametros_equivalente_id=fp.parametros_equivalente_id) AS resolucion,	
