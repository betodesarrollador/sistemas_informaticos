...
function viewDoc(despachos_urbanos_id) {
	$('#despachos_urbanos_id').val(despachos_urbanos_id);
...
function frameAnticipo() {
	...
	if (parseInt(despachos_urbanos_id) > 0) {
		...
		$("#detalleFrameAnticipo").attr("src", "../clases/AnticiposClass.php?despachos_urbanos_id=" + despachos_urbanos_id + "&rand=" + Math.random());
		...
		alertJquery("Por favor seleccione un Despacho");
	}
		...
		try{
		   ...
		   if(propio == 1){			  
             ...
             $(tableAnticipos).find("input[name=anticipo]&&input[id=conductor_id]").each(function(){
				...
				$(Impuestoica).find("input[name=impuesto1]&&input[id=impuesto_id]").attr("disabled","");			
	...
	$.ajax({
	   ...
	   success    : function(resp){		   
		...
		try{
		   ...
		   var modalidad  = despacho[0]['modalidad'];		   
		   var estado     = despacho[0]['estado'];	
		   var observacion_anulacion     = despacho[0]['observacion_anulacion'];	
		   ...
		   if(propio == 1){			  
             ...
             $(tableAnticipos).find("input[name=anticipo]&&input[id=conductor_id]").each(function(){
				...
				if(data[0]['anticipos']){
				   ...
				   $(newRow).html($("#clonAnticipos").html()); 				   
	               $(newRow).find("input[name=anticipo]&&input[id=tenedor_anticipo]&&input[id=devolucion]").each(function(){this.value = $("#tenedor").val();});		
                   $(newRow).find("input[name=anticipo]&&input[id=tenedor_anticipo_id]&&input[id=devolucion]").each(function(){this.value = $("#tenedor_id").val();});			
		   ...
		   //fragmento johnatan
		   if(estado == 'M' || estado == 'L'){
				document.getElementById('foto_precinto_normal').disabled = false;
				document.getElementById('foto_amarres').disabled = false;
				document.getElementById('foto_precinto_normal_checked_file').disabled=false;
				document.getElementById('foto_amarres_checked_file').disabled=false;	
				document.getElementById('origen').disabled=false;			
				document.getElementById('origen_hidden').disabled=false;							
		   ...
		   if(estado == 'A'){ 
				document.getElementById('observacion_anul1').style.display="";
				document.getElementById('observacion_anul2').style.display="";		
				document.getElementById('observacion_anulacion1').value=observacion_anulacion;
		   }
...
function DespachosUrbanosOnSave(formulario,resp){		
	...
	try{
	  ...
	  resp = $.parseJSON(resp);
		...
		var tableAnticipos = document.getElementById('tableAnticipos');
		var Impuestoica = document.getElementById('ica');
	  ...
	  success    : function(resp){	
		try{
			...
			if(error1!='' && error1!='undefined' && error1!=undefined){
				alertJquery(error1,'Despacho Urbano');
			}else{
	  ...
	  $.ajax({
		...
		success    : function(resp){
            ...
            setFormWithJSON(document.forms[0],resp,true,function(){
			   ...
			   var despachos_urbanos_id = $("#despachos_urbanos_id").val();
			   var plac_man = data[0]['plac_man'];
               var plac_du = data[0]['plac_du'];
			   
			   if(despachos_urbanos_id > 0 || parseInt(plac_man)>0 || parseInt(plac_du)>0){
				...
				}else if(parseInt(plac_man)>0 || parseInt(plac_du)>0){
					
					 alertJquery("<div align='center'>Este vehiculo ya se encuentra con un manifiesto o despacho urbano activo!!!<br>Por favor revise.","Validacion Vehiculo");
				 ...
				 QueryString += anticipos;
				  ...
				  $(tableFletePactado).find("#ica").each(function(){			
						...
						$(this).find("input[name=impuesto1],select[name=impuesto1]").each(function(){   
							...
							if(this.id == 'impuestos_despachos_urbanos_id'){
							   impuestos1 += '&impuestos1['+contImpuestos1+'][impuestos_despachos_urbanos_id]='+this.value;							
							}else if(this.id == 'impuesto_id'){
	...
	if (parseInt(despachos_urbanos_id) > 0) {
		if (!formSubmitted) {
			...
			if (parseInt(valor) > 0) {
				...
				var QueryString = "ACTIONCONTROLER=getContabilizar&despachos_urbanos_id=" + despachos_urbanos_id + "&fecha_du=" + fecha_du;
				...
				$.ajax({
					...
					success: function (response) {
						...
						try {
							if ($.trim(response) == 'true') {
								alertJquery('Despacho Urbano Contabilizado', 'Contabilizacion Exitosa');
								$("#refresh_QUERYGRID_manifiestos").click();
								setDataFormWithResponse();
								formSubmitted = false;
							} else {
				...
				alertJquery('No se permite contabilizar Despachos en valor cero (0).');
			...
			alertJquery('Previamente se envio la solicitud, Por favor Espere', 'Contabilizacion');
		}
	} else {
		alertJquery('Debe Seleccionar primero un Despacho', 'Contabilizacion');
	}
}
   ...
   if(propio == 0){
	...
	$("#ica").each(function(){
	   ...
	   $(this).find("input[name=impuesto1],select[name=impuesto1]").each(function(){
         ...
         if(this.id == 'impuestos_despachos_urbanos_id'){
			 impuestos1 += '&impuestos1['+contImpuesto1+'][impuestos_despachos_urbanos_id]='+this.value;
		 }else if(this.id == 'impuesto_id'){
