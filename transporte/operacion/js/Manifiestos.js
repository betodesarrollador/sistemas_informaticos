...
$(document).ready(function(){
	...
	$("select[name=impuesto1]&&select[id=impuesto_id]").change(function () {
		var impuesto_id = this.value;
		var QueryString = "ACTIONCONTROLER=setDataIca&impuesto_id=" + impuesto_id;
		...
		$.ajax({
			...
			success: function (resp) {
				try {
					...
					var error1 = data[0]['error'];

					if (error1 != '' && error1 != 'undefined' && error1 != undefined) {
						alertJquery(error1, 'Manifiesto1');
					} else {
						var porcentaje = data[0]['porcentaje'];
						var base = data[0]['base'];
						var valor = data[0]['valor'];

						var base = $("input[name=impuesto1]&&input[id=base]").val();
						$("input[name=impuesto1]&&input[id=porcentaje]").val(porcentaje);
						calcularFlete(null, this.id, this.form);
					}
				} catch (e) {
					console.log('Manifiesto: '+e);
				}
			}
	...
	//alert("llego1");
		...
		$('#fecha_entrega_mcia_mc').change(function(){

	  	var fecha1 = $('#fecha_entrega_mcia_mc').val();
		var fecha3 = $('#fecha_estimada_salida').val();
		
		//alert("llego");
	  	if ((Date.parse(fecha1)<Date.parse(fecha3))) { 
			//alert("llego1");
	  		alertJquery('La fecha estimada de entrega no puede ser menor a la fecha estimada de salida!.'); 
	  		$('#fecha_entrega_mcia_mc').val(''); 
	  	}
	});

		$('#fecha_estimada_salida').change(function(){
		//alert("llego");
		
		var fecha1 = $('#fecha_estimada_salida').val();
		var fecha2 = $('#fecha_mc').val();
		var diff = Date.parse(fecha1) - Date.parse(fecha2);
		var total= (diff/(1000*60*60*24));
		
	  	if (total > 15) { 
			//alert("llego1");
	  		alertJquery('La fecha estimada de salida no puede  mayor a 15 dias con respecto a la fecha actual'); 
	  		$('#fecha_estimada_salida').val(''); 
	  	}else if(total < -15){
			alertJquery('La fecha estimada de salida no puede  menor a 15 dias con respecto a la fecha actual'); 
			};
	});
		...
		var fecha1 = $('#fecha_estimada_salida').val();
		var fecha2 = $('#fecha_entrega_mcia_mc').val();

		
	  	if (fecha1==fecha2) { 
		
			var h1Entrega = $('#hora_entrega').val();
			var h1Salida = $('#hora_estimada_salida').val();
			
			var horaEntrega =  h1Entrega.split(":");
			var horaSalida =   h1Salida.split(":"); 
			
			 var hh1 = parseInt(horaEntrega[0],10);
   			 var mm1 = parseInt(horaEntrega[1],10);
			 
			 var hh2 = parseInt(horaSalida[0],10);
   			 var mm2 = parseInt(horaSalida[1],10);
			 
			if (hh1 < hh2){alertJquery('La hora estimada de entrega  no puede ser menor a la hora estimada de salida !!.');
	  		$('#hora_entrega').val('');}
			if ((hh1==hh2) && (mm1==mm2)){
				alertJquery('La hora estimada  de entrega  no puede ser igual a la hora estimada de salida !!.');
	  		$('#hora_entrega').val('');
			}
   ...
   $("input[name=manifiesto]").blur(function () {
		var manifiesto = $("#manifiesto").val();	
		var QueryString = "ACTIONCONTROLER=comprobarManifiesto&manifiesto="+manifiesto;
		...
		$.ajax({
			...
			success: function (resp) {
				try {
					...
					var data = $.parseJSON(resp);
					if (data[0]['manifiesto'] == manifiesto && manifiesto != '') {
						alertJquery('Este Manifiesto ya se encuentra creado!');
						$("#manifiesto").val('');	
					}

				} catch (e) {
					alertJquery(e, 'Manifiesto');
				}
			}
...
function size(){
    ...
    $("#foto_precinto_normal,#foto_amarres").blur(function(){
								 
  	var input = document.getElementById('foto_precinto_normal');
	var input1 = document.getElementById('foto_amarres');
	
    var file = document.getElementById('foto_precinto_normal').value=='' ? 0 : input.files[0].size;
	var file1 =  document.getElementById('foto_amarres').value=='' ? 0 : input1.files[0].size;
	
   var total = parseInt(file+file1); 

	if(file>4000000 || file1>4000000 || total>4000000 ){
	...
	$.ajax({
	   ...
	   success    : function(resp){		   
		...
		try{
		   ...
		   var reportado_ministerio2 = manifiesto[0]['reportado_ministerio2'];
		   ...
		   if(propio == 1){			  
             ...
             $(tableAnticipos).find("input[name=anticipo]&&input[id=conductor_id]").each(function(){
			   ...
			   if (data[0]['impuestos1']) {

				   var impuestos1 = data[0]['impuestos1'];


				   var Table = document.getElementById('tableFletePactado');

				   for (var i = 0; i < impuestos1.length; i++) {


					   for (var llave in impuestos1[i]) {

						   $(Table).find("input[name=impuesto1],select[name=impuesto1]").each(function () {

							   if (this.id == llave) {
								   this.value = impuestos1[i][llave];
							   }
				...
				if(data[0]['anticipos']){
				   ...
				   for(var i = 0; i < anticipos.length; i++){
					   ...
					   if(estado == 'P'){
                       $(newRow).find("input[name=addAnticipoTercero]").replaceWith("<input type='button' name='removeAnticipoTercero' class='btn btn-success' id='removeAnticipoTercero' value=' - ' />");	
					   }else{
                              $(newRow).find("input[name=addAnticipoTercero]").replaceWith("<input type='button' name='removeAnticipoTercero' class='btn btn-success' id='removeAnticipoTercero' value=' - ' disabled  />");							   
						   }
		   ...
		   if ((estado=='P' || estado =='M' || estado =='L') && reportado_ministerio2 == 0 ){ // || estado =='L'
			   enabledInputsFormManifiesto(forma);
			   document.getElementById('estado').disabled = true
			   document.getElementById('fecha_mc').disabled = true
			   
		   }
		   if((estado != 'M' && estado!='P' &&  estado !='L') || reportado_ministerio2 == 1){ //&& estado =='L'
		     disabledInputsFormManifiesto(forma);			  
		   }
		   ...
		   if(estado == 'P' || estado == 'M' || estado =='L'){ // || estado =='L'
...
function ManifiestosOnSave(formulario,resp){	
	...
	try{
	  ...
	  resp = $.parseJSON(resp);
		...
		if ($('select[name=impuesto1]&&select[id=impuesto_id]')) $('select[name=impuesto1]&&select[id=impuesto_id]').attr("disabled", "");
...
function beforePrint(formulario) {
	...
	if (ValidaRequeridos(formulario)) {
		...
		var QueryString = "OrganizaImprimirDocumentosClass.php?ACTIONCONTROLER=manifiestosCarga&manifiesto_id=" + manifiesto_id;
...
function separaNombre(conductor_id){
	...
	$.ajax({
	  ...
	  success    : function(resp){		  
			...
			var eps_vencio           = data[0]['eps_vencio'];
			var vencimiento_eps      = data[0]['vencimiento_eps'];
			var arp_vencio           = data[0]['arp_vencio'];
			var vencimiento_arp      = data[0]['vencimiento_arp'];
			reportado_ministerio     =1; //mientras despues borrar		
            setFormWithJSON(document.forms[0],resp,true,function(){
			  ...
			  if(licencia_vencio == 'SI'){
		       ...
		       alertJquery("<div align='center'>La EPS del se&ntilde;or : <b>"+conductor+"</b> <br> tiene fecha de vencimiento <b>"+vencimiento_eps+"</b> en el sistema, el conductor fue bloqueado.<br><br> <b>no se permite asignar el manifiesto al conductor!!!</b><br><br>Contacte el area encargada de autorizar a los conductores por favor.</div>","Validacion Vencimiento EPS");
		       ...
		       alertJquery("<div align='center'>La ARP del se&ntilde;or : <b>"+conductor+"</b> <br> tiene fecha de vencimiento <b>"+vencimiento_arp+"</b> en el sistema, el conductor fue bloqueado.<br><br> <b>no se permite asignar el manifiesto al conductor!!!</b><br><br>Contacte el area encargada de autorizar a los conductores por favor.</div>","Validacion Vencimiento ARP");
...
function setDataIcaoficina() {
	var QueryString = "ACTIONCONTROLER=setDataIcaoficina";
		...
		success: function (resp) {
			try {
				...
				if (error1 != '' && error1 != 'undefined' && error1 != undefined) {
					...
					$("input[name=impuesto1]&&input[id=porcentaje]").val(porcentaje);
					$("select[name=impuesto1]&&select[id=impuesto_id]").val(impuesto_id);

				}
			} catch (e) {
				console.log(e+ ' Manifiesto');
			}
		}
...
function Contabilizar() {
	...
	var manifiesto_id = $("#manifiesto_id").val();
	var fecha_mc = $("#fecha_mc").val();
	var valor = removeFormatCurrency($("#valor_flete").val());

	if (parseInt(manifiesto_id) > 0) {
		document.getElementById('causar').disabled = true;
		/*if(!formSubmitted){	
			formSubmitted = true;	
		*/
		if (parseFloat(valor) > 0) {

			var QueryString = "ACTIONCONTROLER=getContabilizar&manifiesto_id=" + manifiesto_id + "&fecha_mc=" + fecha_mc;
				...
				success: function (response) {
					...
					try {
						if ($.trim(response) == 'true') {
							alertJquery('Manifiesto Contabilizado', 'Contabilizacion Exitosa');
							...
							formSubmitted = false;
							document.getElementById('causar').disabled = false;
						}
					} catch (e) {
						alertJquery(resp, "Error :");
						formSubmitted = false;
			...
			alertJquery('No se permite contabilizar Manifiestos en valor cero (0).');
			if ($('#causar')) document.getElementById('causar').disabled = true;

		}
	} else {
		alertJquery('Debe Seleccionar primero un Manifiesto', 'Contabilizacion');
	}
}
...
function addNewRowAnticipoTercero(){
  ...
  $("input[name=addAnticipoTercero]").click(function(){
    ...
    if(this.parentNode){
		...
		$(this).replaceWith("<input type='button' name='removeAnticipoTercero' class='btn btn-success' id='removeAnticipoTercero' value=' - ' />");	
		removeAnticipoTercero();
		autocompleteTenedorAnticipo();
			  ...
			  if(parseInt(propio) == 1){
				 ...
				 QueryString += anticipos;
				  ...
				  if($.trim(fecha_pago_saldo).length > 0 && oficina_pago_saldo_id != 'NULL' && cargue_pagado_por != 'NULL' && 
					  ...
					  $(tableFletePactado).find("#ica").each(function () {
						  ...
						  $(this).find("input[name=impuesto1],select[name=impuesto1]").each(function () {

							  if (this.id == 'impuestos_manifiesto_id') {
								  impuestos1 += '&impuestos1[' + contImpuestos1 + '][impuestos_manifiesto_id]=' + this.value;
							  } else if (this.id == 'impuesto_id') {
								  impuestos1 += '&impuestos1[' + contImpuestos1 + '][impuesto_id]=' + this.value;
							  } else if (this.id == 'base') {
								  impuestos1 += '&impuestos1[' + contImpuestos1 + '][base]=' + this.value;
							  } else if (this.id == 'porcentaje') {
								  impuestos1 += '&impuestos1[' + contImpuestos1 + '][porcentaje]=' + this.value;
							  } else if (this.id == 'valor') {
								  impuestos1 += '&impuestos1[' + contImpuestos1 + '][valor]=' + this.value;
							  }
						  ...
						  if (adiciono) {
							  ...
							  $(this).find("select[name=anticipo],input[name=anticipo]").each(function () {
								  ...
								  if (this.id == 'anticipos_manifiesto_id') {
									  ...
									  anticipos += '&anticipos[' + contAnticipos + '][valor]=' + this.value;
									  //suma_anticipos = suma_anticipos + removeFormatCurrency(this.value);	
								  }
					  ...
					  /*var origen_hidden= $("#origen_hidden").val();	
					  var destino_hidden= $("#destino_hidden").val();	
					    
					  if(parseInt(origen_hidden)>0 && parseInt(destino_hidden)>0){

						  var QueryString = "ACTIONCONTROLER=setTarifa&origend_id="+origen_hidden+"&destino_id="+destino_hidden;
							...
							success    : function(resp){
							  ...
							  try{
								...
								var data  = $.parseJSON(resp);
									...
									var anticipo = data[0]['anticipo'];
									final_ant = parseFloat(valor_flete) * parseFloat(anticipo)/100;

									if(suma_anticipos>final_ant){
										  alertJquery("El valor del anticipo supera el "+anticipo+"% del valor del flete, por favor disminuya el valor del anticipo.");
										   return false;
									}
	...
	QueryString += impuestos;
	   ...
	   $("#ica").each(function () {
		   ...
		   $(this).find("input[name=impuesto1],select[name=impuesto1]").each(function () {
			   ...
			   if (this.id == 'impuestos_manifiesto_id') {
				   impuestos1 += '&impuestos1[' + contImpuesto1 + '][impuestos_manifiesto_id]=' + this.value;
			   } else if (this.id == 'impuesto_id') {
				   impuestos1 += '&impuestos1[' + contImpuesto1 + '][impuesto_id]=' + this.value;
			   } else if (this.id == 'base') {
				   impuestos1 += '&impuestos1[' + contImpuesto1 + '][base]=' + this.value;
			   } else if (this.id == 'valor') {
				   impuestos1 += '&impuestos1[' + contImpuesto1 + '][valor]=' + this.value;
			   } else if (this.id == 'porcentaje') {
				   impuestos1 += '&impuestos1[' + contImpuesto1 + '][porcentaje]=' + this.value;
			   }
	  ...
	  success    : function(resp){
		  ...
		  try{
			 ...
			 if(data['impuestos']){
			  ...
			  var impuestos1 = data['impuestos1'];
			  if (data['impuestos1']) {

				  for (var i = 0; i < impuestos1.length; i++) {

					  var impuesto_id = impuestos1[i]['impuesto_id'];
					  var valor = impuestos1[i]['valor'];
					  var base = impuestos1[i]['base'];
					  var porcentaje = impuestos1[i]['porcentaje'];

					  $("#ica").find("select[name=impuesto1]&&select[id=impuesto_id]").each(function () {

						  if (this.value == impuesto_id) {
							  var row = this.parentNode.parentNode;

							  $(row).find("input[name=impuesto1]&&input[id=valor]").val(valor);
							  $(row).find("input[name=impuesto1]&&input[id=base]").val(base);
							  $(row).find("input[name=impuesto1]&&input[id=porcentaje]").val(porcentaje);
...
function onclickCancellation(formulario){
	...
	if($("#divAnulacion").is(":visible")){
       ...
       if(ValidaRequeridos(formulario)){
	     ...
	     var QueryString = "ACTIONCONTROLER=onclickCancellation&observacion_anulacion="+observacion_anulacion+"&manifiesto="+manifiesto+"&causal_anulacion_id="+causal_anulacion_id+"&manifiesto_id="+manifiesto_id;
