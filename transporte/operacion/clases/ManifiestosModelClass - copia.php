...
//LISTA MENU
  ...
  public function GetTiposManifiesto($Conex){
    ...
    $select = "SELECT tipo_manifiesto_id AS value, tipo_manifiesto AS text FROM tipo_manifiesto ORDER BY tipo_manifiesto_id ASC";
	$result = $this  -> DbFetchAll($select,$Conex,true);
  ...
  public function cancellation($manifiesto_id,$causal_anulacion_id,$observacion_anulacion,$usuario_anulo_id,$Conex){
   ...
   $this -> Begin($Conex);
	 ...
	 $update = "UPDATE remesa SET desbloqueada = 0,estado = 'P' WHERE remesa_id IN (SELECT remesa_id FROM detalle_despacho WHERE manifiesto_id = $manifiesto_id)";
	 $this -> query($update,$Conex,true);	 	  	 
