...
final class DetalleAnticiposPlacaModel extends Db{
   ...
   public function getAnticiposPlaca($placa_id,$Conex){
      ...
      $select    = "SELECT a.*, (SELECT tipo_documento_id FROM encabezado_de_registro WHERE encabezado_registro_id=a.encabezado_registro_id)as tipo_docu FROM anticipos_placa a WHERE a.placa_id = $placa_id AND a.devolucion=0 ORDER BY a.anticipos_placa_id desc";
   ...
   public function getRegDataplaca($placa_id,$Conex){
     ...
     $select    = "SELECT (MAX(numero)+1) AS numeros FROM anticipos_placa WHERE placa_id = $placa_id AND devolucion=0 ";
  ...
  public function Delete($empresa_id,$oficina_id,$modifica,$usuario_id,$Conex){
    $this -> Begin($Conex);
	 ...
	 if(!$movimientos1>0){
		 $delete="DELETE FROM   anticipos_placa WHERE anticipos_placa_id=$anticipos_placa_id AND devolucion=0 ";
  ...
  public function save($empresa_id,$oficina_id,$modifica,$usuario_id,$Conex){
    ...
    $this -> Begin($Conex);
	 ...
	 $observaciones 		  = $this -> requestData('observaciones');
	   ...
	   $tipo_documento_id				  = $this -> requestData('tipo_doc'); 
   ...
   $select = "SELECT * FROM parametros_anticipo WHERE propio = $propio"; 
    ...
    $descripcion = "ANTICIPO $numero PLACA $placa";	
   ...
   $insert = "INSERT INTO  anticipos_placa (anticipos_placa_id,consecutivo,fecha,propio,oficina_id,placa_id,numero,conductor,conductor_id,tenedor,tenedor_id,valor,estado,forma_pago_id,cuenta_tipo_pago_id,encabezado_registro_id,numero_soporte,fecha_egreso,observaciones)VALUES 
   ($anticipos_placa_id,$consecutivo,'$fecha_egreso',$propio,$oficina_id,$placa_id,$numero,'$nombre',$conductor_id,'$tenedor',$tenedor_id,$valor,'P',$forma_pago_id,$cuenta_tipo_pago,$encabezado_registro_id,'$numero_soporte','$fecha_registro','$observaciones')";
  ...
  public function getTipoDoc($Conex){
    ...
    $select = "SELECT * FROM tipo_de_documento WHERE de_anticipo=1 ORDER BY codigo 	 ASC";
    $result = $this -> DbFetchAll($select,$Conex);	
