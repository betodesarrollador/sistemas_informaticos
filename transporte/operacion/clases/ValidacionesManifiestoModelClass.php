<?php
 ...
 class ValidacionesManifiestoModel extends Db{
  ...
  public function manifiestoEstaManifestado($manifiesto_id,$Conex){
  //echo("ingresa a validalr el manifiesto");
    $select = "SELECT estado FROM manifiesto WHERE manifiesto_id = $manifiesto_id";
 	$result = $this  -> DbFetchAll($select,$Conex);	  
	$estado = $result[0]['estado'];
	...
	if($estado == 'M' || $estado == 'L' ){
	  return true;
		...
		$select = "SELECT estado_liquidacion FROM liquidacion_despacho WHERE liquidacion_despacho_id = (SELECT 
		MAX(liquidacion_despacho_id) FROM liquidacion_despacho WHERE manifiesto_id = $manifiesto_id)";
		
		$result             = $this  -> DbFetchAll($select,$Conex);	  
		$estado_liquidacion = $result[0]['estado_liquidacion'];	
	
	    if($estado_liquidacion == 'A'){
		  return true;
		}else{
