	...
	if(is_numeric($manifiesto_id)){
      ...
      $select = "SELECT d.*, m.* FROM (SELECT (SELECT logo FROM empresa WHERE empresa_id = m.empresa_id) AS logo,(SELECT nombre FROM oficina WHERE oficina_id = m.oficina_id) AS oficina, (SELECT direccion FROM oficina WHERE oficina_id = m.oficina_id) AS direccion_oficina,(SELECT razon_social FROM tercero WHERE tercero_id = (SELECT tercero_id FROM empresa WHERE empresa_id = m.empresa_id)) AS razon_social,(SELECT sigla FROM tercero WHERE tercero_id = (SELECT tercero_id FROM empresa WHERE empresa_id = m.empresa_id)) AS sigla, (SELECT concat(numero_identificacion,'-',digito_verificacion) FROM tercero WHERE tercero_id = (SELECT tercero_id FROM empresa WHERE empresa_id = m.empresa_id)) AS  numero_identificacion_empresa,(SELECT direccion FROM tercero WHERE tercero_id = (SELECT tercero_id FROM empresa WHERE empresa_id = m.empresa_id)) AS direccion,(SELECT nombre FROM ubicacion WHERE ubicacion_id = (SELECT ubicacion_id FROM tercero WHERE tercero_id = (SELECT tercero_id FROM empresa WHERE empresa_id = m.empresa_id))) AS ciudad,
		...
		(SELECT nombre FROM ubicacion WHERE ubicacion_id = m.origen_id) AS origen,(SELECT nombre FROM ubicacion WHERE ubicacion_id = m.destino_id) 
	               AS destino,(SELECT tipo_manifiesto FROM tipo_manifiesto WHERE tipo_manifiesto_id = m.tipo_manifiesto_id) AS tipo_manifiesto,m.numero_identificacion AS numero_identificacion_conductor,
	               ...
	               (IF(descargue_pagado_por = 'RE','REMITENTE',IF(descargue_pagado_por = 'DE','DESTINATARIO',IF(descargue_pagado_por = 'CO','CONDUCTOR',IF(descargue_pagado_por = 'CI','CLIENTE','TRANSPORTADORA'))))) AS descargue_pagado_por,(SELECT SUM(valor) FROM anticipos_manifiesto WHERE manifiesto_id = m.manifiesto_id) AS valor_anticipo,manifiesto_id,empresa_id,oficina_id,manifiesto 	,tipo_manifiesto_id,modalidad,origen_id,destino_id ,aseguradora 	,poliza 	,vencimiento_poliza 	,titular_manifiesto_id 	,ciudad_titular_manifiesto_divipola 	,titular_manifiesto 	,numero_identificacion_titular_manifiesto 	,direccion_titular_manifiesto 	,telefono_titular_manifiesto 	,ciudad_titular_manifiesto 	,placa_id 	,placa 	,propio 	,placa_remolque_id 	,fecha_mc 	,fecha_entrega_mcia_mc 	,hora_entrega 	,valor_flete 	,preliquido 	,marca 	,linea 	,modelo 	,modelo_repotenciado 	,serie 	,color 	,carroceria 	,registro_nacional_carga 	,configuracion 	,peso_vacio 	,numero_soat 	,nombre_aseguradora 	,vencimiento_soat 	,placa_remolque 	,propietario 	,numero_identificacion_propietario 	,direccion_propietario 	,telefono_propietario 	,ciudad_propietario 	,tenedor 	,tenedor_id 	,numero_identificacion_tenedor 	,direccion_tenedor 	,telefono_tenedor 	,ciudad_tenedor 	,conductor_id 	,conductor 	,numero_identificacion 	,(SELECT numero_licencia_cond FROM conductor WHERE conductor_id = m.conductor_id) AS numero_licencia_cond 	,nombre 	,direccion_conductor 	,telefono_conductor 	,ciudad_conductor 	,categoria_licencia_conductor 	,fecha_pago_saldo 	,oficina_pago_saldo_id 	,lugar_pago_saldo 	,saldo_por_pagar 	,valor_neto_pagar 	,numero_precinto 	,observaciones 	,usuario_id 	,usuario_registra 	,usuario_registra_numero_identificacion,estado,id_mobile, numero_contenedor,fecha_estimada_salida,hora_estimada_salida
