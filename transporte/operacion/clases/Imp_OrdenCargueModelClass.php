...
final class Imp_OrdenCargueModel extends Db{
  ...
  public function getOrdenCargue($Conex){
	...
	if(is_numeric($orden_cargue_id)){
		...
		(SELECT  CONCAT(foto,'?',RAND())  AS fotos FROM conductor WHERE conductor_id =o.conductor_id) AS foto_conductor,
		(SELECT  CONCAT(archivo_imagen_frontal,'?',RAND())  AS archivo_imagen_frontals  FROM vehiculo WHERE placa_id=o.placa_id) AS foto_vehiculo,		
		(SELECT tipo_servicio FROM tipo_servicio WHERE tipo_servicio_id=o.tipo_servicio_id) AS tipo_servicio,
		o.fecha,o.hora,o.producto,o.peso,o.peso_volumen,o.cantidad_cargue,o.placa,o.placa_remolque,o.marca,o.linea,o.modelo,o.modelo_repotenciado,o.propietario,o.numero_identificacion_propietario,o.direccion_propietario,o.telefono_propietario,
		...
		o.numero_identificacion,o.nombre,o.direccion_conductor,o.telefono_conductor,o.movil_conductor,o.ciudad_conductor,o.categoria_licencia_conductor, 	    
		o.numero_licencia_cond, 
		(SELECT numero_identificacion FROM remitente_destinatario WHERE remitente_destinatario_id=o.remitente_id) AS nit_remitente,	
		(SELECT telefono FROM remitente_destinatario WHERE remitente_destinatario_id=o.remitente_id) AS telefono_remitente,	
		(SELECT direccion FROM remitente_destinatario WHERE remitente_destinatario_id=o.remitente_id) AS direccion_remitente,	
		(SELECT CONCAT_WS(' - ',nombre,primer_apellido,segundo_apellido)AS remitente FROM remitente_destinatario WHERE remitente_destinatario_id=o.remitente_id) AS nombre_remitente,
		(SELECT numero_identificacion FROM remitente_destinatario WHERE remitente_destinatario_id=o.destinatario_id) AS nit_destintario,	
		(SELECT telefono FROM remitente_destinatario WHERE remitente_destinatario_id=o.destinatario_id) AS telefono_destintario,	
		(SELECT direccion FROM remitente_destinatario WHERE remitente_destinatario_id=o.destinatario_id) AS direccion_destintario,	
		(SELECT CONCAT_WS(' - ',nombre,primer_apellido,segundo_apellido)AS destinatario FROM remitente_destinatario WHERE remitente_destinatario_id=o.destinatario_id) AS nombre_destintario,
		...
		(SELECT TRIM(medida) FROM medida WHERE medida_id=o.unidad_volumen_id) AS unidad_volumen,
		 ...
		 o.estado, o.observaciones, CONCAT('http://162.241.4.183../../../imagenes/transporte/manifiesto/',REPLACE(((SELECT concat_ws(' ',primer_nombre,segundo_nombre,primer_apellido,segundo_apellido) FROM tercero WHERE tercero_id = (SELECT tercero_id FROM usuario WHERE usuario_id = o.usuario_id))),' ','_'),'.JPG') as firma_desp
