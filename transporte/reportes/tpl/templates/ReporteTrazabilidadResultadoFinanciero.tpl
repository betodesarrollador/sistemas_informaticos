		...
		<table>
			...
			<tbody>
				...
				{section name=rows loop=$DETALLESTRAZABILIDAD|count}
					<tr bgcolor="{cycle values="#eeeeee,#d0d0d0"}">
						...
						<td style="border: 1px solid">
							{if $DETALLESTRAZABILIDAD[$k].estado eq 'AN'}
								{$DETALLESTRAZABILIDAD[$k].valfactura|number_format:0:',':'.'}
							{else}
								{$DETALLESTRAZABILIDAD[$k].valfactura|number_format:0:',':'.'}
							{/if}
						</td>
						...
						<td style="border: 1px solid">
							...
							{else}{$DETALLESTRAZABILIDAD[$k].valor_relacion_pago|number_format:0:',':'.'}
							{/if}
						</td>
						...
						<td style="border: 1px solid">
							{if $DETALLESTRAZABILIDAD[$k].estado eq 'AN'}0
							{else}{$DETALLESTRAZABILIDAD[$k].valor_orden_compra|number_format:0:',':'.'}
							{/if}
						</td>
					...
					{assign var="total_util" value=$total_util+$tot_util}
				...
				<tr bgcolor="{cycle values="#eeeeee,#d0d0d0"}">
					...
					<td style="border: 1px solid"></td>
					<td style="border: 1px solid"></td>
					<td style="border: 1px solid">{$total_facturas|number_format:0:',':'.'}</td>
					<td style="border: 1px solid"></td>
					<td style="border: 1px solid"></td>
					<td style="border: 1px solid"></td>
					...
					<td style="border: 1px solid"></td>
					<td style="border: 1px solid"></td>
					<td style="border: 1px solid">{$total_planillas|number_format:0:',':'.'}</td>
					<td style="border: 1px solid"></td>
					<td style="border: 1px solid"></td>
					<td style="border: 1px solid">{$total_oc|number_format:0:',':'.'}</td>

					<td style="border: 1px solid">{assign var=tot_costo value=$total_planillas+$total_oc}{$tot_costo|number_format:0:',':'.'}</td>

					<td style="border: 1px solid">{*assign var=tot_util value=$tot_util*}{$total_util|number_format:0:',':'.'}</td>
					
					{assign var=util value=$total_util/$total_facturas}
					<td style="border: 1px solid">{assign var=per_util value=$util*100}{$per_util|number_format:2:',':'.'}%</td>
				</tr>
			</tbody>
		</table>
	</body>
