...
final class reporteFletesGeneradores extends Controler{
	...
	public function Main(){
		...
		require_once("reporteFletesGeneradoresLayoutClass.php");
		require_once("reporteFletesGeneradoresModelClass.php");

		$Layout   = new reporteFletesGeneradoresLayout($this -> getTitleTab(),$this -> getTitleForm());
		$Model    = new reporteFletesGeneradoresModel();
	...
	protected function OnclickGenerar(){
		...
		$Layout                 = new reporteFletesGeneradoresLayout();
		$Model                  = new reporteFletesGeneradoresModel();	
		...
		if ($cliente_id == 'NULL') {
			...
			$condicion_cliente=' ANd r.remesa_id IN (SELECT dd.remesa_id FROM detalle_despacho dd, remesa r WHERE r.cliente_id = '.$cliente_id.' AND dd.remesa_id = r.remesa_id)';
			if ($estado_id=='MC') {
				$array = $Model -> getReporte4($oficina_id,$desde,$hasta,$condicion_cliente,$Conex);
			}
			if ($estado_id=='DU') {
		...
		if($download == 'true'){
			$Layout -> exportToExcel('reporteFletesGeneradoresResultadoFinancieroExcel.tpl');
		}else{
			$Layout -> RenderLayout('reporteFletesGeneradoresResultadoFinanciero.tpl');
		}
	}
...
$reporteFletesGeneradores = new reporteFletesGeneradores();
