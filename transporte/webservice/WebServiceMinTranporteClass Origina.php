 ...
 class WebServiceMinTransporte{
	...
	public function setOSoapClient($Conex){
	 ...
	 //para nuevo cambio fin
	  ...
	  $h = @get_headers('http://rndcws.mintransporte.gov.co:8080/ws/svr008w.dll/wsdl/IBPMServices');
	  $status = array();
      ...
      $this -> oSoapClient   = new soapclient($this -> wsdl, true);
	  $this -> ruta_archivos = $_SERVER['DOCUMENT_ROOT'].'../../../archivos/transporte/xml_webservice_mintransporte/';

      if ($sError = $this -> oSoapClient->getError()) {	  
        echo "No se pudo realizar la operación [" . $sError . "]";       	   
      }else{
	  	  
	      $this -> username                = "GMT@0878";
		  $this -> password                = "RNDC0878";
		  $this -> ambiente                = "R";
		  $this -> numnitempresatransporte = "8301019592";
		  $this -> Conex                   = $Conex;
	...
	public function sendConductorMintransporte($data,$path_xml = NULL,$printMsj = true){
		...
		$respuesta   = $this -> oSoapClient->call('AtenderMensajeRNDC',$aParametros); 
		...
		if ($this -> oSoapClient->fault) { 		 
		 ...
		 if($printMsj)echo $respuesta;
		  ...
		  $sError = $this -> oSoapClient->getError();
		  ...
		  if ($sError) { 
		    ...
		    if($printMsj)echo $respuesta;
	           ...
	           if(preg_match("/ingresoid/i","$respuesta")){
                 ...
                 require($_SERVER['DOCUMENT_ROOT']."../../../framework/clases/xml2array.php");
						...
						if(preg_match("/ingresoid/i","$respuesta")){
							...
							if(preg_match("/ya fue registrado/i","$respuesta")){
							   ...
							   if(preg_match("/DUPLICADO:/i","$respuesta")){
										...
										$Model -> setAprobacionManifiesto($manifiesto_id,$informacion_viaje,$respuesta_final,$this -> Conex);	 
